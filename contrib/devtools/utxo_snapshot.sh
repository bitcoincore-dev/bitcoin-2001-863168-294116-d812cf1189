...
if (( $# < 3 )); then
  ...
  echo 'Examples:'
  echo
  echo "  utxo_snapshot.sh 570000 utxo.dat ./src/bitcoin-cli -datadir=\$(pwd)/testdata"
  echo '  utxo_snapshot.sh 570000 - ./src/bitcoin-cli'
  exit 1
fi
...
(>&2 echo "Rewinding chain back to height ${GENERATE_AT_HEIGHT}; this may take a while")
${BITCOIN_CLI_CALL} -rpcclienttimeout=9999999 invalidateblock "${PIVOT_BLOCKHASH}"
...
${BITCOIN_CLI_CALL} -rpcclienttimeout=9999999 reconsiderblock "${PIVOT_BLOCKHASH}"
