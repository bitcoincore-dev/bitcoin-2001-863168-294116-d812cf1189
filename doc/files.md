...
2. The data directory path can be specified by `-datadir` option.
...
`./`               | `ip_asn.map`          | [ASN](https://en.wikipedia.org/wiki/Autonomous_system_%28Internet%29) mapping used for bucketing of the peers; path can be specified with the `-asmap` option
