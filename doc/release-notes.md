...
Bitcoin Knots version 0.14.0.knots20170227.rc2 is now available from:

  <https://bitcoinknots.org/files/0.14.x/0.14.0.knots20170227.rc2/>
...
- #9504 `342a564` *dumpmasterprivkey command (achow101)
- #9524 `88883ae` *rpc: Don't FlushStateToDisk when pruneblockchain(0) (MarcoFalke)
- #9622 `0a084dd` *listsinceblock: optionally find and list any transactions that were undone due to reorg when requesting a non-main chain block in a new 'removed' array. (Karl-Johan Alm)
- #9672 `c781a04` *RPC/rawtransaction: createrawtransaction: Check opt_into_rbf when provided with either value (luke-jr)
- n/a   `3d5d3eb` *RPC: Use options object rather than adding a "sort" boolean for multisig methods (luke-jr)
...
- #9503 `c60de5d` *Add address filtering to listreceivedbyaddress (JeremyRubin)
- #9571 `9b9b685` *RPC: getblockchaininfo: BIP9 stats (Matthew Zipkin)
...
- #9749 `93e903b` *If -spkreuse=0, ensure transactions in mempool always have unique scriptPubKeys (luke-jr)
...
- #9495 `f5daff7` *Fix CCheckQueue IsIdle (potential) race condition and remove dangerous constructors. (JeremyRubin)
- n/a   `07de932` *Add a new checkpoint at block 451,840 (luke-jr)
...
- #9481 `c5adf8f` *[Qt] Show more significant warning if we fall back to the default fee (jonasschnelli)
- #9500 `6d8fe35` *'help' rpc commands autocomplete (achow101)
- #9592 `c4e4792` *[Qt] Add simple optin-RBF checkbox and confirmation info (jonasschnelli)
- #9724 `50c5657` *Qt/Intro: Storage shouldn't grow significantly with pruning enabled (luke-jr)
- #9724 `f6d18f5` *Qt/Intro: Explain a bit more what will happen first time (luke-jr)
- #9849 `c282e7a` *Qt: Network Watch tool (luke-jr)
- n/a   `8c4fd22` *Qt/Options: Configure walletrbf using rwconf (luke-jr)
- n/a   `d9cf68b` *Qt/Options: Configure blockreconstructionextratxn using rwconf (luke-jr)
- n/a   `4ae7989` *Qt/Options: Configure incrementalrelayfee using rwconf (luke-jr)
- n/a   `677243b` *Qt/Options: Configure dustrelayfee using rwconf (luke-jr)
- n/a   `8048c3b` *Qt/Options: Configure blockmintxfee using rwconf (luke-jr)
- n/a   `2b1e6bf` *Qt/Intro: Configure autoprune using rwconf (luke-jr)
- n/a   `afc8b5f` *Qt/Options: When resetting options, re-assign prune if it was configured via Intro (luke-jr)
- n/a   `b0a7f76` *Qt/Options: Configure spkreuse using rwconf (luke-jr)
...
- #8775 `d58b9e2` *Wallet: Sanitise -wallet parameter (luke-jr)
...
- #9359 `d29538d` *Add test for CWalletTx::GetImmatureCredit() returning stale values. (ryanofsky)
- #9497 `96c7f2c` *Add CheckQueue Tests (JeremyRubin)
- #9622 `5127a06` *Testing: listsinceblock should display all transactions that were affected since the given block, including transactions that were removed due to a reorg. (Karl-Johan Alm)
- #9503 `e6f053a` *Add tests of listreceivedbyaddress address filtering (JeremyRubin)
...
- #9672 `bc2e702` *bitcoin-tx: rbfoptin: Avoid touching nSequence if the value is already opting in (luke-jr)
- #9422 `3b0ee5f` *Store mempool min fee state in mempool.dat (luke-jr)
