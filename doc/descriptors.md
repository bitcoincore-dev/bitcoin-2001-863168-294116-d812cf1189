...
lockstep, rather than all combinations of child keys from each wildcard path.
...
provided, which takes as input a public key, and matches the P2PK,
P2PKH, P2WPKH, and P2SH-P2WPH scripts for that key. In case the key is
uncompressed, it only matches the P2PK and P2PKH scripts.

### Output Descriptors vs. Addresses

Like a bitcoin address, an bitcoin output descriptor can be used as a human
readable represention of a scriptPubKey that a given wallet knows how to spend.
But despite similarities between output descriptors and addresses, output
descriptors are not an appropriate mechanism for requesting payments because
they:

1. May reduce security and anonymity by revealing more key information than a
   sender needs to know to send a payment.
