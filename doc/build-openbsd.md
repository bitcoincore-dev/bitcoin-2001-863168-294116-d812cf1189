...
...
to: /path/to/bitcoin/depends/amd64-unknown-openbsd[release-number]
```

Then set `BDB_PREFIX`:

```bash
export BDB_PREFIX="/path/to/bitcoin/depends/amd64-unknown-openbsd[release-number]"
```
...
```bash
cmake -S .. -DWITH_SQLITE=ON -DWITH_GUI=Qt5
```

Run `cmake -S .. -LH` to see the full list of available options.
...
ctest            # Use "-j N" for N parallel tests. Some tests are disabled if Python 3 is not available.
```
