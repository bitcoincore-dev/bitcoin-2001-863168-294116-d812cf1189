...
if [[ $DOCKER_NAME_TAG == centos* ]]; then
  ...
  ${CI_RETRY_EXE} DOCKER_EXEC yum -y install $DOCKER_RPM_PACKAGES $PACKAGES
elif [ "$CI_USE_APT_INSTALL" != "no" ]; then
  ${CI_RETRY_EXE} DOCKER_EXEC apt-get update
  ${CI_RETRY_EXE} DOCKER_EXEC apt-get install --no-install-recommends --no-upgrade -y $DOCKER_DEB_PACKAGES $PACKAGES
fi
