...
if [[ $HOST = *-mingw32 ]]; then
  ...
  CI_EXEC make "$MAKEJOBS" -C src univalue/test/object.exe univalue/test/unitester.exe VERBOSE=1
...
if [ -n "$QEMU_USER_CMD" ]; then
  ...
  CI_EXEC make "$MAKEJOBS" -C src univalue/test/object univalue/test/unitester VERBOSE=1
