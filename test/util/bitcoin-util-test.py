...
def bctester(testDir, input_basename, buildenv, component_conf):
    ...
    for testObj in input_data:
        ...
        if not component_conf.getboolean('ENABLE_UTIL_%s' % (m.group(1).upper(),)):
    ...
    if failed_testcases:
        ...
        if skipped_testcases:
            logging.warning('%s tests skipped because %s is not built' % (len(skipped_testcases), skipped_testcase_deps))
        sys.exit(0)
