...
class MultiWalletTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        if self.variant == "rpc":
            node = lambda: self.nodes[0].rpc
            wallet = lambda name: self.nodes[0].get_wallet_rpc(name)
        else:
            node = lambda: self.nodes[0].cli
            wallet = lambda name: node()("-rpcwallet={}".format(name))
