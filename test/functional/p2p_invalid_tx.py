...
class InvalidTxRequestTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        node.p2p.send_txs_and_test([tx_orphan_include_by_block_A], node, success=False)
        ...
        with node.assert_debug_log(["Erased 1 orphan tx included or conflicted by block"]):
            node.p2p.send_blocks_and_test([block_A], node, success=True)
        ...
        self.log.info('Send the orphan ... ')
        node.p2p.send_txs_and_test([tx_orphan_conflict_by_block_B], node, success=False)
        ...
        with node.assert_debug_log(["Erased 1 orphan tx included or conflicted by block"]):
            node.p2p.send_blocks_and_test([block_B], node, success=True)
