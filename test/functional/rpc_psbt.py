...
class PSBTTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        assert_equal(signed_tx, self.nodes[0].walletprocesspsbt(psbtx, {"finalize": False})['psbt'])
        finalized_tx = self.nodes[0].walletprocesspsbt(psbt=psbtx, finalize=True)['psbt']
        assert_equal(finalized_tx, self.nodes[0].walletprocesspsbt(psbtx, {"finalize": True})['psbt'])
