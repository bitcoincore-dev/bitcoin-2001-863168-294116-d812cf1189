...
class MultiWalletTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        if self.options.descriptors:
            exp_stderr = f"Error: SQLiteDatabase: Unable to obtain an exclusive lock on the database, is it being used by another {self.config['environment']['PACKAGE_NAME']}?"
        else:
        ...
        if self.options.descriptors:
            assert_raises_rpc_error(-4, f"Wallet file verification failed. SQLiteDatabase: Unable to obtain an exclusive lock on the database, is it being used by another {self.config['environment']['PACKAGE_NAME']}?", self.nodes[0].loadwallet, wallet_names[0])
        else:
