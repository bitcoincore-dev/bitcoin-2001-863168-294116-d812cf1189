...
class RecoveryAuthorization:
    ...
    """
    script: t.Union[CScript, bytes] = CScript()
...
class VaultsTest(BitcoinTestFramework):
    ...
    def test_revault(
        ...
        for mutation, err in mutation_to_err.items():
            ...
            unvault_tx = get_trigger_tx([spec], presig_tx_mutator=mutation)
    ...
    def get_spend_wit_stack(self, *args, **kwargs) -> t.List[bytes]:
        return [
            *self.script_witness_stack,
            self.script,
            self.tr_info.controlblock_for_script_spend('only-path'),
        ]
...
class UnvaultSpec:
    ...
    def __init__(
        ...
        self.trigger_tr = taproot_from_privkey(
            ...
            scripts=[
                ('recover', example_vault.recover_script),
