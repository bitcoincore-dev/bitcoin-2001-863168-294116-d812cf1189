    ...
    def do_import(self, timestamp):
        ...
        if self.call == Call.single:
            if self.data == Data.address:
                response = self.node.importaddress(self.address["address"], self.label, self.rescan != Rescan.no)
            elif self.data == Data.pub:
                response = self.node.importpubkey(self.address["pubkey"], self.label, self.rescan != Rescan.no)
            elif self.data == Data.priv:
                response = self.node.importprivkey(self.key, self.label, self.rescan != Rescan.no)
...
class ImportRescanTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        for variant in IMPORT_VARIANTS:
            expected_rescan = variant.rescan == Rescan.yes or (variant.rescan == Rescan.late_timestamp and
                                                               variant.call == Call.single)
            variant.node = self.nodes[1 if expected_rescan else 2]
