...
public:
    explicit CChainState(BlockManager& blockman, uint256 from_snapshot_blockhash = uint256(), int script_check_worker_threads = 0);
...
public:
    ...
    //!                                         worker threads.
    CChainState& InitializeChainstate(const uint256& snapshot_blockhash = uint256(), int script_check_worker_threads = 0)
        EXCLUSIVE_LOCKS_REQUIRED(::cs_main);
