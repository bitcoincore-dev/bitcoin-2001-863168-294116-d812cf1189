...
protected:
    ...
    //! Whether chainstate has been fully validated locally starting from the
    //! genesis block. False if is from an assumeutxo snapshot chainstate that
    //! has not been validated yet. True if this is an assumeutxo snapshot
    //! chainstate that has been validated, or a normal non-assumeutxo
    //! chainstate.
    bool m_validated;
...
{
private:
    //! List of chainstates. Chainstates in this list can be disabled once they
    //! are added, but they can't be deleted, because Chainstate pointers can be
    //! used when cs_main is not held. (Some Chainstate methods such as
    //! ActiveBestChain and InvalidateBlock even require cs_main not to be held
    //! when they are called, so there is no way to prevent these methods from
    //! being called while deleting a chainstate.) This could be fixed by using a new lock with broader scope than cs_main
...
public:
    ...
    //! Find the most-work chain which is either fully-validated, or is
    //! assumed-valid based on an assumeutxo snapshot.
    Chainstate& ActiveChainstate(bool require_validated = false) const
    {
        LOCK(::cs_main);
        Chainstate* active{nullptr};
        for (auto& chainstate : m_chainstates) {
           if ((!require_validated || chainstate->m_validated) &&
               (!active || active->m_chain.Tip()->nChainWork < chainstate->m_chain.Tip()->nChainWork)) {
               active = chainstate.get();
           }
        }
        return *Assert(active);
    }
    ...
    Chainstate* DetectSnapshotChainstate(CTxMemPool* mempool) EXCLUSIVE_LOCKS_REQUIRED(::cs_main);

    void ResetChainstates() EXCLUSIVE_LOCKS_REQUIRED(::cs_main);

    //! Remove the chainstate and all on-disk artifacts.
    //! Used when reindex{-chainstate} is called during snapshot use.
    [[nodiscard]] bool DeleteChainstate() EXCLUSIVE_LOCKS_REQUIRED(::cs_main);
    ...
    std::optional<std::pair<const CBlockIndex*, const CBlockIndex*>> GetHistoricalBlockRange() EXCLUSIVE_LOCKS_REQUIRED(::cs_main);
