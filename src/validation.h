...
public:
    ...
    //!
    //! @param[in] max_coins_cache_size_bytes   If not specified, defaults to global value.
    //! @param[in] max_mempool_size_bytes       If not specified, defaults to global value.
    //!
    //! @return the state of the size of the coins cache.
    CoinsCacheSizeState GetCoinsCacheSizeState(
        const CTxMemPool& tx_pool,
        size_t max_coins_cache_size_bytes = -1,
        int64_t max_mempool_size_bytes = -1) EXCLUSIVE_LOCKS_REQUIRED(::cs_main);
