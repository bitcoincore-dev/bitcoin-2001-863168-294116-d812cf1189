...
static bool rest_getutxos(const std::any& context, HTTPRequest* req, const std::string& strURIPart)
{
    ...
    uint256 active_hash;
    {
        auto process_utxos = [&vOutPoints, &outs, &hits, &active_height, &active_hash, &chainman](const CCoinsView& view, const CTxMemPool& mempool) EXCLUSIVE_LOCKS_REQUIRED(::cs_main) {
