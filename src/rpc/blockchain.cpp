...
bool FindScriptPubKey(std::atomic<int>& scan_progress, const std::atomic<bool>& should_abort, int64_t& count, CCoinsViewCursor* cursor, const std::set<CScript>& needles, std::map<COutPoint, Coin>& out_results, NodeContext& node)
{
    scan_progress = 0;
    count = 0;
    while (cursor->Valid()) {
...
UniValue scantxoutset(const JSONRPCRequest& request)
{
    ...
    if (request.params[0].get_str() == "status") {
        ...
        NodeContext& node = EnsureNodeContext(request.context);
        bool res = FindScriptPubKey(g_scan_progress, g_should_abort_scan, count, pcursor.get(), needles, coins, node);
...
UniValue dumptxoutset(const JSONRPCRequest& request)
{
...

    {
        ...
        if (!GetUTXOStats(&::ChainstateActive().CoinsDB(), stats, node.rpc_interruption_point)) {
            throw JSONRPCError(RPC_INTERNAL_ERROR, "Unable to read UTXO set");
        }
