...
static void SoftForkDescPushBack(const CBlockIndex* blockindex, UniValue& softforks, const Consensus::Params& consensusParams, Consensus::DeploymentPos id)
{
    ...
    if (g_versionbitscache.BIP(bip, bip_version, consensusParams, id)) {
        bip9.pushKV("bip", bip);
        bip9.pushKV("bip_version", bip_version);
    }
    bip9.pushKV("start_time", consensusParams.vDeployments[id].nStartTime);
    bip9.pushKV("timeout", consensusParams.vDeployments[id].nTimeout);
    bip9.pushKV("period", (int64_t)consensusParams.nMinerConfirmationWindow);

    // BIP9 status
    const auto since{g_versionbitscache.StateSinceHeight(blockindex->pprev, consensusParams, id)};
    ...
    if (since == 0 && current_state != ThresholdState::DEFINED) {
        ...
        case ThresholdState::STARTED:
            bip9.pushKV("signal_activate", strprintf("%08x", consensusParams.vDeployments[id].signal_activate));
            [[fallthrough]];
...
const std::vector<RPCResult> RPCHelpForDeployment{
    ...
    {
        {RPCResult::Type::NUM, "bip", "the bip id for this soft fork"},
        {RPCResult::Type::NUM, "bip_version", "the version of the bip (0-255)"},
