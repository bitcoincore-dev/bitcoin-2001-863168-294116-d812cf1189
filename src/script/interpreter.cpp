            ...
            switch (opcode)
            {
                ...
                case OP_VAULT:
                {
                    ...
                    const auto& [is_recov_sweep, err] =
                        checker.CheckVaultSpendToRecoveryOutputs(
                            execdata, recovery_params, recovery_spk, flags);
                ...
                case OP_UNVAULT:
                {
                    ...
                    if (is_recov_sweep) {
                        ...
                        if (recovery_spk.size() > 0) {
                            ...
                            if (!VerifyNestedWitnessProgram(
                                    stack, recovery_spk, serror, flags, checker, limit_recursion)) {
                                // The above sets serror if necessary.
                                // TODO: think about wrapping this with a vault-specific
                                // error indicating that recovery auth has failed.
                                return false;
                            }
    ...
    unsigned int flags) const
{
    ...
    if (flags & SCRIPT_VERIFY_VAULT_REPLACEABLE_RECOVERY &&
        ...
        return {true, SCRIPT_ERR_VAULT_RECOVERY_NOT_REPLACEABLE};
    }
    ...
    // output is an ephemeral anchor.
    if (!is_authed_recovery) {
        const unsigned int exp_ea_idx{vout_idx == 0 ? 1U : 0U};
        if (vout.size() > 2 || !IsOutputEphemeralAnchor(vout[exp_ea_idx])) {
            return {true, SCRIPT_ERR_VAULT_BAD_RECOVERY_OUTPUTS};
        }
    } else {
...
static const valtype GetExpectedUnvaultTriggerWitV1Program(
    const valtype& recovery_params,
