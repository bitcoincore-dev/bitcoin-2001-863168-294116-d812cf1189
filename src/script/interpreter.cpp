...
bool EvalScript(std::vector<std::vector<unsigned char> >& stack, const CScript& script, unsigned int flags, const BaseSignatureChecker& checker, SigVersion sigversion, ScriptExecutionData& execdata, ScriptError* serror)
{
    ...
    const auto GetVoutIdxFromStack = [&](
            std::optional<size_t> idx_out,
            const size_t stack_idx) -> bool {
        auto out{0};
        try {
            out = CScriptNum(stacktop(stack_idx), fRequireMinimal).GetInt64();
        } catch (...) {
            return false;
        }
        if (idx_out < 0) {
            ...
            idx_out = std::make_optional(static_cast<size_t>(out));
        }
        return true;
    };
        ...
        for (; pc < pend; ++opcode_pos) {
            ...
            switch (opcode)
            {
                ...
                case OP_VAULT_RECOVER:
                {
                    ...
                    std::optional<size_t> recovery_vout_idx;
                    if (!GetVoutIdxFromStack(recovery_vout_idx, -2) || !recovery_vout_idx) {
    ...
    unsigned int flags) const
{
    ...
    if (num_vout <= static_cast<size_t>(recovery_vout_idx)) {
    ...
    if (flags & SCRIPT_VERIFY_VAULT_UNAUTH_RECOVERY_STRUCTURE && !is_authed_recovery) {
        ...
        if (num_vout > 2 ||
                (num_vout == 2 && !IsOutputEphemeralAnchor(vout[optional_ea_idx]))) {
            return SCRIPT_ERR_VAULT_BAD_RECOVERY_OUTPUTS;
        }
    }
