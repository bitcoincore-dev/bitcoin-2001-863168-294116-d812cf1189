    ...
    unsigned int flags) const
{
    ...
    const bool is_authed_recovery{executing_script.size() > (uint256::WIDTH + 1)};
    ...
    ScriptError* serror) const
{
    ...
    if (!is_unvault_output_wit || trigger_witversion < 1) {
        ...
        if (value_out.scriptPubKey.size() != 32) {
            return SCRIPT_ERR_UNVAULT_MISMATCH;
        }
        const XOnlyPubKey q{value_out.scriptPubKey};
        ...
        if (!q.CheckTapTweak(*execdata.m_internal_key, merkle_root, control[0] & 1)) {
            return SCRIPT_ERR_UNVAULT_MISMATCH;
        }
    }
    ...
    if (revault_out_idx != -1) {
        if (static_cast<size_t>(revault_out_idx) > vout.size()) {
