...
// Run CheckInputs (using CoinsCache()) on the given transaction, for all script
...
static void ValidateCheckInputsForAllFlags(const CTransaction &tx, uint32_t failing_flags, bool add_to_cache) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    for (uint32_t test_flags=0; test_flags < (1U << 16); test_flags += 1) {
        ...
        bool ret = CheckInputs(tx, state, ::ChainstateActive().CoinsCache(), true, test_flags, true, add_to_cache, txdata, nullptr);
        ...
        if (ret && add_to_cache) {
            ...
            BOOST_CHECK(CheckInputs(tx, state, ::ChainstateActive().CoinsCache(), true, test_flags, true, add_to_cache, txdata, &scriptchecks));
...
BOOST_FIXTURE_TEST_CASE(checkinputs_test, TestChain100Setup)
{
    ...
    // spend_tx is invalid according to DERSIG
    {
        ...
        BOOST_CHECK(!CheckInputs(CTransaction(spend_tx), state, ::ChainstateActive().CoinsCache(), true, SCRIPT_VERIFY_P2SH | SCRIPT_VERIFY_DERSIG, true, true, ptd_spend_tx, nullptr));

        // If we call again asking for scriptchecks (as happens in
        // ConnectBlock), we should add a script check object for this -- we're
        // not caching invalidity (if that changes, delete this test case).
        std::vector<CScriptCheck> scriptchecks;
        BOOST_CHECK(CheckInputs(CTransaction(spend_tx), state, ::ChainstateActive().CoinsCache(), true, SCRIPT_VERIFY_P2SH | SCRIPT_VERIFY_DERSIG, true, true, ptd_spend_tx, &scriptchecks));
    ...
    LOCK(cs_main);
    BOOST_CHECK(::ChainActive().Tip()->GetBlockHash() == block.GetHash());
    BOOST_CHECK(::ChainstateActive().CoinsCache()->GetBestBlock() == block.GetHash());
    ...
    // Test CHECKLOCKTIMEVERIFY
    {
        ...
        BOOST_CHECK(CheckInputs(CTransaction(invalid_with_cltv_tx), state, ::ChainstateActive().CoinsCache(), true, SCRIPT_VERIFY_CHECKLOCKTIMEVERIFY, true, true, txdata, nullptr));
    }
    ...
    // TEST CHECKSEQUENCEVERIFY
    {
        ...
        BOOST_CHECK(CheckInputs(CTransaction(invalid_with_csv_tx), state, ::ChainstateActive().CoinsCache(), true, SCRIPT_VERIFY_CHECKSEQUENCEVERIFY, true, true, txdata, nullptr));
    }
...

    {
        ...
        BOOST_CHECK(!CheckInputs(CTransaction(tx), state, ::ChainstateActive().CoinsCache(), true, SCRIPT_VERIFY_P2SH | SCRIPT_VERIFY_WITNESS, true, true, txdata, nullptr));

        std::vector<CScriptCheck> scriptchecks;
        // Make sure this transaction was not cached (ie because the first
        // input was valid)
        BOOST_CHECK(CheckInputs(CTransaction(tx), state, ::ChainstateActive().CoinsCache(), true, SCRIPT_VERIFY_P2SH | SCRIPT_VERIFY_WITNESS, true, true, txdata, &scriptchecks));
