    ...
    for (unsigned int i = 0; i < NUM_SIMULATION_ITERATIONS; i++) {
        ...
        if (InsecureRandRange(100) == 0) {
            ...
            if (stack.size() > 1 && InsecureRandBool() == 0) {
                ...
                BOOST_CHECK(stack[flushIndex]->Flush(should_erase));
    ...
    bool do_erasing_flush)
{
    ...
    auto flush_all = [&all_caches](bool erase) {
        ...
        for (auto i = all_caches.rbegin(); i != all_caches.rend(); ++i) {
            (*i)->SetBestBlock(InsecureRand256());  // Hack to allow flushing.
            (*i)->Flush(erase);
        }
    };
    ...
    all_caches[0]->Flush(/*erase*/ false);
    BOOST_CHECK(base.HaveCoin(outp));
    BOOST_CHECK(all_caches[0]->HaveCoin(outp));
    ...
    flush_all(true); // Erase all cache content.

    // --- Bonus check 2: ensure that a FRESH coin is deleted by Flush(erase=false)
    //
    txid = InsecureRand256();
    outp = COutPoint(txid, 0);
    coin = MakeCoin();
