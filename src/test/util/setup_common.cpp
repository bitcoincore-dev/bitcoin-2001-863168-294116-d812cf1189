    ...
    : BasicTestingSetup(chainName, extra_args)
{
    ...
    CalculateCacheSizes(m_args, m_cache_sizes);

    m_node.chainman = std::make_unique<ChainstateManager>();
    m_node.chainman->m_blockman.m_block_tree_db = std::make_unique<CBlockTreeDB>(m_cache_sizes.block_tree_db_cache_size, true);
    ...
    : ChainTestingSetup(chainName, extra_args)
{
    ...
    auto rv = LoadChainstateSequence(fReindex.load(),
                                     *Assert(m_node.chainman.get()),
                                     Assert(m_node.mempool.get()),
                                     fPruneMode,
                                     chainparams.GetConsensus(),
                                     m_args.GetBoolArg("-reindex-chainstate", false),
                                     m_cache_sizes.block_tree_db_cache_size,
                                     m_cache_sizes.coin_db_cache_size,
                                     m_cache_sizes.coin_cache_usage_size,
