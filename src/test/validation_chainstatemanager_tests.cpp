...
BOOST_FIXTURE_TEST_CASE(chainstatemanager_activate_snapshot, TestChain100Setup)
{
    ...
    constexpr int initial_total_coins{100};
    ...
    // Make some initial assertions about the contents of the chainstate.
    {
        ...
        initial_usage = ibd_coinscache.DynamicMemoryUsage();
        int total_coins{0};
    ...
    //
    fs::path snapshot_path = m_path_root / "test_snapshot.dat";
    FILE* outfile{fsbridge::fopen(snapshot_path, "wb")};
    CAutoFile auto_outfile{outfile, SER_DISK, CLIENT_VERSION};

    UniValue result = CreateUTXOSnapshot(m_node, chainman.ActiveChainstate(), auto_outfile);
    BOOST_TEST_MESSAGE(
        "Wrote UTXO snapshot to " << snapshot_path.native() << ": " << result.write());
    ...
    assert(chainman.ActivateSnapshot(&auto_infile, metadata, /*in_memory*/ true));
    ...
    // reflects the expected content.
    {
        ...
        for (CChainState* chainstate : chainman.GetAll()) {
            ...
            BOOST_CHECK_EQUAL(0, coinscache.GetCacheSize());
    ...
    constexpr int new_coins{100};
    mineBlocks(new_coins);  // Defined in TestChain100Setup.

    {
        LOCK(::cs_main);
        int coins_in_active{0};
        int coins_in_ibd{0};
        int coins_missing_ibd{0};
