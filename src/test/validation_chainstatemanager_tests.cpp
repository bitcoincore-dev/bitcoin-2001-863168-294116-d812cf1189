...
BOOST_AUTO_TEST_CASE(chainstatemanager)
{
    ...
    CChainState& c2 = WITH_LOCK(::cs_main, return manager.ActivateExistingSnapshot(
...
BOOST_AUTO_TEST_CASE(chainstatemanager_rebalance_caches)
{
    ...
    // Create a snapshot-based chainstate.
    //
    CChainState& c2 = WITH_LOCK(cs_main, return manager.ActivateExistingSnapshot(&mempool, GetRandHash()));
    chainstates.push_back(&c2);
    c2.InitCoinsDB(
        /*cache_size_bytes=*/1 << 23, /*in_memory=*/true, /*should_wipe=*/false);
...
struct SnapshotTestSetup : TestChain100Setup {
    ...
    std::tuple<CChainState*, CChainState*> SetupSnapshot()
    {
        ...
        CChainState& snapshot_chainstate = chainman.ActiveChainstate();
...
{
    ChainstateManager& chainman = *Assert(m_node.chainman);
    CChainState& bg_chainstate = chainman.ActiveChainstate();
    ...
    CTxMemPool* mempool = bg_chainstate.GetMempool();
    {
    ...
    mineBlocks(10);
    {
        ...
        // chainstate.
        for (CChainState* cs : chainman_restarted.GetAll()) {
