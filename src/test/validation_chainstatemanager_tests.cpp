...
BOOST_AUTO_TEST_CASE(chainstatemanager)
{
    ...
    CChainState& c2 = WITH_LOCK(::cs_main, return manager.ActivateSnapshotChainstate(
...
BOOST_AUTO_TEST_CASE(chainstatemanager_rebalance_caches)
{
    ...
    // Create a snapshot-based chainstate.
    //
    CChainState& c2 = WITH_LOCK(cs_main, return manager.ActivateSnapshotChainstate(&mempool, GetRandHash()));
    chainstates.push_back(&c2);
    c2.InitCoinsDB(
        /*cache_size_bytes=*/1 << 23, /*in_memory=*/true, /*should_wipe=*/false);
...
BOOST_FIXTURE_TEST_CASE(chainstatemanager_snapshot_completion, SnapshotTestSetup)
{
    ...
    BOOST_CHECK(chainman.IsSnapshotActive());
    const uint256 snapshot_tip_hash = chainman.ActiveTip()->GetBlockHash();
...
BOOST_FIXTURE_TEST_CASE(chainstatemanager_loadblockindex, TestChain100Setup)
{
    ...
    CChainState& cs2 = WITH_LOCK(::cs_main,
        return chainman.ActivateSnapshotChainstate(&mempool, GetRandHash()));
