...
struct FakeCheck {
    std::vector<DeferredCheck>* m_deferred_checks{nullptr};
...
struct FakeGeneratorCheck {
    std::vector<TestDeferredCheck>* m_deferred_checks{nullptr};
    int param{-1};
    ...
    bool operator()()
    {
        m_deferred_checks = new std::vector<TestDeferredCheck>();
