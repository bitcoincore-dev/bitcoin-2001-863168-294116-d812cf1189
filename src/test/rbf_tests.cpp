...
BOOST_FIXTURE_TEST_CASE(rbf_helper_functions, TestChain100Setup)
{
    ...
    BOOST_CHECK(EntriesAndTxidsDisjoint({entry2}, {{tx2->GetWitnessHash(), true}}, unused_txid, &violates_policy) == std::nullopt);
    BOOST_CHECK(!violates_policy);
    BOOST_CHECK(EntriesAndTxidsDisjoint({entry2}, {{tx2->GetHash(), true}}, unused_txid, &violates_policy).has_value());
    BOOST_CHECK(!violates_policy);
    ...
    BOOST_CHECK(EntriesAndTxidsDisjoint({entry2}, {{tx1->GetHash(), true}}, unused_txid, &violates_policy) == std::nullopt);
    BOOST_CHECK(!violates_policy);
