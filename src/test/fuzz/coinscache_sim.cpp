...
FUZZ_TARGET(coinscache_sim)
{
    ...
    while (it != buffer.end()) {
        ...
        case 4: { // AddCoin (only possible_overwrite if necessary)
            uint32_t outpointidx = (it == buffer.end()) ? 0 : *(it++);
            coinidx_type coinidx = (it == buffer.end()) ? 0 : *(it++);
        ...
        case 5: { // AddCoin (always possible_overwrite)
            uint32_t outpointidx = (it == buffer.end()) ? 0 : *(it++);
            coinidx_type coinidx = (it == buffer.end()) ? 0 : *(it++);
            // Invoke on real caches.
            Coin coin = data.coins[coinidx];
        ...
        case 6: { // SpendCoin (moveto = nullptr)
            uint32_t outpointidx = (it == buffer.end()) ? 0 : *(it++);
            // Invoke on real caches.
        ...
        case 8: // Add a cache level (if not already at the max).
        ...
        case 9: // Remove a cache (if we have more than 1).
        ...
        case 14: // GetCacheSize
            (void)caches.back()->GetCacheSize();
            break;

        case 15: // DynamicMemoryUsage
            (void)caches.back()->DynamicMemoryUsage();
            break;

        case 16: { // Decrement height
            uint32_t step = (it == buffer.end()) ? 0 : *(it++);
    ...
    // Compare the bottom coinsview (not CCoinsViewCache) with sim_cache[0].
    for (uint32_t outpointidx = 0; outpointidx < NUM_OUTPOINTS; ++outpointidx) {
        Coin realcoin;
