...
{
    const auto foo = std::make_pair("-foo", ArgsManager::TYPE_OPTIONAL_STRING);
    const auto bar = std::make_pair("-bar", ArgsManager::TYPE_OPTIONAL_STRING);
    SetupArgs({foo, bar});
    ResetArgs("");
...
{
    const auto foo = std::make_pair("-foo", ArgsManager::ALLOW_ANY);
    const auto bar = std::make_pair("-bar", ArgsManager::TYPE_BOOL);
    SetupArgs({foo, bar});
    ResetArgs("-nofoo");
    BOOST_CHECK(!gArgs.GetBoolArg("-foo", true));
    BOOST_CHECK(!gArgs.GetBoolArg("-foo", false));
