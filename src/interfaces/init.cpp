...
public:
    ...
    {
        if (g_config.make_node) return g_config.make_node(*this);
        int fd = SpawnProcess(m_exe_path, "bitcoin-node");
        auto init = g_config.connect(g_config.exe_name, fd);
    ...
    std::unique_ptr<ChainClient> makeWalletClient(Chain& chain, std::vector<std::string> wallet_filenames) override
    {
        ...
        // Subprocess no lnger needed after wallet client is discarded.
...
bool StartServer(int argc, char* argv[], int& exit_status, Init& init)
{
    ...
    try {
        ...
        g_config.listen(g_config.exe_name, fd, init);
