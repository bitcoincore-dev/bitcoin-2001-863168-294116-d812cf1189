#ifndef BITCOIN_INTERFACES_CAPNP_PROXY_IO_H
#define BITCOIN_INTERFACES_CAPNP_PROXY_IO_H
...
public:
    ...
    void startAsyncThread(std::unique_lock<std::mutex>& lock) EXCLUSIVE_LOCKS_REQUIRED(m_mutex);

    //! Add/remove remote client reference counts.
    void addClient(std::unique_lock<std::mutex>& lock) EXCLUSIVE_LOCKS_REQUIRED(m_mutex);
    void removeClient(std::unique_lock<std::mutex>& lock) EXCLUSIVE_LOCKS_REQUIRED(m_mutex);
...
struct Connection
{
    Connection(EventLoop& loop, int fd, bool add_client)
        : m_loop(loop),
...
ProxyClientBase<Interface, Impl>::~ProxyClientBase() noexcept
{
    ...
    if (m_connection) {
        ...
        m_connection->m_loop.sync([&]() {
            ...
            {
                WAIT_LOCK(m_connection->m_loop.m_mutex, lock);
...
template <typename Interface, typename Impl>
ProxyServerBase<Interface, Impl>::ProxyServerBase(Impl& impl, EventLoop& loop)
    : m_impl(&impl), m_owned(false), m_connection(nullptr)
{
    AddClient(loop);
}
...
#endif // BITCOIN_INTERFACES_CAPNP_PROXY_IO_H
