...
interface Chain $X.proxy("interfaces::Chain") {
    lock @0 (tryLock :Bool, context :Context $X.count(0)) -> (result :ChainLock) $X.async;
    assumeLocked @1 () -> (result :ChainLock);
    findBlock @2 (hash :Data, wantBlock :Bool, wantTime :Bool, wantMaxTime :Bool, context :Context $X.count(0)) -> (block :Data, time :Int64, maxTime :Int64, result :Bool) $X.async;
...
interface ChainClient $X.proxy("interfaces::Chain::Client") {
...
interface Wallet $X.proxy("interfaces::Wallet") {
    ...
    abortRescan @6 () -> ();
    backupWallet @7 (filename :Text) -> (result :Bool);
    getWalletName @8 () -> (result :Text);
    getKeyFromPool @9 (internal :Bool) -> (pubKey :Data, result :Bool);
    getPubKey @10 (address :Data) -> (pubKey :Data, result :Bool);
    getPrivKey @11 (address :Data) -> (key :Key, result :Bool);
    isSpendable @12 (dest :TxDestination) -> (result :Bool);
    haveWatchOnly @13 () -> (result :Bool);
    setAddressBook @14 (dest :TxDestination, name :Text, purpose :Text) -> (result :Bool) $X.async;
    delAddressBook @15 (dest :TxDestination) -> (result :Bool);
    getAddress @16 (dest :TxDestination, wantName :Bool, wantIsMine :Bool) -> (name :Text, isMine :Int32, result :Bool);
    getAddresses @17 () -> (result :List(WalletAddress));
    learnRelatedScripts @18 (pubKey: Data, outputType :Int32) -> ();
    addDestData @19 (dest :TxDestination, key :Text, value :Text) -> (result :Bool);
    eraseDestData @20 (dest :TxDestination, key :Text) -> (result :Bool);
    getDestValues @21 (prefix :Text) -> (result :List(Data));
    lockCoin @22 (output :Data) -> ();
    unlockCoin @23 (output :Data) -> ();
    isLockedCoin @24 (output :Data) -> (result :Bool);
    listLockedCoins @25 () -> (outputs :List(Data));
    createTransaction @26 (recipients :List(Recipient), coinControl :CoinControl, sign :Bool, changePos :Int32) -> (changePos :Int32, fee :Int64, failReason :Text, result :PendingWalletTx);
    transactionCanBeAbandoned @27 (txid :Data) -> (result :Bool);
    abandonTransaction @28 (txid :Data) -> (result :Bool);
    transactionCanBeBumped @29 (txid :Data) -> (result :Bool);
    createBumpTransaction @30 (txid :Data, coinControl :CoinControl, totalFee :Int64) -> (errors :List(Text), oldFee :Int64, newFee :Int64, mtx :Data, result :Bool);
    signBumpTransaction @31 (mtx :Data) -> (mtx :Data, result :Bool);
    commitBumpTransaction @32 (txid :Data, mtx :Data) -> (errors :List(Text), bumpedTxid :Data, result :Bool);
    getTx @33 (txid :Data) -> (result :Data);
    getWalletTx @34 (txid :Data) -> (result :WalletTx);
    getWalletTxs @35 () -> (result :List(WalletTx));
    tryGetTxStatus @36 (txid :Data) -> (txStatus :WalletTxStatus, numBlocks :Int32, adjustedTime :Int64, result :Bool);
    getWalletTxDetails @37 (txid :Data) -> (txStatus :WalletTxStatus, orderForm :List(Pair(Text, Text)), inMempool :Bool, numBlocks :Int32, adjustedTime :Int64, result :WalletTx);
    getBalances @38 () -> (result :WalletBalances);
    tryGetBalances @39 () -> (balances :WalletBalances, numBlocks :Int32, result :Bool);
    getBalance @40 () -> (result :Int64);
    getAvailableBalance @41 (coinControl :CoinControl) -> (result :Int64);
    txinIsMine @42 (txin :Data) -> (result :Int32);
    txoutIsMine @43 (txout :Data) -> (result :Int32);
    getDebit @44 (txin :Data, filter :Int32) -> (result :Int64);
    getCredit @45 (txout :Data, filter :Int32) -> (result :Int64);
    listCoins @46 () -> (result :List(Pair(TxDestination, List(Pair(Data, WalletTxOut)))));
    getCoins @47 (outputs :List(Data)) -> (result :List(WalletTxOut));
    hdEnabled @48 () -> (result :Bool);
    getDefaultAddressType @49 () -> (result :Int32);
    getDefaultChangeType @50 () -> (result :Int32);
    handleShowProgress @51 (callback :ShowWalletProgressCallback) -> (result :Handler);
    handleStatusChanged @52 (callback :StatusChangedCallback) -> (result :Handler);
    handleAddressBookChanged @53 (callback :AddressBookChangedCallback) -> (result :Handler);
    handleTransactionChanged @54 (callback :TransactionChangedCallback) -> (result :Handler);
    handleWatchOnlyChanged @55 (callback :WatchOnlyChangedCallback) -> (result :Handler);
}
...
struct GlobalArgs $X.proxy("interfaces::capnp::GlobalArgs") {
