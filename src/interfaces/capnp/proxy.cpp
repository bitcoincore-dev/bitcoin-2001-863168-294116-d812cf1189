...
EventLoop::EventLoop(const char* exe_name, std::thread&& thread)
    : m_exe_name(exe_name), m_init(nullptr), m_io_context(kj::setupAsyncIo()), m_thread(std::move(thread))
{
    LogIpc(*this, "EventLoop %p\n", this);
    int fds[2];
...
ProxyServer<Init>::ProxyServer(EventLoop& loop) : m_init_loop(loop) {}

kj::Promise<void> ProxyServer<Init>::makeThread(MakeThreadContext context)
{
    Waiter* waiter = new Waiter(m_init_loop);
    std::string from = context.getParams().getName();
    {
        ...
        waiter->m_result = std::async([waiter, from, this]() {
            ...
            std::unique_lock<std::mutex> lock(waiter->m_mutex);
            waiter->m_name = ThreadName(m_init_loop.m_exe_name) + " (from " + from + ")";
            waiter->wait(lock, [waiter] { return !waiter->m_result.valid(); });
        });
    }
    auto thread = kj::heap<ProxyServer<Thread>>(*waiter);
    auto thread_client = m_init_loop.m_threads.add(kj::mv(thread));
