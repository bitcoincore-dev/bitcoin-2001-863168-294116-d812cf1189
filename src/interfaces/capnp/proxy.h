...
class ProxyServerMethod
{
    template<typename ProxyServer, typename Method, typename Params, typename Results, typename Args>
    static auto invoke(ProxyServer& server, Method method, typename ParamsArgs&&... args) -> AUTO_RETURN( ((server.m_impl).*m_method)(std::forward<Args>(args)...) )
};

template <typename Params>
class ProxyClientMethod
{
