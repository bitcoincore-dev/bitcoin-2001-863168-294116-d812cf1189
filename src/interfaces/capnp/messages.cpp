...
void ProxyServerCustom<messages::Init, Init>::invokeMethod(MakeWalletClientContext context)
{
    ...
    {
        LOCK(args.cs_args);
        ReadField(TypeList<GlobalArgs>(), *this, MakeValueInput(args_param), args);
    }
    ...
    results.setResult(kj::heap<ProxyServer<messages::ChainClient>>(wallet.release(), true /* owned */, *m_loop));
}
...
void ProxyServerCustom<messages::Chain, Chain>::invokeMethod(HandleNotificationsContext context)
{
    ...
    auto results = context.getResults();
    results.setResult(kj::heap<ProxyServer<messages::Handler>>(handler.release(), true /* owned */, *m_loop));
}
