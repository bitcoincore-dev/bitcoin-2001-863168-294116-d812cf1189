...
void FillGlobalArgs(messages::GlobalArgs::Builder&& builder)
{
    ...
    using FUCK = int;
    BuildField(TypeList<decltype(args.mapArgs)>(), TypeList<FUCK>(), BuildFieldPriority(), *loop, args.mapArgs,
        Make<FieldBuilder>(builder, ProxyStruct<messages::GlobalArgs>::getArgs()));
    BuildField(TypeList<decltype(args.mapMultiArgs)>(), TypeList<FUCK>(), BuildFieldPriority(), *loop,
        args.mapMultiArgs, Make<FieldBuilder>(builder, ProxyStruct<messages::GlobalArgs>::getMultiArgs()));
}

kj::Promise<void> ProxyServerCustom<messages::Chain, Chain>::invokeMethod(LockContext context)
{
    auto future = kj::newPromiseAndFulfiller<void>();
    m_lock_thread.post(MakeAsyncCallable(
        kj::mvCapture(future.fulfiller,
            kj::mvCapture(context, [this](LockContext context, kj::Own<kj::PromiseFulfiller<void>> fulfiller) {
                          auto params = context.getParams();
                          Context call_context;
                          ReadField(TypeList<Context>(), *this, MakeValueInput(params.getContext()), call_context);
                          if (m_locked && *m_locked != call_context) {
                              return false;
                          }
                          m_locked = call_context;
                          auto results = context.getResults();
                          results.setResult(kj::heap<ProxyServer<messages::ChainLock>>(m_impl->lock(), *m_loop, this));
                          m_loop->fulfillPromise(*fulfiller, []() {});
                          return true;
                      }))));
