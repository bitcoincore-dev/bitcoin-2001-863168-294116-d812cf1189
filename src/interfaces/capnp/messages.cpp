    ...
    std::vector<std::string> wallet_filenames)
{
    ...
    interfaces->chain = MakeUnique<ProxyClient<messages::Chain>>(params.getChain(), *context.proxy_server.m_loop);
...
public:
    ...
    {
        RPCTimerBase* result;
        m_loop.checkSync([&] { result = new RpcTimer(m_loop, fn, millis); });
        return result;
    }
    EventLoop& m_loop;
};
