...
void ReadMessage(messages::NodeStats::Reader const& reader, std::tuple<CNodeStats, bool, CNodeStateStats>& node_stats)
{
    ...
    std::nullptr_t proxy_fixme;

    auto&& node = std::get<0>(node_stats);
    ReadField(TypeList<decltype(node)>(), proxy_fixme, MakeValueInput(reader), node);
    if ((std::get<1>(node_stats) = reader.hasStateStats())) {
        auto&& state = std::get<2>(node_stats);
        ReadField(TypeList<decltype(state)>(), proxy_fixme, MakeValueInput(reader.getStateStats()), state);
    }
}
...
void BuildMessage(const CCoinControl& coin_control, messages::CoinControl::Builder&& builder)
{
    ...
    if (coin_control.m_signal_rbf) {
        builder.setHasSignalRbf(true);
        builder.setSignalRbf(*coin_control.m_signal_rbf);
    }
...
void ReadMessage(const messages::CoinControl::Reader& reader, CCoinControl& coin_control)
{
    ...
    if (reader.getHasSignalRbf()) {
        coin_control.m_signal_rbf = reader.getSignalRbf();
    }
...
void ProxyServerCustom<messages::Init, Init>::invokeMethod(MakeWalletClientContext context)
{
    ...
    ReadField(
        TypeList<decltype(wallet_filenames)>(), *this, MakeValueInput(params.getWalletFilenames()), wallet_filenames);

    auto&& args_param = params.getGlobalArgs();
    auto& args = static_cast<Args&>(::gArgs);
    {
        LOCK(args.cs_args);
        ReadField(TypeList<decltype(args.mapArgs)>(), *this, MakeValueInput(args_param.getArgs()), args.mapArgs);
        ReadField(TypeList<decltype(args.mapMultiArgs)>(), *this, MakeValueInput(args_param.getMultiArgs()),
            args.mapMultiArgs);
    }
...
void FillGlobalArgs(EventLoop& loop, messages::GlobalArgs::Builder&& builder)
{
    const auto& args = static_cast<const Args&>(::gArgs);
    LOCK(args.cs_args);
    BuildField(TypeList<decltype(args.mapArgs)>(), BuildFieldPriority(), loop, args.mapArgs,
        Make<FieldOutput>(builder, ProxyStruct<messages::GlobalArgs>::getArgs()));
    BuildField(TypeList<decltype(args.mapMultiArgs)>(), BuildFieldPriority(), loop, args.mapMultiArgs,
        Make<FieldOutput>(builder, ProxyStruct<messages::GlobalArgs>::getMultiArgs()));
}

void ProxyServerCustom<messages::Chain, Chain>::invokeMethod(HandleNotificationsContext context)
{
    auto params = context.getParams();
...
void ProxyServerCustom<messages::Chain, Chain>::invokeMethod(HandleRpcContext context)
{
    auto params = context.getParams();
    ...
    ReadField(TypeList<decltype(actor)>(), *this, MakeValueInput(command.getActor()), actor);
    std::vector<std::string> args;
    ReadField(TypeList<decltype(args)>(), *this, MakeValueInput(command.getArgNames()), args);
...
void ProxyServerCustom<messages::ChainClient, Chain::Client>::invokeMethod(StartContext context)
{
    if (!m_scheduler) {
...
void ProxyServerCustom<messages::Node, Node>::invokeMethod(RpcSetTimerInterfaceIfUnsetContext context)
{
...
void ProxyServerCustom<messages::Node, Node>::invokeMethod(RpcUnsetTimerInterfaceContext context)
{
...
{
    using S = ProxyServer<interfaces::capnp::messages::Chain>;
...
{
    using S = ProxyServer<interfaces::capnp::messages::ChainNotifications>;
    return kj::heap<MainServer<S>>(impl, loop);
}
