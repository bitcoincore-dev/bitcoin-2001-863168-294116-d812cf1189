...
ProxyServerCustom<messages::ChainClient, ChainClient>::ProxyServerCustom(ChainClient* impl,
    ...
    : ProxyServerBase(impl, owned, connection), m_timer_interface(MakeUnique<RpcTimerInterface>(connection.m_loop))
{
...
{
    self().resendWalletTransactions(locked_chain, best_block_time);
}
