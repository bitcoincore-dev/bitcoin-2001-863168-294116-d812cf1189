...
template <typename... LocalTypes, typename Proxy, typename Value, typename Output>
...
void BuildField(TypeList<std::string>, Priority<1>, Proxy& proxy, Value&& value, Output&& output)
{
    auto result = output.set(value.size());
    memcpy(result.begin(), value.data(), value.size());
}

template <typename Proxy, typename Value, typename FnR, typename... FnParams, typename Output>
void BuildField(TypeList<std::function<FnR(FnParams...)>>, Priority<1>, Proxy& proxy, Value&& value, Output&& output)
{
    ...
    output.set(kj::heap<ProxyServer<Interface>>(new Callback(std::move(value)), true /* owned */, *proxy.m_loop));
}

template <typename LocalType, typename Proxy, typename Value, typename Output>
void BuildField(TypeList<LocalType*>, Priority<3>, Proxy& proxy, Value&& value, Output&& output)
{
    if (value) {
        output.setWant();
        // FIXME std::move probably wrong
        BuildField(TypeList<LocalType>(), BuildFieldPriority(), proxy, std::move(*value), output);
    }
}

template <typename LocalType, typename Proxy, typename Value, typename Output>
void BuildField(TypeList<std::shared_ptr<LocalType>>, Priority<1>, Proxy& proxy, Value&& value, Output&& output)
{
    if (value) {
        BuildField(TypeList<LocalType>(), BuildFieldPriority(), proxy, *value, output);
    }
}

template <typename LocalType, typename Proxy, typename Value, typename Output>
void BuildField(TypeList<std::vector<LocalType>>, Priority<1>, Proxy& proxy, Value&& value, Output&& output)
{
    // FIXME dedup with set handler below
    auto list = output.set(value.size());
    size_t i = 0;
    for (auto& elem : value) {
        BuildField(TypeList<LocalType>(), BuildFieldPriority(), proxy, std::move(elem),
...
template <typename LocalType, typename Proxy, typename Value, typename Output>
void BuildField(TypeList<std::set<LocalType>>, Priority<1>, Proxy& proxy, Value&& value, Output&& output)
{
    // FIXME dededup with vector handler above
    auto list = output.set(value.size());
    size_t i = 0;
    for (const auto& elem : value) {
        BuildField(TypeList<LocalType>(), BuildFieldPriority(), proxy, elem,
            ListOutput<typename decltype(list)::Builds>(list, i));
        ++i;
    }
}

template <typename KeyLocalType, typename ValueLocalType, typename Proxy, typename Value, typename Output>
    ...
    Output&& output)
{
    ...
    size_t i = 0;
    for (const auto& elem : value) {
        BuildField(TypeList<std::pair<KeyLocalType, ValueLocalType>>(), BuildFieldPriority(), proxy, elem,
            ListOutput<typename decltype(list)::Builds>(list, i));
        ++i;
    }
}
...
::capnp::Void BuildPrimitive(Proxy& proxy, Value&&, TypeList<::capnp::Void>)
{
    return {};
}

template <typename LocalType, typename Proxy, typename Value>
LocalType BuildPrimitive(Proxy& proxy,
    const Value& value,
    TypeList<LocalType>,
...
template <typename Proxy, typename Impl, typename Client>
Client BuildPrimitive(Proxy& proxy, std::unique_ptr<Impl>&& impl, TypeList<Client>)
{
    using Interface = typename Client::Calls;
    return kj::heap<ProxyServer<Interface>>(impl.release(), true /* owned */, *proxy.m_loop);
}

template <typename LocalType, typename Proxy, typename Output>
void BuildField(TypeList<LocalType&>,
    Priority<1>,
    Proxy& proxy,
    LocalType&& value,
    Output&& output,
    typename Output::Type::Calls* enable = nullptr)
{
    using Interface = typename Output::Type::Calls;
    output.set(kj::heap<ProxyServer<Interface>>(&value, false /* owned */, *proxy.m_loop));
}

template <typename LocalType, typename Proxy, typename Value, typename Output>
void BuildField(TypeList<boost::optional<LocalType>>, Priority<1>, Proxy& proxy, Value&& value, Output&& output)
{
    if (value) {
...
void BuildField(TypeList<std::exception>, Priority<1>, Proxy& proxy, const std::exception& value, Output&& output)
{
    BuildField(TypeList<std::string>(), BuildFieldPriority(), proxy, std::string(value.what()), output);
}
...
{
    auto kv = output.set();
    BuildField(TypeList<KeyLocalType>(), BuildFieldPriority(), proxy, value.first, KeyOutput<decltype(kv)>{kv});
    BuildField(TypeList<ValueLocalType>(), BuildFieldPriority(), proxy, value.second, ValueOutput<decltype(kv)>{kv});
}
...
{
    auto kv = output.set();
    BuildField(TypeList<KeyLocalType>(), BuildFieldPriority(), proxy, std::get<0>(value), KeyOutput<decltype(kv)>{kv});
    BuildField(
        TypeList<ValueLocalType>(), BuildFieldPriority(), proxy, std::get<1>(value), ValueOutput<decltype(kv)>{kv});
}

template <typename LocalType, typename Proxy, typename Value, typename Output>
void BuildField(TypeList<const LocalType>, Priority<0>, Proxy& proxy, Value&& value, Output&& output)
{
    BuildField(TypeList<LocalType>(), BuildFieldPriority(), proxy, std::forward<Value>(value), output);
}
...
template <typename LocalType, typename Proxy, typename Value, typename Output>
void BuildField(TypeList<LocalType&&>, Priority<0>, Proxy& proxy, Value&& value, Output&& output)
{
    BuildField(TypeList<LocalType>(), BuildFieldPriority(), proxy, std::forward<Value>(value), output);
}
...
void BuildField(TypeList<LocalType>, Priority<0>, Proxy& proxy, Value&& value, Output&& output)
{
    output.set(BuildPrimitive(proxy, std::forward<Value>(value), TypeList<typename Plain<Output>::Type>()));
}

template <size_t index, typename LocalType, typename Proxy, typename Value, typename Output>
    ...
    typename std::enable_if < index<ProxyType<LocalType>::fields>::type * enable = nullptr)
{
    ...
    auto&& field_value = value.*ProxyType<LocalType>::get(Index());
    BuildField(TypeList<decltype(field_value)>(), BuildFieldPriority(), proxy, field_value, field_output);
    BuildOne<index + 1>(param, proxy, value, output);
}
...
void BuildField(TypeList<LocalType> local_type,
    ...
    typename ProxyType<LocalType>::Struct* enable = nullptr)
{
    BuildOne<0>(local_type, proxy, value, output.set());
}
...
bool PassField(TypeList<LocalType*>, Proxy& proxy, Input&& input, Output&& output, Fn&& fn, FnParams&&... fn_params)
{
    ...
    if (want) BuildField(TypeList<LocalType>(), BuildFieldPriority(), proxy, *param, output);
    return true;
}
...
bool PassField(TypeList<LocalType>, Proxy& proxy, Input&& input, Output&& output, Fn&& fn, FnParams&&... fn_params)
{
    ...
    BuildField(TypeList<LocalType>(), BuildFieldPriority(), proxy, *param, output);
    return true;
}
...
struct ClientParam
{
    ...
    struct BuildParams : FieldChainHelper<BuildParams, sizeof...(Types)>
    {
        ...
        void handleField(Proxy& proxy, Params& params, ParamList)
        {
            CallBuild<0>(ParamList(), proxy, params, Priority<1>());
        }

        template <size_t I, typename FieldParams, typename Proxy, typename Params, typename... Values>
            ...
            Values&&... values)
        {
            ...
            CallBuild<I + 1>(field_params, proxy, params, priority, std::forward<Values>(values)...,
                std::move(std::get<I>(m_client_param->m_values)));
        }

        template <size_t I, typename FieldParams, typename Proxy, typename Params, typename... Values>
        void CallBuild(FieldParams field_params, Proxy& proxy, Params& params, Priority<0>, Values&&... values)
        {
            using Output = FieldOutput<Params, Accessor>;

            BuildField(field_params, BuildFieldPriority(), proxy, std::forward<Values>(values)...,
...
struct ServerFieldRet
{
        ...
        FwdParams&&... fwd_params) const
    {
        ...
        BuildField(FieldType(), BuildFieldPriority(), server, std::move(result), Output(results, m_accessor));
        return true;
    }
    Accessor m_accessor;
};
...
struct ServerFieldExcept
{
        ...
        FwdParams&&... fwd_params) const
    {
        ...
        try {
            ...
            BuildField(FieldType(), BuildFieldPriority(), server, e, Output(results, m_accessor));
        }
        return true;
    }
    Accessor m_accessor;
...
void clientInvoke(MethodTraits, const GetRequest& get_request, ProxyClient& proxy_client, _Params&&... params)
{
    ...
    proxy_client.m_loop->sync([&]() {
        ...
        FieldChain().handleChain(proxy_client, request, MethodParams(), typename _Params::BuildParams{&params}...);
        ...
        if (proxy_client.m_loop->m_debug) {
            printf("============================================================\n");
            printf("Client Request %s %p pid %i\n%s\n",
                ::capnp::Schema::from<typename Request::Params>().getShortDisplayName().cStr(), &proxy_client,
                getpid(), request.toString().flatten().cStr());
        }
        if (waiting_thread == proxy_client.m_loop->m_thread_id) BreakPoint();
        KJ_ASSERT(waiting_thread != proxy_client.m_loop->m_thread_id,
            ...
            request.send().then([&](::capnp::Response<typename Request::Results>&& response) {
                if (proxy_client.m_loop->m_debug) {
                    ...
                    printf("Client Response %s %p pid %i\n%s\n",
                        ::capnp::Schema::from<typename Request::Results>().getShortDisplayName().cStr(), &proxy_client,
                        getpid(), response.toString().flatten().cStr());
                }
                ...
                try {
                    FieldChain().handleChain(
                        proxy_client, response, MethodParams(), typename _Params::ReadResults{&params}...);
                } catch (...) {
                    promise.set_exception(std::current_exception());
                    return;
                }
