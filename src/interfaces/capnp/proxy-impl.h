    ...
    typename std::enable_if<std::is_same<decltype(output.init()), Context::Builder>::value>::type* enable = nullptr)
{
    ...
    if (!remote_thread) {
        ...
        printf("&&&&&&&&&&&&&&&& m_thread_map.makeThread %i\n", getpid());
    ...
    Output&& output)
{
    if (value) {
        ...
        output.set(kj::heap<ProxyServer<Interface>>(new Callback(std::move(value)), true /* owned */, invoke_context.loop));
    }
}
...
void clientInvoke(ProxyClient& proxy_client, const GetRequest& get_request, FieldObjs&&... fields)
{
    ...
    proxy_client.m_loop->sync([&]() {
        ...
        // CallWithTupleParams(Make<ClientBuild>(proxy_client, request), fields.m_values...);
        ...
        proxy_client.m_loop->m_task_set.add(
            request.send().then([&](::capnp::Response<typename Request::Results>&& response) {
                ...
                // method definitino, and whatever else, or fix/replace
                // CallWithTupleParams(Make<ClientRead>(proxy_client, response), fields.m_values...);
                try {
