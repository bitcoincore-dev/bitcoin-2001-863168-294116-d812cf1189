...
struct PairValueEmplace
{
    ...
    void operator()(KeyTuple&& key_tuple)
    {
        ...
        ReadField(TypeList<ValueLocalType>(), m_context, Make<StructField, ValueAccessor>(pair),
            BindTuple(Make<Compose>(Get<1>(), Bind(m_emplace, std::piecewise_construct, key_tuple))));
    }
};
    ...
    typename std::enable_if < index<ProxyType<LocalType>::fields>::type * enable = nullptr)
{
    ...
    auto&& field_value = value.*ProxyType<LocalType>::get(Index());
    BuildField(
        TypeList<Decay<decltype(field_value)>>(), BuildFieldPriority(), invoke_context, field_value, field_output);
    ...
    -> Require<typename decltype(Accessor::get(server_context.call_context.getParams()))::Calls>
{
    ...
    const auto& input = Make<StructField, Accessor>(params);
    using Interface = typename Decay<decltype(input.get())>::Calls;
    auto param = MakeUnique<ProxyClient<Interface>>(input.get(), *server_context.proxy_server.m_connection);
...
struct ServerRet : Parent
{
    ...
    void invoke(ServerContext& server_context, TypeList<>, Args&&... args) const
    {
        ...
        auto&& results = server_context.call_context.getResults();
        InvokeContext& invoke_context = server_context;
        BuildField(TypeList<decltype(result)>(), BuildFieldPriority(), invoke_context,
            std::forward<decltype(result)>(result), Make<StructField, Accessor>(results));
    }
};
...
void clientDestroy(Client& client)
{
    if (client.m_connection)
        LogIpc(client.m_connection->m_loop, "IPC client destroy %s\n", typeid(client).name());
    else
...
{
    LogIpc(server.m_connection->m_loop, "IPC server destroy %s\n", typeid(server).name());
}
...
void clientInvoke(ProxyClient& proxy_client, const GetRequest& get_request, FieldObjs&&... fields)
{
    if (!proxy_client.m_connection) {
        LogPrintf("IPC error: clientInvoke call made after disconnect");
    ...
    proxy_client.m_connection->m_loop.sync([&]() {
        ...
        proxy_client.m_connection->m_loop.m_task_set->add(request.send().then(
            ...
            [&](const ::kj::Exception& e) {
                ...
                LogPrint(::BCLog::IPC, "{%s} IPC client exception %s\n", invoke_context.thread_context.thread_name,
                    kj_exception);
                std::unique_lock<std::mutex> lock(invoke_context.thread_context.waiter->m_mutex);
                done = true;
                invoke_context.thread_context.waiter->m_cv.notify_all();
    ...
    if (!kj_exception.empty()) throw IpcException(kj_exception);
}
...
kj::Promise<void> serverInvoke(Server& server, CallContext& call_context, Fn fn)
{
    ...
    int req = ++server_reqs;
    LogIpc(server.m_connection->m_loop, "IPC server recv request  #%i %s %s\n", req,
        TypeName<typename Params::Reads>(), params.toString().flatten().cStr());
    ...
    try {
        ...
        return ReplaceVoid([&]() { return fn.invoke(server_context, ArgList()); },
            ...
            .then([&server, req](CallContext call_context) {
                LogIpc(server.m_connection->m_loop, "IPC server send response #%i %s %s\n", req, TypeName<Results>(),
                    call_context.getResults().toString().flatten().cStr());
            });
    } catch (...) {
        LogIpc(server.m_connection->m_loop, "IPC server unhandled exception %s\n",
