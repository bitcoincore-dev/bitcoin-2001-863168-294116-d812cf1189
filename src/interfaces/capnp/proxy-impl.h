...
public:
    ...
    template <typename Callable>
    NODISCARD bool sync(Callable&& callable)
    {
        return post(std::ref(callable));
    }

    template <typename Callable>
    void checkSync(Callable&& callable)
    {
        bool success = post(std::ref(callable));
        assert(success);
    }
...
//! makes an callback into the client in the middling a request, while client
//! thread is blocked waiting for server response, this is what allows client to
//! run the request in the same thread, exactly like a native single-thread
//! callback.
struct Waiter
{
    Waiter(EventLoop& loop) : m_loop(loop) {}

    ~Waiter() { assert(!m_thread.joinable()); }
    ...
    std::thread m_thread; //!< Thread object, only set if this is a thread created with makeThread.
};
...
{
public:
    ProxyServer(ThreadContext& context, bool join_thread);
    ~ProxyServer();
...
struct ThreadContext
{
    ...
    //! in case it needs to make callbacks into the client that execute on the
    //! client thread while it is waiting. This will be set to a local thread
    //! object.
    ...
    //! by makeThread. If client call is being made from a server thead in the
    //! middle of processing a client request, this will be set to the the
    //! `callbackThread` remote thread argument passed in that request.
    ...
    ~ThreadContext()
    {
        ...
        for (auto& thread : remote_threads) {
            if (thread.second) {
                thread.first->checkSync([&] { auto client = std::move(*thread.second); });
            }
        }
    }
};
    ...
    typename std::enable_if<std::is_same<decltype(output.init()), Context::Builder>::value>::type* enable = nullptr)
{
    ...
    if (!local_thread) {
        local_thread = invoke_context.loop.m_threads.add(
            kj::heap<ProxyServer<Thread>>(invoke_context.client_thread, false /* join_thread */));
    }
    ...
    if (!remote_thread) {
        ...
        remote_thread = request.send().getResult(); // Nonblocking due to capnp request pipelining.
    }
        ...
        server.m_loop->m_threads.getLocalServer(thread_client)
            .then([&server, invoke, req](kj::Maybe<Thread::Server&> perhaps) {
                KJ_IF_MAYBE(thread_server, perhaps)
                {
                    ...
                    LogIpc(
                        *server.m_loop, "IPC server post request  #%i {%s}\n", req, thread.m_context.waiter->m_name);
                    thread.m_context.waiter->post(std::move(invoke));
                }
                else
                {
                    LogIpc(*server.m_loop, "IPC server error request #%i, missing thread to execute request\n", req);
...
ProxyClientBase<Interface, Impl>::~ProxyClientBase() noexcept
{
    ...
    if (m_loop) {
        ...
        // Invoke CloseHook handlers to free any resources associated with
        // this object that are no longer needed.
        Base::close();
    }
}
...
void clientDestroy(Client& client)
{
    if (client.m_loop)
        ...
        LogPrintf("IPC interrupted client destroy %s\n", typeid(client).name());
}
    ...
    try {
        ...
        return ReplaceVoid([&]() { return fn.invoke(server_context, ArgList()); },
            ...
            .then([&server, req](CallContext call_context) {
                LogIpc(*server.m_loop, "IPC server send response #%i %s %s\n", req, TypeName<Results>(),
