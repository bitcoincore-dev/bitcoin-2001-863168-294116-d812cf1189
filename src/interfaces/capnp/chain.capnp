...
interface Chain $Proxy.wrap("interfaces::Chain") {
    ...
    relayTransaction @7 (context :Proxy.Context, txid :Data) -> ();
...
interface ChainLock $Proxy.wrap("interfaces::Chain::Lock") {
    ...
    submitToMemoryPool @14 (context :Proxy.Context, tx :Data, absurdFee: Int64, state :ValidationState) -> (state :ValidationState, result :Bool);
}
...
interface ChainNotifications $Proxy.wrap("interfaces::Chain::Notifications") {
    ...
    chainStateFlushed @5 (context :Proxy.Context, locator :Data) -> () $Proxy.name("ChainStateFlushed");
}
