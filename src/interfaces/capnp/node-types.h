...
{
    return interfaces::capnp::g_config.make_node_client(context, kj::mv(client));
}
...
CustomMakeProxyServer<interfaces::capnp::messages::Node, interfaces::Node>(InvokeContext& context,
    std::unique_ptr<interfaces::Node>&& impl)
{
    return interfaces::capnp::g_config.make_node_server(context, std::move(impl));
}
