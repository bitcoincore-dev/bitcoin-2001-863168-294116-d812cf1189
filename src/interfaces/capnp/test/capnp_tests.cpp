...
#include <interfaces/capnp/test/foo.capnp.proxy.h>
...
// FIXME: Duplicated from capnp/ipc.cpp
struct ServerVatId
{
...
public:
    ...
    void onClose(Base& interface) override { m_loop.shutdown(); }
    EventLoop& m_loop;
};
...
BOOST_AUTO_TEST_CASE(capnp_tests)
{
    ...
    std::thread thread([&]() {
        ...
        auto foo_server = kj::heap<ProxyServer<messages::FooInterface>>(
            MakeFooInterface().release(), true /* owned */, loop, nullptr /* connection */);
        ...
        auto foo_client = MakeUnique<ProxyClient<messages::FooInterface>>(
            rpc_client.bootstrap(ServerVatId().vat_id).castAs<messages::FooInterface>(), connection);
