...
interface Node $Proxy.wrap("interfaces::Node") {
    ...
    getReindex @38 (context :Proxy.Context) -> (result :Bool);
    getImporting @39 (context :Proxy.Context) -> (result :Bool);
    setNetworkActive @40 (context :Proxy.Context, active :Bool) -> ();
    getNetworkActive @41 (context :Proxy.Context) -> (result :Bool);
    estimateSmartFee @42 (context :Proxy.Context, numBlocks :Int32, conservative :Bool, wantReturnedTarget :Bool) -> (returnedTarget :Int32, result :Data);
    getDustRelayFee @43 (context :Proxy.Context) -> (result :Data);
    executeRpc @44 (context :Proxy.Context, command :Text, params :Common.UniValue, uri :Text) -> (error :Text $Proxy.exception("std::exception"), rpcError :Common.UniValue $Proxy.exception("UniValue"), result :Common.UniValue);
    listRpcCommands @45 (context :Proxy.Context) -> (result :List(Text));
    rpcSetTimerInterfaceIfUnset @46 (context :Proxy.Context, iface :Void) -> ();
    rpcUnsetTimerInterface @47 (context :Proxy.Context, iface :Void) -> ();
    getUnspentOutput @48 (context :Proxy.Context, output :Data) -> (coin :Data, result :Bool);
    getWalletDir @49 (context :Proxy.Context) -> (result :Text);
    listWalletDir @50 (context :Proxy.Context) -> (result :List(Text));
    getWallets @51 (context :Proxy.Context) -> (result :List(Wallet.Wallet));
    loadWallet @52 (context :Proxy.Context, name :Text) -> (error :Text, warning: Text, result :Wallet.Wallet);
    handleInitMessage @53 (context :Proxy.Context, callback :InitMessageCallback) -> (result :Handler.Handler);
    handleMessageBox @54 (context :Proxy.Context, callback :MessageBoxCallback) -> (result :Handler.Handler);
    handleQuestion @55 (context :Proxy.Context, callback :QuestionCallback) -> (result :Handler.Handler);
    handleShowProgress @56 (context :Proxy.Context, callback :ShowNodeProgressCallback) -> (result :Handler.Handler);
    handleLoadWallet @57 (context :Proxy.Context, callback :LoadWalletCallback) -> (result :Handler.Handler);
    handleNotifyNumConnectionsChanged @58 (context :Proxy.Context, callback :NotifyNumConnectionsChangedCallback) -> (result :Handler.Handler);
    handleNotifyNetworkActiveChanged @59 (context :Proxy.Context, callback :NotifyNetworkActiveChangedCallback) -> (result :Handler.Handler);
    handleNotifyAlertChanged @60 (context :Proxy.Context, callback :NotifyAlertChangedCallback) -> (result :Handler.Handler);
    handleBannedListChanged @61 (context :Proxy.Context, callback :BannedListChangedCallback) -> (result :Handler.Handler);
    handleNotifyBlockTip @62 (context :Proxy.Context, callback :NotifyBlockTipCallback) -> (result :Handler.Handler);
    handleNotifyHeaderTip @63 (context :Proxy.Context, callback :NotifyHeaderTipCallback) -> (result :Handler.Handler);
}
