...
template <typename Proxy, typename Value, typename Output>
void BuildField(TypeList<SecureString>, Priority<1>, Proxy& proxy, Value&& str, Output&& output)
{
    ...
    Output&& output)
{
    ...
    BuildField(TypeList<CNodeStats>(), BuildFieldPriority(), proxy, std::get<0>(stats), output);
    if (std::get<1>(stats)) {
        ...
        BuildField(TypeList<CNodeStateStats>(), BuildFieldPriority(), proxy, std::get<2>(stats), field_output);
    }
}

template <typename LocalType, typename Proxy, typename Value, typename Output>
...
template <typename Proxy, typename Value, typename Output>
void BuildField(TypeList<CReserveScript>, Priority<1>, Proxy& proxy, Value&& script, Output&& output)
{
    CDataStream stream(SER_NETWORK, CLIENT_VERSION);
...
static inline ::capnp::Void BuildPrimitive(Proxy& proxy, RPCTimerInterface*, TypeList<::capnp::Void>)
{
    return {};
}
...
template <typename Proxy, typename Output>
        ...
        enable = nullptr)
{
    ...
    LOCK(args.cs_args);
    BuildField(TypeList<GlobalArgs>(), BuildFieldPriority(), proxy, args, output);
}
