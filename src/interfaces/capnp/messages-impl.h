    ...
    Builder&& builder)
{
    ...
    if (std::get<1>(stats)) {
        ...
        BuildField(TypeList<CNodeStateStats>(), TypeList<FUCK>(), BuildFieldPriority(), loop, std::get<2>(stats),
            field_builder);
    }
}
...
template <typename FUCK, typename Fn, typename Proxy, typename Reader, typename... FnParams>
    ...
    FnParams&&... fn_params)
{
    ...
    {
        LOCK(args.cs_args);
        ReadField(TypeList<decltype(args.mapArgs)>(), proxy, MakeValueInput(reader.get().getArgs(), reader.loop()),
            args.mapArgs);
        ReadField(TypeList<decltype(args.mapMultiArgs)>(), proxy,
            MakeValueInput(reader.get().getMultiArgs(), reader.loop()), args.mapMultiArgs);
    }
}
...
void BuildField(TypeList<>, TypeList<messages::GlobalArgs::Builder>, Priority<1>, EventLoop& loop, Builder&& builder)
{
    ...
    {
        LOCK(args.cs_args);
        // FIXME BuildField(TypeList<decltype(args.mapArgs)>(), BuildFieldPriority(), loop, args.mapArgs,
        // builder.getArgs());
...
template <typename Builder>
void BuildField(TypeList<>, TypeList<messages::Context::Builder>, Priority<1>, EventLoop& loop, Builder&& builder)
{
    auto context = builder.set();
    context.setPid(getpid());
    context.setTid(syscall(SYS_gettid));
}

template <typename FUCK, typename Proxy, typename Reader, typename Builder, typename Fn, typename... FnParams>
void PassField(TypeList<FUCK>,
