...
void ReadFieldNew(TypeList<SecureString>, Proxy& proxy, Reader&& reader, Emplace&& emplace)
{
    auto data = reader.get();
...
template <typename Proxy, typename Reader, typename Value>
void ReadFieldUpdate(TypeList<CReserveScript>, Proxy& proxy, Reader&& reader, Value&& value)
{
    ReadFieldUpdate(TypeList<decltype(value.reserveScript)>(), proxy, reader, value.reserveScript);
}

template <typename Value, typename Output>
void BuildField(TypeList<SecureString>, Priority<1>, EventLoop& loop, Value&& str, Output&& output)
{
    auto result = output.set(str.size());
    ...
    Output&& output)
{
    ...
    BuildField(TypeList<CNodeStats>(), BuildFieldPriority(), loop, std::get<0>(stats), output);
    if (std::get<1>(stats)) {
        auto message_builder = output.set();
        auto accessor = ProxyStruct<messages::NodeStats>::getStateStats();
        FieldOutput<messages::NodeStats::Builder, decltype(accessor)> field_output{message_builder, accessor};
        BuildField(TypeList<CNodeStateStats>(), BuildFieldPriority(), loop, std::get<2>(stats), field_output);
    }
}
...
void BuildField(TypeList<LocalType>,
    ...
    Value&& value,
    Output&& output,
    decltype(BuildMessage(value, output.set()))* enable = nullptr)
{
    BuildMessage(value, output.set());
}
        ...
        typename std::remove_cv<typename std::remove_reference<Value>::type>::type>::value>::type* enable = nullptr)
{
    CDataStream stream(SER_NETWORK, CLIENT_VERSION);
    value.Serialize(stream);
    auto result = output.set(stream.size());
    memcpy(result.begin(), stream.data(), stream.size());
}

template <typename Value, typename Output>
void BuildField(TypeList<CReserveScript>, Priority<1>, EventLoop& loop, Value&& script, Output&& output)
{
    CDataStream stream(SER_NETWORK, CLIENT_VERSION);
...
messages::Chain::Client BuildPrimitive(EventLoop& loop, interfaces::Chain&& impl, TypeList<messages::Chain::Client>);

// FIXME should be nonrvalue reference
messages::ChainNotifications::Client BuildPrimitive(EventLoop& loop,
    interfaces::Chain::Notifications&& impl,
...
template <typename Fn, typename Proxy, typename Input, typename Output, typename... FnParams>
typename std::enable_if<std::is_same<typename Plain<Input>::CapType, messages::Context>::value, bool>::type
PassField(TypeList<>, Proxy& proxy, Input&& input, Output&& output, Fn&& fn, FnParams&&... fn_params)
{
    using Context = std::pair<int32_t, int32_t>;
    Context call_context;
    ReadField(TypeList<Context>(), proxy, input, call_context);
    return fn(std::forward<FnParams>(fn_params)...);
}

template <typename Fn, typename Proxy, typename Input, typename Output, typename... FnParams>
typename std::enable_if<std::is_same<typename Plain<Input>::CapType, messages::GlobalArgs>::value, bool>::type
PassField(TypeList<>, Proxy& proxy, Input&& input, Output&& output, Fn&& fn, FnParams&&... fn_params)
{
    auto& args = static_cast<Args&>(::gArgs);
    {
        LOCK(args.cs_args);
        ReadField(TypeList<decltype(args.mapArgs)>(), proxy, MakeValueInput(input.get().getArgs()), args.mapArgs);
        ReadField(TypeList<decltype(args.mapMultiArgs)>(), proxy, MakeValueInput(input.get().getMultiArgs()),
            args.mapMultiArgs);
    }
    return fn(std::forward<FnParams>(fn_params)...);
}

void FillGlobalArgs(EventLoop& loop, messages::GlobalArgs::Builder&&);
...
void BuildField(TypeList<>,
    ...
    EventLoop& loop,
    Output&& output,
    typename std::enable_if<std::is_same<typename Plain<Output>::Type, messages::GlobalArgs::Builder>::value>::type*
        enable = nullptr)
{
    FillGlobalArgs(loop, output.set());
}
...
void BuildField(TypeList<>,
    ...
    EventLoop& loop,
    Output&& output,
    typename std::enable_if<std::is_same<typename Plain<Output>::Type, messages::Context::Builder>::value>::type*
        enable = nullptr)
{
    auto context = output.set();
    context.setPid(getpid());
    context.setTid(syscall(SYS_gettid));
}

template <typename Proxy, typename Input, typename Output, typename Fn, typename... FnParams>
bool PassField(TypeList<int, const char* const*>,
    Proxy& proxy,
    Input&& input,
    Output&& output,
    Fn&& fn,
    FnParams&&... fn_params)
{
    ...
    return fn(std::forward<FnParams>(fn_params)..., argc, argv);
}
