...
template <typename LocalType, typename Reader, typename Emplace>
...
void ReadFieldNew(TypeList<SecureString>, InvokeContext& invoke_context, Reader&& reader, Emplace&& emplace)
{
    auto data = reader.get();
...
void ReadFieldUpdate(TypeList<CReserveScript>, InvokeContext& invoke_context, Reader&& reader, Value&& value)
{
    ReadFieldUpdate(TypeList<decltype(value.reserveScript)>(), invoke_context, reader, value.reserveScript);
}
...
void PassField(TypeList<messages::GlobalArgs>,
    TypeList<>,
    ...
    Args&&... args)
{
    // FIXME: Do globalargs handling here instead of in invokeMethod overloads
    return fn.invoke(server_context, std::forward<Args>(args)...);
}

template <typename MethodContext, typename Server>
class AsyncClosure
{
...
void BuildField(TypeList<>,
    ...
    InvokeContext& invoke_context,
    Output&& output,
    typename std::enable_if<std::is_same<typename Decay<Output>::CapType, messages::GlobalArgs>::value>::type* enable =
        nullptr)
{
    const auto& args = static_cast<const GlobalArgs&>(::gArgs);
    LOCK(args.cs_args);
    BuildField(TypeList<GlobalArgs>(), BuildFieldPriority(), invoke_context, args, output);
}

template <typename ServerContext, typename Accessor, typename Fn, typename... Args>
void PassField(TypeList<::capnp::List<::capnp::Text>>,
    ...
    Args&&... args)
{
    auto value = Make<FieldInput>(server_context.method_context.getParams(), accessor).get();
