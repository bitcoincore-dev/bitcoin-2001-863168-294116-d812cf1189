    ...
    try {
    {
        ...
        if (fPruneMode && (fCheckForPruning || nManualPruneHeight > 0) && !fReindex) {
            if (nManualPruneHeight > 0) {
                TIME_MILLIS("find files to prune (manual", BCLog::BENCH);

                FindFilesToPruneManual(setFilesToPrune, nManualPruneHeight);
            } else {
                TIME_MILLIS("find files to prune", BCLog::BENCH);
        ...
        if (fDoFullFlush || fPeriodicWrite) {
            ...
            {
                TIME_MILLIS("write block and undo data to disk", BCLog::BENCH);
            ...
            {
                TIME_MILLIS("write block index to disk", BCLog::BENCH);
            ...
            if (fFlushForPrune) {
                TIME_MILLIS("unlink pruned files", BCLog::BENCH);
        ...
        if (fDoFullFlush && !CoinsTip().GetBestBlock().IsNull()) {
            TIME_SECONDS(strprintf("write coins cache to disk (%d coins, %.2fkB)",
