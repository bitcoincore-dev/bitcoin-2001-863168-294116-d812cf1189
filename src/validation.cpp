    ...
    EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
{
    ...
    if (is_snapshot) {
        fs::path base_blockhash_path = db_path / SNAPSHOT_BLOCKHASH_FILENAME;
    ...
    std::string dbpath = fs::PathToString(db_path);
    LogPrintf("Removing leveldb dir at %s\n", dbpath);

    // We have to destruct before this call leveldb::DB in order to release the db
    // lock, otherwise `DestroyDB` will fail. See `leveldb::~DBImpl()`.
    const bool destroyed = dbwrapper::DestroyDB(db_path, db_options).ok();
...
bool ChainstateManager::ValidatedSnapshotCleanup()
{
    ...
    // Copy out the leveldb options of the database we're removing before we
    // destruct the object.
    //
    // We have to destruct leveldb::DB in order to release the db lock, otherwise
    // `DestroyDB` will fail. See `leveldb::~DBImpl()`.
    dbwrapper::Options ibd_coinsdb_options = m_ibd_chainstate->PrepareForCoinsDBDeletion();
    ...
    if (!DeleteCoinsDBFromDisk(tmp_old, ibd_coinsdb_options)) {
