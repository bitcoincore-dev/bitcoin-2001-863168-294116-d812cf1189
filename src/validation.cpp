...
CChain& ChainActive() { return g_chainman.m_active_chainstate->m_chain; }
...
bool CheckSequenceLocks(const CTxMemPool& pool, const CTransaction& tx, int flags, LockPoints* lp, bool useExistingLockPoints)
{
    ...
    if (useExistingLockPoints) {
        ...
        CCoinsViewMemPool viewMemPool(g_chainman.ActiveCoinsCache(), pool);
...
static void LimitMempoolSize(CTxMemPool& pool, size_t limit, unsigned long age) {
    ...
    for (const COutPoint& removed : vNoSpendsRemaining)
        ::ChainstateActive().CoinsCache()->Uncache(removed);
}
...
static void UpdateMempoolForReorg(DisconnectedBlockTransactions &disconnectpool, bool fAddToMempool) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    mempool.removeForReorg(::ChainstateActive().CoinsCache(), ::ChainActive().Tip()->nHeight + 1, STANDARD_LOCKTIME_VERIFY_FLAGS);
...
static bool CheckInputsFromMempoolAndCache(const CTransaction& tx, CValidationState& state, const CCoinsViewCache& view, const CTxMemPool& pool,
    ...
    for (const CTxIn& txin : tx.vin) {
        ...
        if (txFrom) {
            ...
            const Coin& coinFromDisk = ::ChainstateActive().CoinsCache()->AccessCoin(txin.prevout);
                        ...
                        bool bypass_limits, const CAmount nAbsurdFee, bool test_accept) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    if (!res) {
        for (const COutPoint& hashTx : coins_to_uncache)
            ::ChainstateActive().CoinsCache()->Uncache(hashTx);
    }
...
// Note that though this is marked const, we may end up modifying `m_cached_in_ibd`, which
...
bool CChainState::IsInitialBlockDownload() const
{
    ...
    if (m_cached_in_ibd.load(std::memory_order_relaxed))
        return false;
    ...
    m_cached_in_ibd.store(true, std::memory_order_relaxed);
    return false;
}
    ...
    int nManualPruneHeight)
{
    ...
    try {
    {
        ...
        int64_t cacheSize = CoinsCache()->DynamicMemoryUsage();

        // Divide up coincache usage evenly per chainstate in use.
        int64_t coincacheusage_per_chainstate = nCoinCacheUsage / g_chainman.GetAll().size();
        int64_t nTotalSpace = coincacheusage_per_chainstate + std::max<int64_t>(nMempoolSizeMax - nMempoolUsage, 0);
        ...
        if (fDoFullFlush && !CoinsCache()->GetBestBlock().IsNull()) {
            // Typical Coin structures on disk are around 48 bytes in size.
            // Pushing a new one to the database can cause it to be written
            // twice (once in the log, and once in the tables). This is already
            // an overestimation, as most will delete an existing entry or
            // overwrite one. Still, use a conservative safety factor of 2.
            if (!CheckDiskSpace(GetDataDir(), 48 * 2 * 2 * CoinsCache()->GetCacheSize())) {
                return AbortNode(state, "Disk space is low!", _("Error: Disk space is low!"));
            }
...
void FlushStateToDisk(CChainState& chainstate) {
    CValidationState state;
    const CChainParams& chainparams = Params();
    if (!chainstate.FlushStateToDisk(chainparams, state, FlushStateMode::ALWAYS)) {
...
void PruneAndFlush(CChainState& chainstate) {
    ...
    const CChainParams& chainparams = Params();
    if (!chainstate.FlushStateToDisk(chainparams, state, FlushStateMode::NONE)) {
        LogPrintf("%s: failed to flush state (%s)\n", __func__, FormatStateMessage(state));
    }
}
...
void static UpdateTip(const CBlockIndex *pindexNew, const CChainParams& chainParams, const CCoinsViewCache* coins_view) {
    ...
    if (coins_view != g_chainman.ActiveCoinsCache()) {
        return;
    }
    ...
    LogPrintf("%s: new best=%s height=%d version=0x%08x log2_work=%.8g tx=%lu date='%s' progress=%f cache=%.1fMiB(%utxo)", __func__, /* Continued */
      ...
      GuessVerificationProgress(chainParams.TxData(), pindexNew), ::ChainstateActive().CoinsCache()->DynamicMemoryUsage() * (1.0 / (1<<20)), ::ChainstateActive().CoinsCache()->GetCacheSize());
...
bool CChainState::DisconnectTip(CValidationState& state, const CChainParams& chainparams, DisconnectedBlockTransactions *disconnectpool)
{
    ...
    {
        CCoinsViewCache view(CoinsCache());
    ...
    UpdateTip(pindexDelete->pprev, chainparams, CoinsCache());
...
bool CChainState::ConnectTip(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions &disconnectpool)
{
    ...
    UpdateTip(pindexNew, chainparams, CoinsCache());
    ...
    if (g_chainman.IsBackgroundValidationChainstate(this)) {
        if (pindexNew->nHeight > g_chainman.SnapshotHeight()) {
            ...
            LogPrintf(
                "[snapshot] something is wrong! validation chain should not have " /* Continued */
                "continued past the snapshot origin\n");
...
bool ProcessNewBlockHeaders(const std::vector<CBlockHeader>& headers, CValidationState& state, const CChainParams& chainparams, const CBlockIndex** ppindex, CBlockHeader *first_invalid)
{
    if (first_invalid != nullptr) first_invalid->SetNull();
    {
        ...
        for (const CBlockHeader& header : headers) {
            ...
            if (!g_chainman.m_blockman.AcceptBlockHeader(header, state, chainparams, &pindex)) {
...
bool CChainState::AcceptBlock(const std::shared_ptr<const CBlock>& pblock, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, bool fRequested, const FlatFilePos* dbp, bool* fNewBlock)
{
    ...
    if (!m_blockman.AcceptBlockHeader(block, state, chainparams, &pindex))
        return false;
...
bool ProcessNewBlock(const CChainParams& chainparams, const std::shared_ptr<const CBlock> pblock, bool fForceProcessing, bool *fNewBlock)
{
...

    {
        ...
        if (!ret) {
            GetMainSignals().BlockChecked(*chainstate, *pblock, state);
...
bool TestBlockValidity(CValidationState& state, const CChainParams& chainparams, const CBlock& block, CBlockIndex* pindexPrev, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    CCoinsViewCache viewNew(::ChainstateActive().CoinsCache());
        ...
        if (pindex->nFile == fileNumber) {
            ...
            std::pair<std::multimap<CBlockIndex*, CBlockIndex*>::iterator,
                std::multimap<CBlockIndex*, CBlockIndex*>::iterator> range =
                    g_chainman.m_blockman.m_blocks_unlinked.equal_range(pindex->pprev);
            while (range.first != range.second) {
...
bool CChainState::LoadChainTip(const CChainParams& chainparams)
{
    ...
    CCoinsViewCache* coins_cache = CoinsCache();
    ...
    int nCheckLevel, int nCheckDepth)
{
    ...
    for (pindex = chainstate->m_chain.Tip(); pindex && pindex->pprev; pindex = pindex->pprev) {
        ...
        int64_t coincacheusage_per_chainstate = nCoinCacheUsage / g_chainman.GetAll().size();
        int64_t curr_coins_usage = coins.DynamicMemoryUsage() + chainstate->CoinsCache()->DynamicMemoryUsage();

        if (nCheckLevel >= 3 && curr_coins_usage <= coincacheusage_per_chainstate) {
...
bool CChainState::ReplayBlocks(const CChainParams& params)
{
    ...
    CCoinsViewCache cache(coins_cache);

    std::vector<uint256> hashHeads = coins_cache->GetHeadBlocks();
...
bool LoadGenesisBlock(const CChainParams& chainparams)
{
    ...
    // BlockIndex(). Note that we can't use m_chain here, since it is
...
void CChainState::CheckBlockIndex(const Consensus::Params& consensusParams)
{
    ...
    while (pindex != nullptr) {
        ...
        assert(pindexFirstNotTreeValid == nullptr); // All BlockIndex() entries must at least be TREE valid
    ...
    bool snapshot_needs_initialization)
{
    ...
    to_modify.reset(new CChainState(
        m_blockman, cache_size_bytes, in_memory, should_wipe, "chainstate", snapshot_blockhash));
        ...
        bool in_memory)
{
    ...
    m_unready_snapshot_chainstate->m_chain.SetTip(snapshot_start_block);
    m_unready_snapshot_chainstate->m_chain.FakeNTx(metadata.m_nchaintx);

    int current_height = m_unready_snapshot_chainstate->m_chain.Height();
    if (current_height != base_height) {
        LogPrintf("[snapshot] bad snapshot height: expected %d, got %d\n", base_height, current_height);
        return false;
    }

    m_unready_snapshot_chainstate->setBlockIndexCandidates.insert(snapshot_start_block);
    m_snapshot_chainstate.swap(m_unready_snapshot_chainstate);
