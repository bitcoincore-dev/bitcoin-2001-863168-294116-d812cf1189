...
void InitScriptExecutionCache() {
    ...
    size_t nMaxCacheSize = std::clamp<int>(gArgs.GetArg("-maxsigcachesize", DEFAULT_MAX_SIG_CACHE_SIZE) / 2, 0, MAX_MAX_SIG_CACHE_SIZE) * ((size_t)1 << 20);
    ...
    try {
    {
        ...
        if (fPruneMode && (fCheckForPruning || nManualPruneHeight > 0) && !fReindex) {
            ...
            ForEachBlockFilterIndex([&](BlockFilterIndex& index) {
                last_prune = std::clamp(last_prune, 1, index.GetSummary().best_block_height);
            });
