...
static bool IsCurrentForFeeEstimation(CChainState& active_chainstate) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    if (active_chainstate.m_chain.Tip()->GetBlockTime() < count_seconds(GetTime<std::chrono::seconds>() - MAX_FEE_ESTIMATION_TIP_AGE))
        return false;
    if (active_chainstate.m_chain.Height() < active_chainstate.m_chainman.m_blockman.pindexBestHeader->nHeight - 1)
        return false;
    return true;
}
...
void CChainState::InvalidChainFound(CBlockIndex* pindexNew)
{
    ...
    if (m_chainman.m_blockman.pindexBestHeader != nullptr && m_chainman.m_blockman.pindexBestHeader->GetAncestor(pindexNew->nHeight) == pindexNew) {
        m_chainman.m_blockman.pindexBestHeader = m_chain.Tip();
    }
    ...
    if (!hashAssumeValid.IsNull()) {
        ...
        if (it != m_blockman.m_block_index.end()) {
            if (it->second.GetAncestor(pindex->nHeight) == pindex &&
                m_chainman.m_blockman.pindexBestHeader->GetAncestor(pindex->nHeight) == pindex &&
                m_chainman.m_blockman.pindexBestHeader->nChainWork >= nMinimumChainWork) {
                ...
                fScriptChecks = (GetBlockProofEquivalentTime(*m_chainman.m_blockman.pindexBestHeader, *pindex, *m_chainman.m_blockman.pindexBestHeader, m_params.GetConsensus()) <= 60 * 60 * 24 * 7 * 2);
            }
        }
    }
...
static bool NotifyHeaderTip(CChainState& chainstate) LOCKS_EXCLUDED(cs_main) {
    ...
    {
        LOCK(cs_main);
        pindexHeader = chainstate.m_chainman.m_blockman.pindexBestHeader;
