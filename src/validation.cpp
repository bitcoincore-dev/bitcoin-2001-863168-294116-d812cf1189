...
bool CChainState::removeSnapshotDatadir(std::optional<fs::path> maybe_datadir)
{
    ...
    fs::path snapshot_datadir = maybe_datadir.value_or(*snapshotDatadirPath());
...
void ChainstateManager::validatedSnapshotCleanup()
{
    ...
    if (!should_cleanup_snapshot || !ibd_chainstate_path_maybe || !snapshot_chainstate_path_maybe) {
        // No need to clean up.
        return;
        ...
        assert(!ibd_chainstate_path_maybe && !snapshot_chainstate_path_maybe);
        return;
    }
