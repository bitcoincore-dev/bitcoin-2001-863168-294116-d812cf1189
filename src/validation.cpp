...
bool Chainstate::AcceptBlock(const std::shared_ptr<const CBlock>& pblock, BlockValidationState& state, CBlockIndex** ppindex, bool fRequested, const FlatFilePos* dbp, bool* fNewBlock, bool min_pow_checked)
{
    ...
    try {
        FlatFilePos blockPos{m_blockman.SaveBlockToDisk(block, pindex->nHeight, *this, params, dbp)};
        if (blockPos.IsNull()) {
...
bool Chainstate::LoadGenesisBlock()
{
    ...
    try {
        ...
        FlatFilePos blockPos{m_blockman.SaveBlockToDisk(block, 0, *this, params, nullptr)};
        if (blockPos.IsNull()) {
    ...
    const Chainstate& chainstate, unsigned int prune_height)
{
    ...
    if (&chainstate != &this->ActiveChainstate()) {
        prune_end = std::max(0U, chain_height - 1);
    } else {
        // last block to prune is the lesser of (user-specified height, MIN_BLOCKS_TO_KEEP from the tip)
        prune_end = std::max(0U, std::min(prune_height, chain_height - MIN_BLOCKS_TO_KEEP));
    }
