    ...
    CBlockTreeDB& blocktree)
{
    ...
    for (const std::pair<int, CBlockIndex*>& item : vSortedByHeight)
    {
        ...
        if (pindex->nTx > 0) {
            if (pindex->pprev) {
                if (using_unvalidated_snapshot && pindex->GetBlockHash() ==
                        g_chainman.SnapshotBlockhash().value_or(uint256())) {
                    ...
                    //
                    pindex->nChainTx = g_chainman.GetSnapshotNChainTx();
...
{
    if (!chainman.m_blockman.LoadBlockIndex(chainparams.GetConsensus(), *pblocktree)) {
        return false;
    }
    ...
    const SnapshotMetadata& metadata)
{
    ...
    // gets fully validated. This value will be used in.
...
bool ChainstateManager::CompleteSnapshotValidation(CChainState* validation_chainstate)
{
    ...
    if (!GetUTXOStats(&ibd_coins_db, ibd_stats, CoinStatsHashType::HASH_SERIALIZED)) {
    ...
    for (fs::directory_iterator it(GetDataDir()); it != fs::directory_iterator(); it++) {
            ...
            it->path().filename().string().substr(0,11) == "chainstate_")
        {
            ...
            this->InitializeChainstate(
                /*snapshot_blockhash*/ uint256S(path.filename().string().substr(11)));
            return true;
        }
    }
    return false;
