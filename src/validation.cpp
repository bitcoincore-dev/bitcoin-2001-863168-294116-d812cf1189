    ...
    CBlockTreeDB& blocktree)
{
    ...
    const uint256 snapshot_blockhash = g_chainman.SnapshotBlockhash().value_or(uint256());
    ...
    for (const std::pair<int, CBlockIndex*>& item : vSortedByHeight)
    {
        ...
        if (pindex->nTx > 0) {
            if (pindex->pprev) {
                if (using_unvalidated_snapshot && pindex->GetBlockHash() == snapshot_blockhash) {
...
bool ChainstateManager::CompleteSnapshotValidation(CChainState* validation_chainstate)
{
    ...
    LogPrintf("[snapshot] snapshot beginning at %s has been fully validated\n",
        SnapshotBlockhash().get().ToString());
