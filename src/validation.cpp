...
CChainState::CChainState(BlockManager& blockman, uint256 from_snapshot_blockhash, int worker_threads)
    : m_blockman(blockman),
      m_script_check_queue{MakeUnique<CCheckQueue<CScriptCheck>>(128 /* batch size */)},
      m_from_snapshot_blockhash(from_snapshot_blockhash)
{
    CreateScriptCheckWorkerThreads(worker_threads);
}
...
void CChainState::CreateScriptCheckWorkerThreads(int worker_threads)
{
    if (worker_threads >= 1) {
        m_parallel_script_checks = true;
        for (auto i = 0; i < worker_threads; ++i) {
            m_script_check_queue->AddWorkerThread();
        }
    }
}
...
CChainState& ChainstateManager::InitializeChainstate(const uint256& snapshot_blockhash, int script_threads)
{
    ...
    to_modify.reset(new CChainState(m_blockman, snapshot_blockhash, script_threads));
