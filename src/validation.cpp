    ...
    int nCheckLevel, int nCheckDepth)
{
    ...
    const bool is_snapshot_cs{chainstate.reliesOnAssumedValid()};
    ...
    if (!fReindex) {
        ...
        for (const CBlockIndex* block : vSortedByHeight) {
            if (block->IsAssumedValid()) {
                // There should be multiple chainstates if we encounter an assumedvalid
                // block.
                Assume(chainstates.size() >= 2);
...
void Chainstate::CheckBlockIndex()
{
    ...
    while (pindex != nullptr) {
        ...
        if (pindex->pprev && (pindex->nStatus & BLOCK_HAVE_DATA) && pindexFirstNeverProcessed == nullptr && pindexFirstMissing != nullptr) {
            ...
            assert(m_blockman.m_have_pruned || this->reliesOnAssumedValid()); // We must have pruned.
