...
void static UpdateTip(const CBlockIndex *pindexNew, const CChainParams& chainParams, const CCoinsViewCache& coins_view) {
    ...
    {
        LogPrintf("%s%s: new best=%s height=%d version=0x%08x log2_work=%.8g tx=%lu date='%s' progress=%f cache=%.1fMiB(%utxo)", /* Continued */
          prefix, func_name,
          pindexNew->GetBlockHash().ToString(), pindexNew->nHeight, pindexNew->nVersion,
          log(pindexNew->nChainWork.getdouble())/log(2.0), (unsigned long)pindexNew->nChainTx,
          FormatISO8601DateTime(pindexNew->GetBlockTime()),
          GuessVerificationProgress(chainParams.TxData(), pindexNew),
          coins_view.DynamicMemoryUsage() * (1.0 / (1<<20)), coins_view.GetCacheSize());
        if (!warning_messages.empty())
            LogPrintf(" warning='%s'", warning_messages); /* Continued */
        LogPrintf("\n");
    };
