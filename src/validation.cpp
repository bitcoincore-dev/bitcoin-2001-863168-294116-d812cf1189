...
bool CScriptCheck::operator()() {
    ...
    if (m_deferred_checks && m_deferred_checks->size() > 0) {
                       ...
                       std::vector<CScriptCheck>* pvChecks)
{
    ...
    for (unsigned int i = 0; i < tx.vin.size(); i++) {
        ...
        if (pvChecks) {
            ...
            if (check_ok && check.m_deferred_checks) {
                deferred_checks.insert(
                    deferred_checks.end(),
                    std::make_move_iterator(check.m_deferred_checks->begin()),
                    std::make_move_iterator(check.m_deferred_checks->end()));
            } else if (!check_ok) {
    ...
    // in ConnectBlock().
    if (!pvChecks && deferred_checks.size() > 0) {
