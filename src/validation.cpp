                        ...
                        const LockPoints* lp)
{
    ...
    return EvaluateSequenceLocks(index, {lp->height, lp->time});
}
    ...
    bool fAddToMempool)
{
    ...
    const auto filter_final_and_mature = [this, flags=STANDARD_LOCKTIME_VERIFY_FLAGS](CTxMemPool::txiter it)
        ...
        if (!validLP) {
            if (!CalculateLockPoints(m_chain.Tip(), view_mempool, tx, flags, &lp)) return true;
            m_mempool->mapTx.modify(it, [&lp](CTxMemPoolEntry& e) { e.UpdateLockPoints(lp); });
        }
        // CheckSequenceLocks checks if the transaction will be final in the next block to be
        // created on top of the new chain.
        if (!CheckSequenceLocks(m_chain.Tip(), &lp)) return true;
...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    if (!CalculateLockPoints(m_active_chainstate.m_chain.Tip(), m_view, tx, STANDARD_LOCKTIME_VERIFY_FLAGS, &lp) || !CheckSequenceLocks(m_active_chainstate.m_chain.Tip(), &lp)) {
