    ...
    int nManualPruneHeight)
{
    ...
    auto timer_start = [&time_started]() {
        assert(time_started == 0);
        time_started = GetTimeMillis();
    };
    auto timer_stop = [&time_started](std::string title, bool always_log = false) {
        const auto msg = "FlushStateToDisk: %s completed (%.3fms)\n";
        const auto end_time = GetTimeMillis() - time_started;

        if (always_log) {
            LogPrintf(msg, title, end_time);
        } else {
            LogPrint(BCLog::BENCH, msg, title, end_time);
        }
        time_started = 0;
    };
    ...
    try {
    {
        ...
        if (fDoFullFlush && !CoinsTip().GetBestBlock().IsNull()) {
            ...
            timer_stop(strprintf("write coins cache to disk (%d coins, %.2fkB)",
                coins_count, coins_mem_usage / 1000), /*always_log*/ true);
        }
    }
