...
{
    assert(iter != m_mempool.mapTx.end());
    for (const auto& parent : iter->GetMemPoolParentsConst()) {
        auto parent_it = m_mempool.mapTx.iterator_to(parent);
...
void BlockAssembler::addPriorityTxs(int &nPackagesSelected)
{
    ...
    while (!vecPriority.empty() && !blockFinished) { // add a tx from priority queue to fill the blockprioritysize
        ...
        if (TestForBlock(iter)) {
            ...
            {
                auto child_it = m_mempool.mapTx.iterator_to(child);
                waitPriIter wpiter = waitPriMap.find(child_it);
                if (wpiter != waitPriMap.end()) {
                    vecPriority.push_back(TxCoinAgePriority(wpiter->second, child_it));
