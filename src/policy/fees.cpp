...
bool CBlockPolicyEstimator::removeTx(uint256 hash, bool inBlock)
{
    ...
    return removeTxNonThreadSafeHelper(hash, inBlock);
}

bool CBlockPolicyEstimator::removeTxNonThreadSafeHelper(const uint256& hash, bool inBlock)
{
...
{
    AssertLockHeld(m_cs_fee_estimator);
    if (!removeTxNonThreadSafeHelper(entry->GetTx().GetHash(), true)) {
...
void CBlockPolicyEstimator::FlushUnconfirmed() {
    ...
    while (!mapMemPoolTxs.empty()) {
        ...
        removeTxNonThreadSafeHelper(mi->first, false); // this calls erase() on mapMemPoolTxs
    }
