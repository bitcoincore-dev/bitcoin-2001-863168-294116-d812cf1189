...
bool IsStandardTx(const CTransaction& tx, std::string& out_reason, const ignore_rejects_type& ignore_rejects)
{
...
bool AreInputsStandard(const CTransaction& tx, const CCoinsViewCache& mapInputs, const std::string& reason_prefix, std::string& out_reason, const ignore_rejects_type& ignore_rejects)
{
    ...
    for (unsigned int i = 0; i < tx.vin.size(); i++)
    {
        ...
        if (whichType == TX_SCRIPTHASH)
        {
            ...
            if (!EvalScript(ScriptExecution::Context::Sig, stack, tx.vin[i].scriptSig, SCRIPT_VERIFY_NONE, BaseSignatureChecker(), SigVersion::BASE))
            {
