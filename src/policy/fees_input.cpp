...
UniValue TxValue(const CTxMemPoolEntry& entry)
{
    ...
    value.pushKV("size", entry.GetTxSize());
    return value;
}
...
void CBlockPolicyInput::flushUnconfirmed(CTxMemPool& pool)
{
    int64_t startclear = GetTimeMicros();
    std::vector<uint256> txids;
    pool.queryHashes(txids);
    estimator.flushUnconfirmed(txids);
...
bool CBlockPolicyInput::readLog(const std::string& filename, std::function<bool(UniValue&)> filter)
{
    ...
    while (std::getline(file, line)) {
        ...
        if (flush.isArray()) {
            std::vector<uint256> txids;
            for (const UniValue& txid : flush.getValues()) {
                txids.emplace_back(uint256S(txid.get_str()));
            }
            estimator.flushUnconfirmed(txids);
            continue;
        }
