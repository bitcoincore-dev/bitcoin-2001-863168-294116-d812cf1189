...
private:
    ...
    int32_t nSizeWithDescendants;      //!< ... and size
    CAmount nModFeesWithDescendants;   //!< ... and total fees (all including us)

    // Analogous statistics for ancestor transactions
    uint64_t nCountWithAncestors{1};
    int32_t nSizeWithAncestors;
...
public:
    ...
    int32_t GetSizeWithDescendants() const { return nSizeWithDescendants; }
    CAmount GetModFeesWithDescendants() const { return nModFeesWithDescendants; }

    bool GetSpendsCoinbase() const { return spendsCoinbase; }

    uint64_t GetCountWithAncestors() const { return nCountWithAncestors; }
    int32_t GetSizeWithAncestors() const { return nSizeWithAncestors; }
...
public:
    ...
    void UpdateTransactionsFromBlock(const std::vector<uint256>& vHashesToUpdate,
            int32_t ancestor_size_limit, uint64_t ancestor_count_limit) EXCLUSIVE_LOCKS_REQUIRED(cs, cs_main) LOCKS_EXCLUDED(m_epoch);
...
private:
    ...
    void UpdateForDescendants(txiter updateIt, cacheMap& cachedDescendants,
                              ...
                              int32_t ancestor_size_limit, uint64_t ancestor_count_limit) EXCLUSIVE_LOCKS_REQUIRED(cs);
