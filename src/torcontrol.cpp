...
bool TorControlConnection::Connect(const std::string& control_socket, const ConnectionCB& _connected, const ConnectionCB&  _disconnected)
{
    if (b_conn)
        Disconnect();
    // Parse control_socket address:port
    struct sockaddr_storage connect_to_addr;
    int connect_to_addrlen = sizeof(connect_to_addr);
    if (evutil_parse_sockaddr_port(control_socket.c_str(),
    ...
    // Finally, connect to control_socket
...
{
public:
    TorController(struct event_base* base, const std::string& control_socket, const CService& target);
    ~TorController();
...
private:
    struct event_base* base;
    const std::string m_control_socket;
...
TorController::TorController(struct event_base* _base, const std::string& control_socket, const CService& target):
    base(_base),
    m_control_socket(control_socket), conn(base), reconnect(true), reconnect_ev(0),
    ...
    m_target(target)
{
    ...
    if (!conn.Connect(m_control_socket, std::bind(&TorController::connected_cb, this, std::placeholders::_1),
