...
void CWallet::BlockConnected(const std::shared_ptr<const CBlock>& pblock, const CBlockIndex *pindex, const std::vector<CTransactionRef>& vtxConflicted) {
    ...
    for (size_t i = 0; i < pblock->vtx.size(); i++) {
        SyncTransaction(*locked_chain, pblock->vtx[i], pindex->GetBlockHash(), i);
 ...
 */
int64_t CWallet::RescanFromTime(interface::Chain::Lock& locked_chain, int64_t startTime, bool update)
{
    ...
    const Optional<int> start_block = locked_chain.findEarliestAtLeast(startTime - TIMESTAMP_WINDOW);
    LogPrintf("%s: Rescanning last %i blocks\n", __func__, tip_height && start_block ? *tip_height - *start_block + 1 : 0);

    if (start_block) {
        if (Optional<int> failed_block = ScanForWalletTransactions(locked_chain, *start_block, nullopt /* stop height */, update)) {
...
/**
 * Scan the block chain (starting at start_height) for transactions
 * from or to us. If fUpdate is true, found transactions that already
 * exist in the wallet will be updated.
 *
 * Returns nothing if scan was successful. Otherwise, if a complete rescan was not
 * possible (due to pruning or corruption), returns height of the most recent
 * block that could not be scanned.
 *
 * If stop_height is specified, the scan will stop after the specified height.
 */
Optional<int> CWallet::ScanForWalletTransactions(interface::Chain::Lock& locked_chain, int start_height, Optional<int> stop_height, bool fUpdate)
{
    ...
    Optional<int> ret;
    {
        ...
        while (tip_height && index <= *tip_height && !fAbortRescan)
        {
            ...
            CBlock block;
            if (locked_chain.readBlockFromDisk(index, block)) {
                const uint256& block_hash = locked_chain.getBlockHash(index);
            ...
            if (stop_height && index == *stop_height) {
                break;
            }
            ++index;
        }
                                 ...
                                 std::set<CInputCoin>& setCoinsRet, CAmount& nValueRet) const
{
    ...
    for (const COutput &output : vCoins)
    {
        ...
        if (!m_chain->transactionWithinChainLimit(pcoin->GetHash(), nMaxAncestors))
            continue;
...
bool CWallet::FundTransaction(interface::Chain::Lock& locked_chain, CMutableTransaction& tx, CAmount& nFeeRet, int& nChangePosInOut, std::string& strFailReason, bool lockUnspents, const std::set<int>& setSubtractFeeFromOutputs, CCoinControl coinControl)
{
    ...
    CWalletTx wtx;
    if (!CreateTransaction(locked_chain, vecSend, wtx, reservekey, nFeeRet, nChangePosInOut, strFailReason, coinControl, false)) {
        return false;
    }
                                ...
                                int& nChangePosInOut, std::string& strFailReason, const CCoinControl& coin_control, bool sign)
{
    ...
    if (gArgs.GetBoolArg("-walletrejectlongchains", DEFAULT_WALLET_REJECT_LONG_CHAINS)) {
        ...
        if (!m_chain->checkChainLimits(wtxNew.tx)) {
...
void CWallet::GetKeyBirthTimes(interface::Chain::Lock& locked_chain, std::map<CTxDestination, int64_t> &mapKeyBirth) const {
    ...
    for (std::map<CKeyID, int>::const_iterator it = mapKeyFirstBlock.begin(); it != mapKeyFirstBlock.end(); it++)
        mapKeyBirth[it->first] = locked_chain.getBlockTime(it->second) - TIMESTAMP_WINDOW; // block times can be 2h off
}
...
CWallet* CWallet::CreateWalletFromFile(interface::Chain& chain, const std::string walletFile)
{
    ...
    if (gArgs.GetBoolArg("-zapwallettxes", false)) {
        ...
        std::unique_ptr<CWallet> tempWallet = MakeUnique<CWallet>(&chain, std::move(dbw));
    ...
    bool fFirstRun = true;
    std::unique_ptr<CWalletDBWrapper> dbw(new CWalletDBWrapper(&bitdb, walletFile));
    CWallet *walletInstance = new CWallet(&chain, std::move(dbw));
    ...
    if (tip_height && *tip_height != index_rescan)
    {
        ...
        walletInstance->ScanForWalletTransactions(*locked_chain, index_rescan, {} /* stop height */, true /* update */);
