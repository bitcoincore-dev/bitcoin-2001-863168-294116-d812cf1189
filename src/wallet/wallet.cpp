    ...
    unsigned int nBytes;
    {
        ...
        LOCK(cs_wallet);
        {
            ...
            while (true)
            {
                ...
                nFeeNeeded = m_ipc_chain->getMinTxFee(nBytes, coin_control, &feeCalc);
                ...
                if (nFeeRet >= nFeeNeeded) {
                    ...
                    if (nChangePosInOut == -1 && nSubtractFeeFromAmount == 0 && pick_new_inputs) {
                        ...
                        CAmount fee_needed_with_change = m_ipc_chain->getMinTxFee(tx_size_with_change, coin_control, nullptr);
...
CWallet* CWallet::CreateWalletFromFile(ipc::Chain& ipc_chain, const std::string walletFile)
{
    ...
    if (!gArgs.GetBoolArg("-rescan", false))
    {
        ...
        if (walletdb.ReadBestBlock(locator)) {
            int fork_block = ipc_locked->findLocatorFork(locator);
            if (fork_block >= 0) {
    ...
    if (tip_height >= 0 && tip_height != index_rescan)
    {
        ...
        if (walletInstance->nTimeFirstKey) {
            int last_older_block = ipc_locked->findLastBefore(walletInstance->nTimeFirstKey - TIMESTAMP_WINDOW, index_rescan);
