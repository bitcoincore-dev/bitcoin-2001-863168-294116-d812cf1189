...
int64_t CWallet::RescanFromTime(interface::Chain::Lock& locked_chain, int64_t startTime, bool update)
{
    ...
    if (startBlock) {
        const CBlockIndex* const failedBlock = ScanForWalletTransactions(locked_chain, startBlock, update);
        if (failedBlock) {
 ...
 */
CBlockIndex* CWallet::ScanForWalletTransactions(interface::Chain::Lock& locked_chain, CBlockIndex* pindexStart, bool fUpdate)
{
...
CWallet* CWallet::CreateWalletFromFile(interface::Chain& chain, const std::string walletFile)
{
    ...
    if (chainActive.Tip() && chainActive.Tip() != pindexRescan)
    {
        ...
        walletInstance->ScanForWalletTransactions(*locked_chain, pindexRescan, true);
