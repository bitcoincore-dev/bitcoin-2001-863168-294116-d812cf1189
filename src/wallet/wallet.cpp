...
bool CWallet::AddToWalletIfInvolvingMe(const CTransactionRef& ptx, CWalletTx::Status status, const uint256& block_hash, int posInBlock, bool fUpdate)
{
    const CTransaction& tx = *ptx;
    {
        ...
        if (fExisted || IsMine(tx) || IsFromMe(tx))
        {
            ...
            return AddToWallet(MakeTransactionRef(tx), {status, block_hash, posInBlock}, /* update_wtx= */ nullptr, /* fFlushOnClose= */ false);
        }
    }
    return false;
}
...
void CWallet::CommitTransaction(CTransactionRef tx, mapValue_t mapValue, std::vector<std::pair<std::string, std::string>> orderForm)
{
    ...
    if (!wtx_new->SubmitMemoryPoolAndRelay(err_string, true, *locked_chain)) {
