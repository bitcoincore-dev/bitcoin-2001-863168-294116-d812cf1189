 ...
 */
bool CWallet::AddToWalletIfInvolvingMe(interfaces::Chain::Lock& locked_chain, const CTransactionRef& ptx, const CBlockIndex* pIndex, int posInBlock, bool fUpdate)
{
...
void CWallet::SyncTransaction(interfaces::Chain::Lock& locked_chain, const CTransactionRef& ptx, const CBlockIndex *pindex, int posInBlock) {
    const CTransaction& tx = *ptx;
...
int CMerkleTx::GetDepthInMainChain(interfaces::Chain::Lock& locked_chain, const CBlockIndex* &pindexRet) const
{
    if (hashUnset())
        return 0;
