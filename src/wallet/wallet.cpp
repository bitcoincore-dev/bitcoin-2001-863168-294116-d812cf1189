...
{
    {
        ...
        TxEntry* pEntry = nullptr;
        {
            ...
            AddToWallet(tx, [&](TxEntry& entry, bool fNew) {
                assert(fNew);
                entry.second.mapValue = std::move(mapValue);
                entry.second.vOrderForm = std::move(orderForm);
                entry.second.strFromAccount = std::move(fromAccount);
