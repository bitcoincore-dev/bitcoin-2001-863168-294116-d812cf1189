...
void CWallet::SyncTransaction(interfaces::Chain::Lock& locked_chain, const CTransactionRef& ptx, const CBlockIndex *pindex, int posInBlock, bool update_tx) {
    if (!AddToWalletIfInvolvingMe(locked_chain, ptx, pindex, posInBlock, update_tx))
        ...
        while (pindex && !fAbortRescan && !ShutdownRequested())
        {
            ...
            if (ReadBlockFromDisk(block, pindex, Params().GetConsensus())) {
                ...
                for (size_t posInBlock = 0; posInBlock < block.vtx.size(); ++posInBlock) {
                    SyncTransaction(*locked_chain, block.vtx[posInBlock], pindex, posInBlock, fUpdate);
                }
            } else {
                ret = pindex;
            }
