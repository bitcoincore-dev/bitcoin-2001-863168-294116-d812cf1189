...
UniValue ListReceived(CWallet * const pwallet, const UniValue& params, bool fByAccounts)
{
    ...
    std::map<CTxDestination, CAddressBookData>::const_iterator start = pwallet->mapAddressBook.begin();
    std::map<CTxDestination, CAddressBookData>::const_iterator end = pwallet->mapAddressBook.end();
 ...
 */
bool ListTransactions(CWallet * const pwallet, const CWalletTx& wtx, const string& strAccount, int nMinDepth, bool fLong, UniValue& ret, const isminefilter& filter)
{
    CAmount nFee;
    string strSentAccount;
    list<COutputEntry> listReceived;
...
UniValue listsinceblock(const JSONRPCRequest& request)
{
    ...
    while (include_removed && paltindex && paltindex != pindex)
    {
        ...
        {
            if (listed.count(tx->GetHash()) == 0 && pwallet->mapWallet.count(tx->GetHash()) > 0)
            {
