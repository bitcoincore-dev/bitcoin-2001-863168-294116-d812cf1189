...
UniValue bumpfee(const JSONRPCRequest& request)
{
    ...
    BumpFeeResult res = FeeBumper::CreateTransaction(pwallet, hash, newConfirmTarget, ignoreGlobalPayTxFee, totalFee, replaceable, fee_mode, errors, oldFee, newFee, mtx);
    ...
    uint256 txid;
    if (FeeBumper::CommitTransaction(pwallet, hash, std::move(mtx), errors, txid) != BumpFeeResult::OK) {
        throw JSONRPCError(RPC_WALLET_ERROR, errors[0]);
    }
    UniValue result(UniValue::VOBJ);
    result.push_back(Pair("txid", txid.GetHex()));
    result.push_back(Pair("origfee", ValueFromAmount(oldFee)));
    result.push_back(Pair("fee", ValueFromAmount(newFee)));
