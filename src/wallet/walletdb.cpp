...
#endif
        return {util::Error{Untranslated(strprintf("Failed to open database path '%s'. Build does not support SQLite database format.", fs::PathToString(path)))}, DatabaseError::FAILED_BAD_FORMAT};
    }

#ifdef USE_BDB
    return MakeBerkeleyDatabase(path, options);
#endif
    return {util::Error{Untranslated(strprintf("Failed to open database path '%s'. Build does not support Berkeley DB database format.", fs::PathToString(path)))}, DatabaseError::FAILED_BAD_FORMAT};
}
} // namespace wallet
