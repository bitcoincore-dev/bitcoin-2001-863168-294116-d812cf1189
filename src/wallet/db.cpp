...
void CheckUniqueFileid(const BerkeleyEnvironment& env, const std::string& filename, Db& db)
{
    ...
    if (ret != 0) {
        throw std::runtime_error(strprintf("BerkeleyCheckpoint: Can't open database %s (get_fileid failed with %d)", filename, ret));
    }
    ...
    for (const auto& item : env.mapDb) {
        ...
        if (item.second && item.second->get_mpf()->get_fileid(item_fileid) == 0 &&
            ...
            throw std::runtime_error(strprintf("BerkeleyCheckpoint: Can't open database %s (duplicates fileid %s from %s)", filename,
...
bool BerkeleyCheckpoint::Recover(const std::string& filename, void *callbackDataIn, bool (*recoverKVcallback)(void* callbackData, CDataStream ssKey, CDataStream ssValue), std::string& newFilename)
{
    // Recovery procedure:
...
bool BerkeleyCheckpoint::VerifyEnvironment(const std::string& walletFile, const fs::path& walletDir, std::string& errorStr)
{
...
bool BerkeleyCheckpoint::VerifyDatabaseFile(const std::string& walletFile, const fs::path& walletDir, std::string& warningStr, std::string& errorStr, BerkeleyEnvironment::recoverFunc_type recoverFunc)
{
...
BerkeleyCheckpoint::BerkeleyCheckpoint(BerkeleyDatabase& dbw, const char* pszMode, bool fFlushOnCloseIn) : pdb(nullptr), activeTxn(nullptr)
{
...

    {
        ...
        if (!env->Open(GetWalletDir()))
            throw std::runtime_error("BerkeleyCheckpoint: Failed to open database environment.");
        ...
        if (pdb == nullptr) {
            ...
            if (fMockDb) {
                ...
                if (ret != 0) {
                    throw std::runtime_error(strprintf("BerkeleyCheckpoint: Failed to configure for no temp file backing for database %s", strFilename));
                }
            }
            ...
            if (ret != 0) {
                throw std::runtime_error(strprintf("BerkeleyCheckpoint: Error %d, can't open database %s", ret, strFilename));
            }
...
bool BerkeleyCheckpoint::Rewrite(BerkeleyDatabase& dbw, const char* pszSkip)
{
    if (dbw.IsDummy()) {
        return true;
    }
    ...
    while (true) {
        {
            ...
            if (!env->mapFileUseCount.count(strFile) || env->mapFileUseCount[strFile] == 0) {
                ...
                bool fSuccess = true;
                LogPrintf("BerkeleyCheckpoint::Rewrite: Rewriting %s...\n", strFile);
                std::string strFileRes = strFile + ".rewrite";
                { // surround usage of db with extra {}
                    BerkeleyCheckpoint db(dbw, "r");
                    ...
                    int ret = pdbCopy->open(nullptr,               // Txn pointer
                                            ...
                                            0);
                    if (ret > 0) {
                        LogPrintf("BerkeleyCheckpoint::Rewrite: Can't create database file %s\n", strFileRes);
                        fSuccess = false;
                    }
                ...
                if (!fSuccess)
                    LogPrintf("BerkeleyCheckpoint::Rewrite: Failed to rewrite database file %s\n", strFileRes);
                return fSuccess;
            }
        }
        MilliSleep(100);
...
bool BerkeleyCheckpoint::PeriodicFlush(BerkeleyDatabase& dbw)
{
    if (dbw.IsDummy()) {
        return true;
    }
...
{
    return BerkeleyCheckpoint::Rewrite(*this, pszSkip);
}
