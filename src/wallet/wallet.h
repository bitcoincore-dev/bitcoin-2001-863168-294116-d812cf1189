...
public:
     ...
     */
    int GetDepthInMainChain(interfaces::Chain::Lock& locked_chain, const CBlockIndex* &pindexRet) const;
    int GetDepthInMainChain(interfaces::Chain::Lock& locked_chain) const { const CBlockIndex *pindexRet; return GetDepthInMainChain(locked_chain, pindexRet); }
    bool IsInMainChain(interfaces::Chain::Lock& locked_chain) const { const CBlockIndex *pindexRet; return GetDepthInMainChain(locked_chain, pindexRet) > 0; }
...
public:
    ...
    CAmount GetAvailableCredit(interfaces::Chain::Lock& locked_chain, bool fUseCache=true) const;
...
private:
    ...
    void SyncTransaction(interfaces::Chain::Lock& locked_chain, const CTransactionRef& tx, const CBlockIndex *pindex = nullptr, int posInBlock = 0);
...
public:
    ...
    CWallet(interfaces::Chain* chain, std::string name, std::unique_ptr<CWalletDBWrapper> dbw) : m_chain(chain), m_name(std::move(name)), dbw(std::move(dbw))
    {
    }
     ...
     */
    std::map<CTxDestination, std::vector<COutput>> ListCoins(interfaces::Chain::Lock& locked_chain) const;
    ...
    bool IsSpent(interfaces::Chain::Lock& locked_chain, const uint256& hash, unsigned int n) const;
    ...
    void GetKeyBirthTimes(interfaces::Chain::Lock& locked_chain, std::map<CTxDestination, int64_t> &mapKeyBirth) const;
    ...
    bool AddToWalletIfInvolvingMe(interfaces::Chain::Lock& locked_chain, const CTransactionRef& tx, const CBlockIndex* pIndex, int posInBlock, bool fUpdate);
     ...
     */
    bool FundTransaction(interfaces::Chain::Lock& locked_chain, CMutableTransaction& tx, CAmount& nFeeRet, int& nChangePosInOut, std::string& strFailReason, bool lockUnspents, const std::set<int>& setSubtractFeeFromOutputs, CCoinControl);
