...
static feebumper::Result PreconditionChecks(interfaces::Chain::Lock& locked_chain, const CWallet* wallet, const CWalletTx& wtx, std::vector<std::string>& errors)
{
                         ...
                         CAmount& old_fee, CAmount& new_fee, CMutableTransaction& mtx)
{
    ...
    const CFeeRate incremental_relay_fee_rate = wallet->chain().getIncrementalRelayFeeRate();
    ...
    if (total_fee > 0) {
        ...
        CAmount requiredFee = wallet->chain().getRequiredTxFee(maxNewTxSize);
        ...
        new_fee = wallet->chain().getMinTxFee(maxNewTxSize, coin_control, nullptr /* FeeCalculation */);
    ...
    const CAmount max_tx_fee = wallet->chain().getMaxTxFee();
    ...
    CFeeRate minMempoolFeeRate = wallet->chain().getMinPoolFeeRate();
    ...
    if (poutput->nValue <= GetDustThreshold(*poutput, wallet->chain().getMaxDiscardFeeRate())) {
...
Result CommitTransaction(CWallet* wallet, const uint256& txid, CMutableTransaction&& mtx, std::vector<std::string>& errors, uint256& bumped_txid)
{
    ...
    if (!wallet->CommitTransaction(tx, std::move(mapValue), oldWtx.vOrderForm, oldWtx.strFromAccount, reservekey, state)) {
