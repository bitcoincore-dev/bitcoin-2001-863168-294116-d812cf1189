...
static auto testChain = ipc::MakeChain(ipc::LOCAL);
static const CWallet testWallet(testChain.get(), ipc::MakeUnique<CWalletDBWrapper>());
...
BOOST_FIXTURE_TEST_CASE(rescan, TestChain100Setup)
{
    ...
    // and new block files.
    {
        ...
        BOOST_CHECK_EQUAL(-1, wallet.ScanForWalletTransactions(*ipc_locked, oldTip->nHeight));
    ...
    // file.
    {
        CWallet wallet(ipc_chain.get(), ipc::MakeUnique<CWalletDBWrapper>());
        AddKey(wallet, coinbaseKey);
        BOOST_CHECK_EQUAL(oldTip->nHeight, wallet.ScanForWalletTransactions(*ipc_locked, oldTip->nHeight));
...
BOOST_FIXTURE_TEST_CASE(coin_mark_dirty_immature_credit, TestChain100Setup)
{
    ...
    BOOST_CHECK_EQUAL(wtx.GetImmatureCredit(*ipc_locked), 0);

    // Invalidate the cached value, add the key, and make sure a new immature
    // credit amount is calculated.
    wtx.MarkDirty();
    wallet.AddKeyPubKey(coinbaseKey, coinbaseKey.GetPubKey());
    BOOST_CHECK_EQUAL(wtx.GetImmatureCredit(*ipc_locked), 50*COIN);
}
...
public:
    ListCoinsTestingSetup()
    {
        CreateAndProcessBlock({}, GetScriptForRawPubKey(coinbaseKey.GetPubKey()));
        ::bitdb.MakeMock();
        wallet.reset(new CWallet(m_ipc_chain.get(), std::unique_ptr<CWalletDBWrapper>(new CWalletDBWrapper(&bitdb, "wallet_test.dat"))));
        bool firstRun;
        wallet->LoadWallet(firstRun);
        AddKey(*wallet, coinbaseKey);
        wallet->ScanForWalletTransactions(*m_ipc_locked, 0 /* start_height */);
    }
...
BOOST_FIXTURE_TEST_CASE(ListCoins, ListCoinsTestingSetup)
{
    ...
    // address.
    auto list = wallet->ListCoins(*m_ipc_locked);
    ...
    list = wallet->ListCoins(*m_ipc_locked);
    BOOST_CHECK_EQUAL(list.size(), 1);
    BOOST_CHECK_EQUAL(boost::get<CKeyID>(list.begin()->first).ToString(), coinbaseAddress);
    ...
    wallet->AvailableCoins(*m_ipc_locked, available);
