...
static auto testChain = interface::MakeChain();
static const CWallet testWallet(testChain.get(), "dummy", CWalletDBWrapper::CreateDummy());
...
BOOST_FIXTURE_TEST_CASE(rescan, TestChain100Setup)
{
    ...
    // and new block files.
    {
        CWallet wallet(chain.get(), "dummy", CWalletDBWrapper::CreateDummy());
        AddKey(wallet, coinbaseKey);
        WalletRescanReserver reserver(&wallet);
        reserver.reserve();
        BOOST_CHECK(wallet.ScanForWalletTransactions(oldTip->GetBlockHash(), {} /* stop block */, reserver, false /* update */).IsNull());
    ...
    // file.
    {
        ...
        WalletRescanReserver reserver(&wallet);
        reserver.reserve();
        BOOST_CHECK(oldTip->GetBlockHash() == wallet.ScanForWalletTransactions(oldTip->GetBlockHash(), {} /* stop block */, reserver, false /* update */));
...
public:
    ListCoinsTestingSetup()
    {
        ...
        g_address_type = OUTPUT_TYPE_DEFAULT;
        g_change_type = OUTPUT_TYPE_DEFAULT;
        wallet = MakeUnique<CWallet>(m_chain.get(), "mock", CWalletDBWrapper::CreateMock());
        bool firstRun;
        wallet->LoadWallet(firstRun);
        AddKey(*wallet, coinbaseKey);
        WalletRescanReserver reserver(wallet.get());
        reserver.reserve();
        wallet->ScanForWalletTransactions(chainActive.Genesis()->GetBlockHash(), {} /* stop block */, reserver, false /* update */);
    }
