...
BOOST_FIXTURE_TEST_CASE(scan_for_wallet_transactions, TestChain100Setup)
{
    ...
    CBlockIndex* oldTip = WITH_LOCK(::cs_main, return m_node.chainman->ActiveChain().Tip());
    WITH_LOCK(::cs_main, m_node.chainman->m_blockman.GetBlockFileInfo(oldTip->GetBlockPos().nFile)->nSize = MAX_BLOCKFILE_SIZE);
    CreateAndProcessBlock({}, GetScriptForRawPubKey(coinbaseKey.GetPubKey()));
    CBlockIndex* newTip = WITH_LOCK(::cs_main, return m_node.chainman->ActiveChain().Tip());
...
BOOST_FIXTURE_TEST_CASE(importwallet_rescan, TestChain100Setup)
{
    ...
    const int64_t BLOCK_TIME = WITH_LOCK(::cs_main, return m_node.chainman->ActiveChain().Tip()->GetBlockTimeMax() + 5);
...
public:
    ...
    {
        CreateAndProcessBlock({}, GetScriptForRawPubKey(coinbaseKey.GetPubKey()));
        wallet = CreateSyncedWallet(*m_node.chain, WITH_LOCK(::cs_main, return m_node.chainman->ActiveChain()), m_args, coinbaseKey);
    }
