...
BOOST_FIXTURE_TEST_CASE(rescan, TestChain100Setup)
{
    ...
    // and new block files.
    {
        CWallet wallet(*chain, "dummy", WalletDatabase::CreateDummy());
    ...
    // after.
    {
        std::shared_ptr<CWallet> wallet = std::make_shared<CWallet>(*chain, "dummy", WalletDatabase::CreateDummy());
        AddWallet(wallet);
        UniValue keys;
        keys.setArray();
        UniValue key;
...
public:
    ...
    {
        CreateAndProcessBlock({}, GetScriptForRawPubKey(coinbaseKey.GetPubKey()));
        wallet = MakeUnique<CWallet>(*m_chain, "mock", WalletDatabase::CreateMock());
        bool firstRun;
