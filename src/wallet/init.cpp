...
std::string GetWalletHelpString(bool showDebug)
{
    ...
    strUsage += HelpMessageOpt("-wallet=<path>", _("Specify wallet database path. Can be specified multiple times to load multiple wallets. Path string can either be a full absolute path, or a plain wallet name that will be looked up in <walletdir>. If the path does not exist, it will be created as a directory containing a wallet.dat file and log files. For backwards compatibility this will also accept names of existing data files in <walletdir>.)"));
...
bool VerifyWallets()
{
    ...
    for (const std::string& walletFile : gArgs.GetArgs("-wallet")) {
        fs::path wallet_arg = walletFile;
        if (!wallet_arg.is_absolute() && wallet_arg.filename() != walletFile) {
            return InitError(strprintf(_("Invalid -wallet path '%s'. -wallet argument should either be an absolute path or a plain filename with no directory separators."), walletFile));
        }
        ...
        fs::path wallet_path = fs::absolute(wallet_arg, GetWalletDir());
        fs::file_type path_type = fs::symlink_status(wallet_path).type();
        if (!(path_type == fs::file_not_found || path_type == fs::directory_file ||
              (path_type == fs::symlink_file && fs::is_directory(wallet_path)) ||
              (path_type == fs::regular_file && wallet_arg.filename() == walletFile))) {
