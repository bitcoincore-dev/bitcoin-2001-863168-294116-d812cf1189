...
UniValue importprunedfunds(const JSONRPCRequest& request)
{
    ...
    if (pwalletMain->IsMine(tx)) {
      pwalletMain->AddToWallet(MakeTransactionRef(std::move(tx)), [&](TxEntry& entry, bool fNew){
          uint256 hashBlock = merkleBlock.header.GetHash();
          if (entry.second.nIndex != (int)txnIndex || entry.second.hashBlock != hashBlock) {
              entry.second.nIndex = txnIndex;
              entry.second.hashBlock = merkleBlock.header.GetHash();
              return true;
          }
          return fNew;
        }, false);
