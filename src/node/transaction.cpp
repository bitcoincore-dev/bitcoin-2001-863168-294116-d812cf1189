...
TransactionError BroadcastTransaction(const CTransactionRef tx, std::string& err_string, const CAmount& max_tx_fee, bool relay, bool wait_callback, const ignore_rejects_type& ignore_rejects)
{
    ...
    if (!mempool.exists(hashTx)) {
        ...
        if (!AcceptToMemoryPool(mempool, state, std::move(tx), &fMissingInputs,
                nullptr /* plTxnReplaced */, ignore_rejects, max_tx_fee)) {
            if (state.IsInvalid()) {
