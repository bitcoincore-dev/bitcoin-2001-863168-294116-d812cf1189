...
private:
    ...
    void FlushChainstateBlockFile(ChainstateRole chainstate_role, bool fFinalize = false, bool finalize_undo = false);
    void FlushUndoFile(int block_file, bool finalize = false);
    bool FindBlockPos(FlatFilePos& pos, unsigned int nAddSize, unsigned int nHeight, ChainstateRole chainstate_role, uint64_t nTime, bool fKnown);
...
public:
    ...
    bool WriteUndoDataForBlock(const CBlockUndo& blockundo, BlockValidationState& state, ChainstateRole role, CBlockIndex& block)
        EXCLUSIVE_LOCKS_REQUIRED(::cs_main);

    /** Store block on disk. If dbp is not nullptr, then it provides the known position of the block within a block file on disk. */
    FlatFilePos SaveBlockToDisk(const CBlock& block, int nHeight, ChainstateRole chainstate_role, const FlatFilePos* dbp);
