...
public:
    ...
    void start() override { m_start(); }
    void interrupt() override { m_interrupt(); m_start(); }
...
public:
    ...
    std::unique_ptr<Handler> attachChain(std::shared_ptr<Notifications> notifications, const CBlockLocator& locator, const NotifyOptions& options, const StartSyncFn& start_sync) override
    {
        ...
        // std::promise to wait for the start_sync callback, so caller is
        // notified about the starting block before this returns.
        ...
        handler->m_sync_thread = std::thread(&util::TraceThread, handler->m_proxy->m_options.thread_name,
            ...
            if (handler->m_interrupt) return;
            kernel::SyncChain(chainman().m_blockman, active.m_chain, start_block_index, notifications, handler->m_interrupt,
                              /*on_sync=*/[proxy = handler->m_proxy] { CallFunctionInValidationInterfaceQueue([proxy] { NotificationsProxy::connect(proxy); }); });
        });
        }
        // Wait for start_sync result and return null if it fails.
        return promise.get_future().get() ? std::move(handler) : nullptr;
    }
