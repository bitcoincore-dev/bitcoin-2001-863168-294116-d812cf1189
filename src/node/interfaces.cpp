...
public:
    ...
    void BlockConnected(const std::shared_ptr<const CBlock>& block, const CBlockIndex* index) override
    {
        ...
        ReadBlockData(index, /*data=*/nullptr, m_options.connect_undo_data ? &undo_data : nullptr, block_info);
    ...
    void BlockDisconnected(const std::shared_ptr<const CBlock>& block, const CBlockIndex* index) override
    {
        interfaces::BlockInfo block_info = MakeBlockInfo(index, block.get());
        CBlockUndo undo_data;
        ReadBlockData(index, /*data=*/nullptr, m_options.disconnect_undo_data ? &undo_data : nullptr, block_info);
    ...
    {
        LOCK(m_mutex);
        bool unregistered = m_state == UNREGISTERED;
        if (unregistered) m_state = REGISTERED;
        return unregistered;
    }
    bool disconnect()
    {
        LOCK(m_mutex);
        bool registered = m_state == REGISTERED;
    ...
    Mutex m_mutex;
    enum { UNREGISTERED, REGISTERED, DISCONNECTED } m_state = UNREGISTERED;
};

class NotificationsHandlerImpl : public Handler
{
public:
    explicit NotificationsHandlerImpl(std::shared_ptr<Chain::Notifications> notifications, const Chain::NotifyOptions& options, bool connect)
        : m_proxy(std::make_shared<NotificationsProxy>(std::move(notifications), options))
    {
        if (connect && m_proxy->connect()) RegisterSharedValidationInterface(m_proxy);
    }
    ...
    void disconnect() override
    {
        m_interrupt();
        if (m_proxy) {
            if (m_proxy->disconnect()) UnregisterSharedValidationInterface(m_proxy);
            m_proxy.reset();
        }
...
public:
    ...
    std::unique_ptr<Handler> attachChain(std::shared_ptr<Notifications> notifications, const CBlockLocator& locator, const NotifyOptions& options) override
    {
        ...
        handler = std::make_unique<NotificationsHandlerImpl>(notifications, options, /*connect=*/false);
    ...
    {
        return std::make_unique<NotificationsHandlerImpl>(std::move(notifications), Chain::NotifyOptions{}, /*connect=*/true);
    }
