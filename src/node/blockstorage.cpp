...
bool BlockManager::LoadBlockIndexDB(const Consensus::Params& consensus_params)
{
    ...
    int max_blockfile_num{WITH_LOCK(cs_LastBlockFile, return this->MaxBlockfileNum())};
...
bool BlockManager::FindBlockPos(FlatFilePos& pos, unsigned int nAddSize, unsigned int nHeight, const Chainstate& chainstate, uint64_t nTime, bool fKnown)
{
    ...
    // do that now.
    if (chain_type == ChainType::ASSUMED && !m_blockfile_cursors[chain_type]) {
    ...
    if (!fKnown) {
        ...
        unsigned int tip_height = tip != nullptr ? tip->nHeight : -1;
