...
bool BlockManager::WriteUndoDataForBlock(const CBlockUndo& blockundo, BlockValidationState& state, ChainstateRole role, CBlockIndex& block)
{
    AssertLockHeld(::cs_main);
...
FlatFilePos BlockManager::SaveBlockToDisk(const CBlock& block, int nHeight, ChainstateRole chainstate_role, const FlatFilePos* dbp)
{
