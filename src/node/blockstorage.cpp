...
bool BlockManager::FindBlockPos(FlatFilePos& pos, unsigned int nAddSize, unsigned int nHeight, const Chainstate& chainstate, uint64_t nTime, bool fKnown)
{
    ...
    if (chain_type == ChainType::ASSUMED && !m_blockfile_cursors[chain_type]) {
        unsigned int snapshot_base_height = *chainstate.m_chainman.GetSnapshotBaseHeight();
    ...
    unsigned int last_blockfile = *m_blockfile_cursors[chain_type];
    ...
    if (!fKnown) {
        ...
        unsigned int tip_height = chainstate.m_chain.Tip()->nHeight;
