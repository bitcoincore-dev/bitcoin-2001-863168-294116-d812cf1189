...
void SetupServerArgs(NodeContext& node, DefaultArgHints hints)
{
    assert(!node.args);
    node.args = &gArgs;
    ...
    argsman.AddArg("-printtoconsole", strprintf("Send trace/debug info to console (default: %u).%s To disable logging to file, set -nodebuglogfile", hints.printtoconsole, hints.gui ? "" : " Disabled when -daemon=1."), ArgsManager::ALLOW_ANY, OptionsCategory::DEBUG_TEST);
    ...
    argsman.AddArg("-server", strprintf("Accept command line and JSON-RPC commands (default: %u)", hints.server), ArgsManager::ALLOW_ANY, OptionsCategory::RPC);
