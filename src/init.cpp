...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-softwareexpiry", strprintf("Stop working after this POSIX timestamp (default: %s)", DEFAULT_SOFTWARE_EXPIRY), true, OptionsCategory::OPTIONS);
#ifndef WIN32
    ...
    gArgs.AddArg("-startupnotify=<cmd>", "Execute command on startup.", false, OptionsCategory::OPTIONS);
    ...
    gArgs.AddArg("-whitelist=<IP address or network>", "Whitelist peers using the given IP address (e.g. 1.2.3.4) or CIDR notated network (e.g. 1.2.3.0/24). Can be specified multiple times."
    ...
    gArgs.AddArg("-zmqpubhashwallettx=<address>", "Enable publish hash wallet transaction in <address>", false, OptionsCategory::ZMQ);
    ...
    gArgs.AddArg("-acceptnonstdtxn", strprintf("Relay and mine \"non-standard\" transactions (%sdefault: %u)", "", strprintf("%u or testnet: %u", !defaultChainParams->RequireStandardDefault(), !testnetChainParams->RequireStandardDefault())), false, OptionsCategory::NODE_RELAY);
    ...
    gArgs.AddArg("-bytespersigopstrict", strprintf("Minimum bytes per sigop in transactions we relay and mine (default: %u)", DEFAULT_BYTES_PER_SIGOP_STRICT), false, OptionsCategory::NODE_RELAY);
...
std::string LicenseInfo()
{
    ...
    const std::string URL_WEBSITE = "<" PACKAGE_URL ">";
