...
bool AppInitMain(Node& node)
{
    ...
    node.chain->setConnman(node.connman.get());

    peerLogic.reset(new PeerLogicValidation(node.connman.get(), node.banman.get(), scheduler, gArgs.GetBoolArg("-enablebip61", DEFAULT_ENABLE_BIP61)));
