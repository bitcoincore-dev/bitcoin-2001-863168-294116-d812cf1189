...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-softwareexpiry", strprintf("Stop working after this POSIX timestamp (default: %s)", DEFAULT_SOFTWARE_EXPIRY), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::OPTIONS);
#ifndef WIN32
    ...
    gArgs.AddArg("-printpriority", strprintf("Log transaction priority and fee per kB when mining blocks (default: %u)", DEFAULT_PRINTPRIORITY), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::DEBUG_TEST);
    ...
    gArgs.AddArg("-uaappend=<cmt>", "Append literal to the user agent string (should only be used for software embedding)", ArgsManager::ALLOW_ANY, OptionsCategory::CONNECTION);
    gArgs.AddArg("-uacomment=<cmt>", "Append comment to the user agent string", ArgsManager::ALLOW_ANY, OptionsCategory::DEBUG_TEST);

    SetupChainParamsBaseOptions();

    gArgs.AddArg("-acceptnonstdtxn", strprintf("Relay and mine \"non-standard\" transactions (%sdefault: %u)", "", strprintf("%u or testnet: %u", !defaultChainParams->RequireStandardDefault(), !testnetChainParams->RequireStandardDefault())), ArgsManager::ALLOW_ANY, OptionsCategory::NODE_RELAY);
    ...
    gArgs.AddArg("-blockprioritysize=<n>", strprintf("Set maximum size of high-priority/low-fee transactions in bytes (default: %d)", DEFAULT_BLOCK_PRIORITY_SIZE), ArgsManager::ALLOW_ANY, OptionsCategory::BLOCK_CREATION);
...
void InitParameterInteraction()
{
    ...
    if (gArgs.IsArgSet("-peercfilters") && !gArgs.IsArgSet("-peerblockfilters")) {
        const bool val = gArgs.GetBoolArg("-peercfilters", DEFAULT_PEERBLOCKFILTERS);
        if (gArgs.SoftSetBoolArg("-peerblockfilters", val)) {
            LogPrintf("%s: parameter interaction: -peercfilters set -> setting -peerblockfilters=%d\n", __func__, val);
        }
    }
}
