...
public:
    ...
    {
        auto call = util::MakeCall(loop, [&]() { return client.getBannedRequest(); });
        return call.send([&]() {
            if (call.response->hasBanMap()) {
                for (const auto& entry : call.response->getBanMap().getEntries()) {
                    banMap.emplace(serialize::Unserialize<CSubNet>(entry.getSubnet()),
                                   serialize::Unserialize<CBanEntry>(entry.getBanEntry()));
                }
                return true;
            }
            return false;
