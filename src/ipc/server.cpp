    ...
    kj::Promise<void> getBanned(GetBannedContext context) override
    {
        ...
        if (impl->getBanned(banmap)) {
            ...
            for (const auto& entry : banmap) {
                resultEntries[i].setSubnet(serialize::ToArray(serialize::Serialize(entry.first)));
                resultEntries[i].setBanEntry(serialize::ToArray(serialize::Serialize(entry.first)));
                ++i;
            }
        }
        return kj::READY_NOW;
