...
interface WalletLoader extends(Chain.ChainClient) $Proxy.wrap("interfaces::WalletLoader") {
    ...
    listWalletDir @5 (context :Proxy.Context) -> (result :List(Text));
    getWallets @6 (context :Proxy.Context) -> (result :List(Wallet));
    handleLoadWallet @7 (context :Proxy.Context, callback :LoadWalletCallback) -> (result :Handler.Handler);
}
