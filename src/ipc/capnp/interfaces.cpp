...
public:
    ...
    bool transactionCanBeBumped(const uint256& txid) override
    {
        ...
        return call.send([&]() { return call.m_response->getValue(); });
    }
        ...
        CMutableTransaction& mtx) override
    {
        auto call = MakeCall(m_loop, [&]() {
            ...
            request.setTxid(FromBlob(txid));
            request.setConfirmTarget(confirm_target);
            request.setIgnoreGlobalPayTxFee(ignore_global_pay_tx_fee);
        ...
        return call.send([&]() {
            ...
            return call.m_response->getValue();
        });
    }
    ...
    {
        auto call = MakeCall(m_loop, [&]() {
            auto request = m_client.isMineRequest();
...
public:
    ...
    {
        auto call = MakeCall(m_loop, [&]() {
            auto request = m_client.appInitRequest();
            return request;
        });
        return call.send([&]() {
            if (call.m_response->hasError()) {
    ...
    CFeeRate estimateSmartFee(int num_blocks, int* answer_found_at_blocks) override
    {
        auto call = MakeCall(m_loop, [&]() {
        ...
        return call.send([&]() {
            if (answer_found_at_blocks) {
                *answer_found_at_blocks = call.m_response->getAnswerFoundAtBlocks();
            }
