...
interface Wallet {
    ...
    isMine @40 (txin :Data, txout :Data) -> (result :Int32);
    getDebit @41 (txin :Data, filter :Int32) -> (result :Int64);
    getCredit @42 (txout :Data, filter :Int32) -> (result :Int64);
    listCoins @43 () -> (result :CoinsList);
    getCoins @44 (outputs :List(Data)) -> (result :List(WalletTxOut));
    hdEnabled @45 () -> (result :Bool);
    handleShowProgress @46 (callback: ShowProgressCallback) -> (handler :Handler);
    handleStatusChanged @47 (callback: StatusChangedCallback) -> (handler :Handler);
    handleAddressBookChanged @48 (callback: AddressBookChangedCallback) -> (handler :Handler);
    handleTransactionChanged @49 (callback: TransactionChangedCallback) -> (handler :Handler);
    handleWatchonlyChanged @50 (callback: WatchonlyChangedCallback) -> (handler :Handler);
}
