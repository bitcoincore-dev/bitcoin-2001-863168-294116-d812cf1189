...
interface Node {
    ...
    getProxy @16 (net :Int32) -> (proxyInfo :Data, result :Bool);
    ...
    executeRpc @46 (command :Text, params :Text) -> (result :Text);
...
interface Wallet {
    ...
    getKey @9 (address :Data) -> (key :Data, result :Bool);
    ...
    getDestValues @19 (prefix :Text) -> (values :List(Text));
    ...
    createTransaction @24 (recipients :List(Recipient), coinControl :CoinControl, sign :Bool) -> (changePos :Int32, fee :Int64, failReason :Text, result :PendingWalletTx);
...
interface PendingWalletTx {
    ...
    commit @2 (mapValue :WalletValueMap, orderForm :WalletOrderForm, fromAccount :Text) -> (state :Text, result :Bool);
}
