...
void CustomBuildField(
    ...
    // Enable if serializeable and if LocalType is not cv or reference qualified.
    // If LocalTyp is cv or reference qualified, it is important to fall back to
    // Priority<0> implementation of this function that strips cv references, to
    // prevent this overload from unduly taking precedence over more specific
    // overloads for particular LocalTypes.
    typename std::enable_if<ipc::capnp::Serializable<LocalType>::value &&
                            std::is_same_v<LocalType, std::remove_cv_t<std::remove_reference_t<LocalType>>>>::type* enable = nullptr)
{
    CDataStream stream(SER_NETWORK, CLIENT_VERSION);
    value.Serialize(stream);
    auto result = output.init(stream.size());
