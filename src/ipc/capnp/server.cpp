...
public:
    ...
    {
        CKey key;
        context.getResults().setResult(impl->getPrivKey(Unserialize<CKeyID>(context.getParams().getAddress()), key));
        BuildKey(context.getResults().initKey(), key);
        return kj::READY_NOW;
    }
    kj::Promise<void> havePrivKey(HavePrivKeyContext context) override
    {
        context.getResults().setResult(impl->havePrivKey(Unserialize<CKeyID>(context.getParams().getAddress())));
        return kj::READY_NOW;
    }
    ...
    {
        auto values = impl->getDestValues(context.getParams().getPrefix());
        auto resultValues = context.getResults().initResult(values.size());
        size_t i = 0;
    ...
    {
        context.getResults().setValue(impl->transactionCanBeBumped(ToBlob<uint256>(context.getParams().getTxHash())));
        return kj::READY_NOW;
    }
    kj::Promise<void> createBumpTransaction(CreateBumpTransactionContext context) override
    {
        ...
        context.getResults().setValue(impl->createBumpTransaction(ToBlob<uint256>(context.getParams().getTxHash()),
            context.getParams().getConfirmTarget(), context.getParams().getIgnoreUserSetFee(),
            context.getParams().getTotalFee(), context.getParams().getReplaceable(), errors, oldFee, newFee, mtx));
        auto resultErrors = context.getResults().initErrors(errors.size());
        size_t i = 0;
        for (const auto& error : errors) {
            resultErrors.set(i, error);
            ++i;
        }
        context.getResults().setOldFee(oldFee);
        context.getResults().setNewFee(newFee);
    ...
    kj::Promise<void> signBumpTransaction(SignBumpTransactionContext context) override
    {
        ...
        context.getResults().setValue(impl->signBumpTransaction(mtx));
        context.getResults().setMtx(ToArray(Serialize(mtx)));
        return kj::READY_NOW;
    }
    kj::Promise<void> commitBumpTransaction(CommitBumpTransactionContext context) override
    {
        ...
        context.getResults().setValue(impl->commitBumpTransaction(ToBlob<uint256>(context.getParams().getTxHash()),
            Unserialize<CMutableTransaction>(context.getParams().getMtx()), errors, bumpedTxHash));
        auto resultErrors = context.getResults().initErrors(errors.size());
        size_t i = 0;
        for (const auto& error : errors) {
            resultErrors.set(i, error);
        ...
        context.getResults().setBumpedTxHash(FromBlob(bumpedTxHash));
        return kj::READY_NOW;
    }
...
public:
    ...
    kj::Promise<void> getUnspentOutput(GetUnspentOutputContext context) override
    {
        Coin coin;
        context.getResults().setResult(
            impl->getUnspentOutput(Unserialize<COutPoint>(context.getParams().getOutput()), coin));
