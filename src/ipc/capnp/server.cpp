...
public:
    ...
    {
        WalletValueMap mapValue;           // TODO context.getParams().getMapValue()
        WalletOrderForm orderForm;         // TODO context.getParams().getOrderForm()
        CValidationState* state = nullptr; // TODO replace CValidationState with error message string
        context.getResults().setResult(
            impl->commit(std::move(mapValue), std::move(orderForm), context.getParams().getFromAccount(), *state));
        // context.getResults().setState(state); TODO
        return kj::READY_NOW;
    }
...
public:
    ...
    kj::Promise<void> getKey(GetKeyContext context) override
    {
        ...
        // TODO context.getResults().setKey(ToArray(Serialize(key)));
        return kj::READY_NOW;
    }
    ...
    {
        CTxDestination dest; // TODO context.getParams().getDest()
        context.getResults().setResult(
    ...
    kj::Promise<void> getAddresses(GetAddressesContext context) override
    {
        ...
        context.getResults().initResult(addresses.size());
        // TODO fill
        return kj::READY_NOW;
    }
    ...
    kj::Promise<void> getDestValues(GetDestValuesContext context) override
    {
        ...
        context.getResults().initValues(values.size()); // TODO fill
        return kj::READY_NOW;
    }
    ...
    kj::Promise<void> listLockedCoins(ListLockedCoinsContext context) override
    {
        ...
        context.getResults().initOutputs(outputs.size()); // TODO
        return kj::READY_NOW;
    }
    kj::Promise<void> createTransaction(CreateTransactionContext context) override
    {
        std::vector<CRecipient>* recipients = nullptr; // TODO context.getParams().getRecipients()
        int changePos;
        CAmount fee;
        std::string failReason;
        const CCoinControl* coinControl = nullptr; // TODO context.getParams().getCoinControl()
        context.getResults().setResult(messages::PendingWalletTx::Client(
            kj::heap<PendingWalletTxServer>(loop, impl->createTransaction(*recipients, coinControl,
                                                      context.getParams().getSign(), changePos, fee, failReason))));
    ...
    {
        auto walletTx = impl->getWalletTx(ToBlob<uint256>(context.getParams().getTxHash()));
        context.getResults().initResult(); // TODO
        return kj::READY_NOW;
    }
    kj::Promise<void> getWalletTxs(GetWalletTxsContext context) override
    {
        auto walletTxs = impl->getWalletTxs();
        context.getResults().initResult(walletTxs.size()); // TODO
        return kj::READY_NOW;
    }
    kj::Promise<void> tryGetTxStatus(TryGetTxStatusContext context) override
    {
        ...
        // TODO context.getResults().setTxStatus(txStatus);
    ...
    kj::Promise<void> getWalletTxDetails(GetWalletTxDetailsContext context) override
    {
        ...
        // TODO context.getResults().setResult(walletTx);
        // TODO context.getResults().setTxStatus(txStatus);
        // TODO context.getResults().setOrderForm(orderForm);
    ...
    kj::Promise<void> tryGetBalances(TryGetBalancesContext context) override
    {
        ...
        // context.getResults().setBalances(balances);
        context.getResults().setNumBlocks(numBlocks);
        return kj::READY_NOW;
    }
    ...
    {
        CCoinControl* coinControl = nullptr; // TODO context.getParams().getCoinControl()
    ...
    {
        std::vector<COutPoint> outputs; // context.getParams().getOutputs()
        auto coins = impl->getCoins(outputs);
        context.getResults().initResult(coins.size());
        return kj::READY_NOW;
    }
    ...
    kj::Promise<void> handleAddressBookChanged(HandleAddressBookChangedContext context) override
    {
        SetHandler(context, [this](messages::AddressBookChangedCallback::Client& client) {
            return impl->handleAddressBookChanged([this, &client](const CTxDestination& address,
                ...
                auto call = MakeCall(this->loop, [&]() {
                    auto request = client.callRequest();
                    // TODO request.setAddress(address);
    ...
    kj::Promise<void> handleTransactionChanged(HandleTransactionChangedContext context) override
    {
        SetHandler(context, [this](messages::TransactionChangedCallback::Client& client) {
            return impl->handleTransactionChanged([this, &client](const uint256& hashTx, ChangeType status) {
                auto call = MakeCall(this->loop, [&]() {
                    auto request = client.callRequest();
                    request.setHashTx(ToArray(hashTx));
                    request.setStatus(status);
                    return request;
                });
                return call.send();
...
public:
    ...
    kj::Promise<void> getBanned(GetBannedContext context) override
    {
        ...
        if (impl->getBanned(banmap)) {
            auto resultEntries = context.getResults().initBanMap().initEntries(banmap.size());
            size_t i = 0;
    ...
    {
        context.getResults().setResult(
            impl->executeRpc(ToString(context.getParams().getCommand()), ToString(context.getParams().getParams()))
                .write());
        return kj::READY_NOW;
    }
    kj::Promise<void> listRpcCommands(ListRpcCommandsContext context) override
    {
        ...
        context.getResults().initResult(commands.size()); // TODO
        return kj::READY_NOW;
    }
