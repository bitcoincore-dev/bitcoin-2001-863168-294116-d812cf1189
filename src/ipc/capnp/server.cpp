...
public:
    ...
    {
        CPubKey pub_key;
        context.getResults().setResult(m_impl->getKeyFromPool(context.getParams().getInternal(), pub_key));
    ...
    kj::Promise<void> createTransaction(CreateTransactionContext context) override
    {
        ...
        if (auto tx = m_impl->createTransaction(
                recipients, coin_control, context.getParams().getSign(), change_pos, fee, fail_reason)) {
            context.getResults().setResult(
    ...
    {
        context.getResults().setResult(m_impl->transactionCanBeBumped(ToBlob<uint256>(context.getParams().getTxid())));
        return kj::READY_NOW;
    }
    kj::Promise<void> createBumpTransaction(CreateBumpTransactionContext context) override
    {
        ...
        context.getResults().setResult(m_impl->createBumpTransaction(ToBlob<uint256>(context.getParams().getTxid()),
            coin_control, context.getParams().getTotalFee(), errors, old_fee, new_fee, mtx));
    ...
    kj::Promise<void> signBumpTransaction(SignBumpTransactionContext context) override
    {
        ...
        context.getResults().setResult(m_impl->signBumpTransaction(mtx));
        context.getResults().setMtx(ToArray(Serialize(mtx)));
        return kj::READY_NOW;
    }
    kj::Promise<void> commitBumpTransaction(CommitBumpTransactionContext context) override
    {
        std::vector<std::string> errors;
        uint256 bumped_txid;
        context.getResults().setResult(m_impl->commitBumpTransaction(ToBlob<uint256>(context.getParams().getTxid()),
    ...
    kj::Promise<void> txinIsMine(TxinIsMineContext context) override
    {
        context.getResults().setResult(m_impl->txinIsMine(Unserialize<CTxIn>(context.getParams().getTxin())));
        return kj::READY_NOW;
    }
    kj::Promise<void> txoutIsMine(TxoutIsMineContext context) override
    {
        context.getResults().setResult(m_impl->txoutIsMine(Unserialize<CTxOut>(context.getParams().getTxout())));
        return kj::READY_NOW;
    }
...
public:
    ...
    kj::Promise<void> baseInitialize(BaseInitializeContext context) override
    {
        context.releaseParams();
        return m_loop.async(kj::mvCapture(context, [this](BaseInitializeContext context) {
            try {
                context.getResults().setResult(m_impl->baseInitialize());
            } catch (const std::exception& e) {
                context.getResults().setError(e.what());
            }
        }));
    }
    kj::Promise<void> appInitMain(AppInitMainContext context) override
    {
        context.releaseParams();
        return m_loop.async(kj::mvCapture(context, [this](AppInitMainContext context) {
            try {
                context.getResults().setResult(m_impl->appInitMain());
            } catch (const std::exception& e) {
                context.getResults().setError(e.what());
            }
        }));
    ...
    kj::Promise<void> interruptInit(InterruptInitContext context) override
    {
        context.getResults().setResult(m_impl->interruptInit());
        return kj::READY_NOW;
    }
    ...
    kj::Promise<void> getNodesStats(GetNodesStatsContext context) override
    {
        ...
        context.getResults().setResult(m_impl->getNodesStats(stats));
    ...
    {
        banmap_t banmap;
        context.getResults().setResult(m_impl->getBanned(banmap));
    ...
    kj::Promise<void> getMinimumFee(GetMinimumFeeContext context) override
    {
        ...
        FeeReason reason;
        context.getResults().setResult(
            m_impl->getMinimumFee(context.getParams().getTxBytes(), coin_control, &returned_target, &reason));
        context.getResults().setReturnedTarget(returned_target);
        context.getResults().setReason(int32_t(reason));
        return kj::READY_NOW;
    }
    ...
    {
        int returned_target;
        context.getResults().setResult(ToArray(Serialize(m_impl->estimateSmartFee(
            context.getParams().getNumBlocks(), context.getParams().getConservative(), &returned_target))));
        context.getResults().setReturnedTarget(returned_target);
        return kj::READY_NOW;
    }
    ...
    kj::Promise<void> executeRpc(ExecuteRpcContext context) override
    {
        ...
        try {
            BuildUniValue(context.getResults().initResult(),
                m_impl->executeRpc(ToString(context.getParams().getCommand()), params, context.getParams().getUri()));
