...
public:
    ...
    std::vector<std::string> getDestValues(const std::string& prefix) override { return wallet.GetDestValues(prefix); }
    ...
    bool transactionCanBeBumped(const uint256& txHash) { return FeeBumper::transactionCanBeBumped(&wallet, txHash); }
    ...
    bool signBumpTransaction(CMutableTransaction& mtx) { return FeeBumper::signTransaction(&wallet, mtx); }
...
public:
    ...
    std::unique_ptr<Wallet> getWallet(size_t index) override { CHECK_WALLET(return index < ::vpwallets.size() ? MakeUnique<WalletImpl>(*::vpwallets[index]) : std::unique_ptr<WalletImpl>()); }
