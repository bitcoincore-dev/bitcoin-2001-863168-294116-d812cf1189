...
public:
    ...
    bool getKeyFromPool(CPubKey& pub_key, bool internal) override
    {
        return m_wallet.GetKeyFromPool(pub_key, internal);
    }
    bool getPubKey(const CKeyID& address, CPubKey& pub_key) override { return m_wallet.GetPubKey(address, pub_key); }
    bool getPrivKey(const CKeyID& address, CKey& key) override { return m_wallet.GetKey(address, key); }
    bool havePrivKey(const CKeyID& address) override { return m_wallet.HaveKey(address); }
    ...
    bool transactionCanBeBumped(const uint256& txid) { return FeeBumper::TransactionCanBeBumped(&m_wallet, txid); }
    bool createBumpTransaction(const uint256& txid,
        int confirm_target,
        bool ignore_global_pay_tx_fee,
        CAmount total_fee,
        bool replaceable,
    ...
    {
        return FeeBumper::CreateTransaction(&m_wallet, txid, confirm_target, ignore_global_pay_tx_fee, total_fee,
                   replaceable, FeeEstimateMode::UNSET, errors, old_fee, new_fee, mtx) == BumpFeeResult::OK;
    }
    bool signBumpTransaction(CMutableTransaction& mtx) { return FeeBumper::SignTransaction(&m_wallet, mtx); }
    bool commitBumpTransaction(const uint256& txid,
        CMutableTransaction&& mtx,
        std::vector<std::string>& errors,
        uint256& bumped_txid)
    {
        return FeeBumper::CommitTransaction(&m_wallet, txid, std::move(mtx), errors, bumped_txid) == BumpFeeResult::OK;
    }
...
public:
    ...
    CAmount getMinimumFee(unsigned int tx_bytes, int confirm_target, bool conservative) override
    {
        CHECK_WALLET(return CWallet::GetMinimumFee(tx_bytes, confirm_target, ::mempool, ::feeEstimator,
            nullptr /* FeeCalculation */, false /* ignoreGlobalPayTxFee */, conservative));
    }
    CAmount getMaxTxFee() override { return ::maxTxFee; }
    CFeeRate estimateSmartFee(int num_blocks, bool conservative, int* answer_found_at_blocks = nullptr) override
    {
        FeeCalculation fee_calc;
        CFeeRate result = ::feeEstimator.estimateSmartFee(num_blocks, &fee_calc, ::mempool, conservative);
        if (answer_found_at_blocks) {
            *answer_found_at_blocks = fee_calc.returnedTarget;
        }
        return result;
    }
