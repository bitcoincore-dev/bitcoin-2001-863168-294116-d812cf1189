...
QString TransactionDesc::toHTML(ipc::Node& ipc_node, ipc::Wallet& ipc_wallet, TransactionRecord *rec, int unit)
{
    ...
    if (ipc_node.getLogCategories() != BCLog::NONE)
    {
        ...
        for (const CTxIn& txin : wtx.tx->vin)
            if(ipc_wallet.isMine(txin))
                strHTML += "<b>" + tr("Debit") + ":</b> " + BitcoinUnits::formatHtmlWithUnit(unit, -ipc_wallet.getDebit(txin, ISMINE_ALL)) + "<br>";
        for (const CTxOut& txout : wtx.tx->vout)
            if(ipc_wallet.isMine(txout))
                strHTML += "<b>" + tr("Credit") + ":</b> " + BitcoinUnits::formatHtmlWithUnit(unit, ipc_wallet.getCredit(txout, ISMINE_ALL)) + "<br>";
            ...
            {
                {
                    ...
                    strHTML = strHTML + " IsMine=" + (ipc_wallet.isMine(vout) & ISMINE_SPENDABLE ? tr("true") : tr("false")) + "</li>";
                    strHTML = strHTML + " IsWatchOnly=" + (ipc_wallet.isMine(vout) & ISMINE_WATCH_ONLY ? tr("true") : tr("false")) + "</li>";
                }
            }
        }
