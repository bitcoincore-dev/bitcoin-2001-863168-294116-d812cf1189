...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog* dialog)
{
    ...
    if (nQuantity > 0)
    {
        ...
        // Fee
        nPayFee = model->getIpcNode().getMinimumFee(nBytes, coinControl->nConfirmTarget, conservative_estimate);
    ...
    if (model->getIpcNode().getPayTxFee().GetFeePerK() > 0)
        ...
        dFeeVary = (double)std::max(model->getIpcNode().getRequiredFee(1000), model->getIpcNode().estimateSmartFee(coinControl->nConfirmTarget, conservative_estimate).GetFeePerK()) / 1000;
    }
