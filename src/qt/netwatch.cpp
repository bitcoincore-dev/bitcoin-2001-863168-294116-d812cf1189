...
{
    if (m_client_model) {
        disconnect(m_client_model->getOptionsModel(), SIGNAL(displayUnitChanged(int)), this, SLOT(updateDisplayUnit()));
    }
    ...
    QWidget(parent)
{
    ...
    connect(m_search_editor, SIGNAL(textChanged(const QString&)), this, SLOT(doSearch(const QString&)));

    connect(log_model, SIGNAL(rowsRemoved(const QModelIndex&, int, int)), this, SLOT(rowsRemoved(const QModelIndex&, int, int)));
    connect(log_model, SIGNAL(rowsAboutToBeInserted(const QModelIndex&, int, int)), this, SLOT(aboutToInsert()));
    connect(log_model, SIGNAL(rowsInserted(const QModelIndex&, int, int)), this, SLOT(maybeScrollToBottom()));
    connect(log_model, SIGNAL(moreSearchResults(const QList<int>&)), this, SLOT(moreSearchResults(const QList<int>&)));

    connect(m_log_view->selectionModel(), SIGNAL(selectionChanged(const QItemSelection &, const QItemSelection &)), this, SLOT(maybeCancelSearch()));
