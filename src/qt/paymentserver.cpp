...
//
void PaymentServer::ipcParseCommandLine(ipc::Node& ipcNode, int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        if (arg.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // bitcoin: URI
        {
            ...
            if (GUIUtil::parseBitcoinURI(arg, &r) && !r.address.isEmpty())
            {
                ...
                {
                    ipcNode.selectParams(CBaseChainParams::MAIN);
                }
                else {
                    tempChainParams = CreateChainParams(CBaseChainParams::TESTNET);
                    if (address.IsValid(*tempChainParams))
                        ipcNode.selectParams(CBaseChainParams::TESTNET);
                }
            }
        }
...
bool PaymentServer::verifyNetwork(ipc::Node& ipcNode, const payments::PaymentDetails& requestDetails)
{
    bool fVerified = requestDetails.network() == ipcNode.getNetwork();
    if (!fVerified) {
        qWarning() << QString("PaymentServer::%1: Payment request network \"%2\" doesn't match client network \"%3\".")
            .arg(__func__)
            .arg(QString::fromStdString(requestDetails.network()))
            .arg(QString::fromStdString(ipcNode.getNetwork()));
    }
    return fVerified;
}
