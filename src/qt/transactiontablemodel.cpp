...
public:
        ...
        cachedWallet.clear();
        {
            ...
            for(std::map<uint256, CWalletTx>::iterator it = wallet->mapWallet.begin(); it != wallet->mapWallet.end(); ++it)
            {
                if(TransactionRecord::showTransaction(it->second))
                    cachedWallet.append(TransactionRecord::decomposeTransaction(*ipc_locked, wallet, it->second));
            }
        }
    }
        ...
        case CT_NEW:
            ...
            if(showTransaction)
            {
                ...
                QList<TransactionRecord> toInsert =
                        TransactionRecord::decomposeTransaction(*ipc_locked, wallet, mi->second);
    ...
    TransactionRecord *index(int idx)
    {
        if(idx >= 0 && idx < cachedWallet.size())
        {
            ...
            if (auto ipc_locked = wallet->ipc_chain().lockState(true /* try_lock */))
            {
                ...
                if(lockWallet && rec->statusUpdateNeeded())
                {
                    ...
                    if(mi != wallet->mapWallet.end())
                    {
                        rec->updateStatus(*ipc_locked, mi->second);
                    }
                }
            }
            return rec;
