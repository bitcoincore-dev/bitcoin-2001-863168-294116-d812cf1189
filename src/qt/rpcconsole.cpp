...
class RPCExecutor : public QObject
{
    Q_OBJECT
public:
    RPCExecutor(ipc::Node& ipc_node) : m_ipc_node(ipc_node) {}
 ...
 *
 * @param[in]    ipc_node    optional node to execute command on
...
bool RPCConsole::RPCParseCommandLine(ipc::Node* ipc_node, std::string &strResult, const std::string &strCommand, const bool fExecute, std::string * const pstrFilteredOut)
{
...
#endif
                            lastResult = ipc_node->executeRpc(stack.back()[0],RPCConvertValues(stack.back()[0], std::vector<std::string>(stack.back().begin() + 1, stack.back().end())), uri);
                        }
...
void RPCExecutor::request(const QString &command)
{
    try
    {
        ...
        if(!RPCConsole::RPCExecuteCommandLine(m_ipc_node, result, executableCommand))
        {
...
RPCConsole::RPCConsole(ipc::Node& ipc_node, const PlatformStyle *_platformStyle, QWidget *parent) :
    QWidget(parent),
    m_ipc_node(ipc_node),
    ...
    consoleFontSize(0)
{
    ...
    m_ipc_node.rpcSetTimerInterfaceIfUnset(rpcTimerInterface);
...
RPCConsole::~RPCConsole()
{
    ...
    m_ipc_node.rpcUnsetTimerInterface(rpcTimerInterface);
...
void RPCConsole::setClientModel(ClientModel *model)
{
    ...
    if (model && clientModel->getPeerTableModel() && clientModel->getBanTableModel()) {
        ...
        std::vector<std::string> commandList = m_ipc_node.listRpcCommands();
...
{
    RPCExecutor *executor = new RPCExecutor(m_ipc_node);
...
void RPCConsole::unbanSelectedNode()
{
    ...
    for(int i = 0; i < nodes.count(); i++)
    {
        ...
        if (possibleSubnet.IsValid() && m_ipc_node.unban(possibleSubnet))
        {
            clientModel->getBanTableModel()->refresh();
        }
    }
