...
bool RPCConsole::RPCExecuteCommandLine(std::string &strResult, const std::string &strCommand, CRPCRequestInfo& reqinfo)
{
                ...
                switch(ch)
            {
                ...
                case '(': case ')': case '\n':
                    ...
                    if ((ch == ')' || ch == '\n') && stack.size() > 0)
                    {
                        ...
                        lastResult = tableRPC.execute(stack.back()[0], RPCConvertValues(stack.back()[0], std::vector<std::string>(stack.back().begin() + 1, stack.back().end())), reqinfo);
...
#endif
        if(!RPCConsole::RPCExecuteCommandLine(result, executableCommand, reqinfo))
        {
...
void RPCConsole::updateNodeDetail(const CNodeCombinedStats *stats)
{
    ...
    ui->peerMinPing->setText(GUIUtil::formatPingTime(stats->nodeStats.dPingMin));
...
void RPCConsole::disconnectSelectedNode()
{
    ...
    for(int i = 0; i < nodes.count(); i++)
    {
        ...
        if (CNode *bannedNode = FindNode(id)) {
