...
struct PairValueEmplace
{
    ...
    {
        auto value_accessor =
            ProxyStruct<typename Plain<Reader>::MessageType>::get(std::integral_constant<size_t, 1>());
        FieldReader<Plain<decltype(m_reader.get())>, decltype(value_accessor)> value_reader{
            m_reader.get(), value_accessor, m_reader.loop()};
        ReadField(TypeList<Value>(), value_reader,
...
void ReadFieldNew(TypeList<std::pair<Key, Value>>, Reader&& reader, Emplace&& emplace)
{
    ...
    */
    auto key_accessor = ProxyStruct<typename Plain<Reader>::MessageType>::get(std::integral_constant<size_t, 0>());
    FieldReader<Plain<decltype(reader.get())>, decltype(key_accessor)> key_reader{
        reader.get(), key_accessor, reader.loop()};
    ReadField(
        TypeList<Key>(), key_reader, MakeTupleEmplace(PairValueEmplace<Value, Reader, Emplace>{reader, emplace}));
}
...
{
    auto key_accessor = ProxyStruct<typename Plain<Reader>::MessageType>::get(std::integral_constant<size_t, 0>());
    FieldReader<Plain<decltype(reader.get())>, decltype(key_accessor)> key_reader{reader.get(), key_accessor, reader.loop()};

    auto value_accessor = ProxyStruct<typename Plain<Reader>::MessageType>::get(std::integral_constant<size_t, 1>());
    FieldReader<Plain<decltype(reader.get())>, decltype(value_accessor)> value_reader{
        reader.get(), value_accessor, reader.loop()};

    ReadField(TypeList<Key>(), key_reader, std::get<0>(tuple));
    ReadField(TypeList<Value>(), value_reader, std::get<1>(tuple));
}
...
template <typename Reader, typename Emplace>
void ReadFieldNew(TypeList<std::exception>, Reader&& reader, Emplace&& emplace)
{
    auto data = reader.get();
...
void PassField(TypeList<FUCK>, TypeList<Value*>, Reader&& reader, Builder&& builder, Fn&& fn, FnParams&&... fn_params)
{
    ...
    if (want) BuildField(TypeList<Value>(), TypeList<FUCK>(), BuildFieldPriority(), reader.loop(), *param, builder);
}
...
void PassField(TypeList<FUCK>, TypeList<Value>, Reader&& reader, Builder&& builder, Fn&& fn, FnParams&&... fn_params)
{
    ...
    BuildField(TypeList<Value>(), TypeList<FUCK>(), BuildFieldPriority(), reader.loop(), *param, builder);
}
...
struct ClientException
{
    ...
    struct ReadResults : FieldChainHelper<ReadResults, 0>
    {
        ...
        void handleField(EventLoop& loop, Results& results, ParamList)
        {
            FieldReader<Results, Accessor> reader(results, m_client_exception->m_accessor, loop);
            if (reader.has()) {
                ReadField(TypeList<Exception>(), reader, ThrowEmplace<Exception>());
            }
        }
...
struct ClientParam
{
    ...
    struct ReadResults : FieldChainHelper<ReadResults, sizeof...(Types)>
    {
        ...
        template <int n, typename ParamList, typename Reader, typename... Args>
        typename std::enable_if<n == argc && 0 < argc>::type readTuple(ParamList, Reader&& reader, Args&&... args)
        {
            ClientReadField(ParamList(), reader, std::forward<Args>(args)...);
        }

        template <int n, typename ParamList, typename Reader>
        typename std::enable_if<n == 0 && argc == 0>::type readTuple(ParamList, Reader&&)
        {
...
struct ServerFieldExcept
{
        ...
        FwdParams&&... fwd_params) const
    {
        ...
        // FIXME Weird rvalue issue static_cast below
        try {
            fn(param_types, loop, params, results, static_cast<FwdParams&>(fwd_params)...);
        } catch (const Exception& e) {
            using FieldType = TypeList<decltype(e)>;
            BuildField(FieldType(), TypeList<typename Builder::Type>(), BuildFieldPriority(), loop, e,
                Builder(results, m_accessor));
        }
    }
    Accessor m_accessor;
    ...
    _Params&&... params)
{
    ...
    m_loop->sync([&]() {
        ...
        m_loop->m_task_set.add(request.send().then([&](::capnp::Response<typename Request::Results>&& response) {
            ...
            try {
                FieldChain().handleChain(*m_loop, response, MethodParams(), typename _Params::ReadResults{&params}...);
            } catch (...) {
                promise.set_exception(std::current_exception());
                return;
            }
