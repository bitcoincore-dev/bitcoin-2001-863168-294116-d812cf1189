    ...
    Builder&& builder)
{
    ...
    BuildField(TypeList<V>(), TypeList<FUCK>(), BuildFieldPriority(), loop, value.second, ValueBuilder<decltype(kv)>{kv});
}
...
{
    auto kv = builder.set();
    BuildField(TypeList<K>(), TypeList<FUCK>(), BuildFieldPriority(), loop, std::get<0>(value), KeyBuilder<decltype(kv)>{kv});
    BuildField(
        TypeList<V>(), TypeList<FUCK>(), BuildFieldPriority(), loop, std::get<1>(value), ValueBuilder<decltype(kv)>{kv});
}
    ...
    typename std::enable_if < index<ProxyType<Param>::fields>::type * enable = nullptr)
{
    ...
    auto&& field_value = value.*ProxyType<Param>::get(Index());
    using FUCK = int;
    BuildField(TypeList<decltype(field_value)>(), TypeList<FUCK>(), BuildFieldPriority(), loop, field_value, field_builder);
