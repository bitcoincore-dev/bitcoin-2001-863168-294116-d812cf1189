...
interface Chain $X.proxy("interface::Chain") {
    ...
    generateBlocks @25 (coinbaseScript :Data, numBlocks :Int32, maxTries:UInt64, keepScript:Bool) -> (result :UniValue) $X.async;
    parseConfirmTarget @26 (value :UniValue) -> (result :UInt32);
    getSpendZeroConfChange @27 () -> (result :Bool);
    getDefaultRbf @28 () -> (result :Bool);
    getDefaultAddressType @29 () -> (result :Int32);
    getDefaultChangeType @30 () -> (result :Int32);
    handleNotifications @31 (notifications :ChainNotifications) -> (result :Handler);
    waitForNotifications @32 () -> () $X.async;
    handleRpc @33 (command :RPCCommand) -> (result :Handler);
}
...
interface Node $X.proxy("interface::Node") {
    ...
    getDefaultAddressType @51 () -> (result :Int32);
    getWallets @52 () -> (result :List(Wallet));
