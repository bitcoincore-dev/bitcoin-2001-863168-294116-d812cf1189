...
void BuildMessage(const CCoinControl& coin_control, messages::CoinControl::Builder&& builder)
{
    ...
    if (coin_control.m_change_type) {
        ...
        builder.setChangeType(*coin_control.m_change_type);
    }
...
void FillGlobalArgs(messages::GlobalArgs::Builder&& builder)
{
    ...
    BuildField(TypeList<decltype(args.mapMultiArgs)>(), TypeList<FUCK>(), BuildFieldPriority(), *loop, args.mapMultiArgs,
