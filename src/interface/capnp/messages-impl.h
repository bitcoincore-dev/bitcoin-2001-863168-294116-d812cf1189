    ...
    Builder&& builder)
{
    ...
    BuildField(TypeList<CNodeStats>(), TypeList<FUCK>(), Priority<2>(), loop, std::get<0>(stats), builder);
    if (std::get<1>(stats)) {
        ...
        BuildField(
            TypeList<CNodeStateStats>(), TypeList<FUCK>(), Priority<2>(), loop, std::get<2>(stats), field_builder);
    }
}
...
void BuildField(TypeList<>, TypeList<FUCK>, Priority<1>, EventLoop& loop, messages::GlobalArgs::Builder&& builder)
{
    ...
    {
        LOCK(args.cs_args);
        // FIXME BuildField(TypeList<decltype(args.mapArgs)>(), Priority<2>(), loop, args.mapArgs, builder.getArgs());
        // FIXME BuildField(TypeList<decltype(args.mapMultiArgs)>(), Priority<2>(), loop, args.mapMultiArgs,
