        ...
        if (!fBound) {
            ...
            if (GetListenPort() != defaultPort) {
                bool fRet = uiInterface.ThreadSafeQuestion(
                    strprintf(_("Do you want to use the standard network port for %s (port %s) instead?").translated, PACKAGE_NAME, defaultPort),
                ...
                if (fRet) {
                    ...
                    struct in_addr inaddr_any;
                    inaddr_any.s_addr = INADDR_ANY;
                    fBound |= Bind(CService(inaddr6_any, defaultPort), BF_NONE, NetPermissionFlags::PF_NONE);
                    fBound |= Bind(CService(inaddr_any, defaultPort), BF_NONE, NetPermissionFlags::PF_NONE);
                }
            }
        }
    }
...
std::vector<CAddress> CConnman::GetAddresses(Optional<Network> requestor_network)
{
    if (requestor_network) {
        ...
        if (m_addr_response_caches.find(network) == m_addr_response_caches.end() ||
            ...
            m_addr_response_caches[network].m_update_addr_response = current_time + std::chrono::hours(21) + GetRandMicros(std::chrono::hours(6));
        }
