    ...
    while (true)
    {
        ...
        for (const AddedNodeInfo& info : vInfo) {
            if (!info.fConnected) {
                ...
                if (!g_interrupt_addcon_thread.sleep_for(std::chrono::milliseconds(500)))
                    return;
            }
        }
        // Retry every 60 seconds if a connection was attempted, otherwise two seconds
        if (!g_interrupt_addcon_thread.sleep_for(std::chrono::seconds(tried ? 60 : 2)))
            return;
    }
}
...
bool CConnman::Start(CScheduler& scheduler, const Options& connOptions)
{
    ...
    g_interrupt_addcon_thread.reset();
    interruptNet.reset();
