...
public:
    ...
    void BlockChecked(const CBlock& block, const BlockValidationState& state) override
        EXCLUSIVE_LOCKS_REQUIRED(!m_peer_mutex);
    void NewPoWValidBlock(ChainstateRole role, const CBlockIndex *pindex, const std::shared_ptr<const CBlock>& pblock) override
...
{
    if (role == ChainstateRole::BACKGROUND) {
        // Background chainstates won't exceed `m_highest_fast_announce`.
        return;
    }
