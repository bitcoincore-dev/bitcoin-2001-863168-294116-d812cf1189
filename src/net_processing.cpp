...
/** Update chainstate_to_last_common_block and add not-in-flight missing successors to vBlocks, until it has
 *  at most count entries. */
static void FindNextBlocksToDownload(const CChainState* const chainstate, NodeId nodeid, unsigned int count, std::vector<const CBlockIndex*>& vBlocks, NodeId& nodeStaller, const Consensus::Params& consensusParams) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    if (count == 0)
        return;
    ...
    if (!ProcessNewBlockHeaders(headers, state, chainparams, &pindexLast, &first_invalid_header)) {
        ...
        if (state.IsInvalid(nDoS)) {
            ...
            if (punish_duplicate_invalid && LookupBlockIndex(first_invalid_header.GetHash())) {
                ...
                // was received, and hence stored in m_block_index) but know the
                // block is invalid, and that a peer has announced that same
                // block as being on its active chain.
                // Disconnect the peer in such a situation.
                //
                // Note: if the header that is invalid was not accepted to our
                // m_block_index at all, that may also be grounds for
...
void static ProcessOrphanTx(CConnman* connman, std::set<uint256>& orphan_work_set, std::list<CTransactionRef>& removed_txn) EXCLUSIVE_LOCKS_REQUIRED(cs_main, g_cs_orphans)
{
    ...
    while (!done && !orphan_work_set.empty()) {
        ...
        mempool.check(::ChainstateActive().CoinsCache());
    }
}
...
bool static ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CConnman* connman, const std::atomic<bool>& interruptMsgProc, bool enable_bip61)
{
    ...
    if (strCommand == NetMsgType::BLOCKTXN)
    {
        ...
        if (fBlockRead) {
            ...
            // Okay to restrict this to the active chainstate because we'll
...
bool PeerLogicValidation::SendMessages(CNode* pto)
{
    const Consensus::Params& consensusParams = Params().GetConsensus();
    {
        ...
        // snapshot.
        for (CChainState* chainstate : g_chainman.GetAllForBlockDownload()) {
