...
void PeerLogicValidation::BlockChecked(const CBlock& block, const BlockValidationState& state) {
    ...
    if (state.IsInvalid() &&
        ...
        State(it->second.first)) {
            CBlockReject reject = {(unsigned char)GetRejectCode(state), state.GetRejectReason().substr(0, MAX_REJECT_MESSAGE_LENGTH), hash};
    ...
    std::vector<uint256> headers(stop_index->nHeight / CFCHECKPT_INTERVAL);
    {
        ...
        for (; i >= 0; i--) {
            ...
            if (!filter_index->LookupFilterHeader(block_index, headers[i])) {
                LogPrint(BCLog::NET, "Failed to find block filter header in index: " /* Continued */
    ...
    if (msg_type == NetMsgType::REJECT)
    {
        if (LogAcceptCategory(BCLog::NET)) {
            try {
                ...
                ss << strMsg << " code " << ToString(ccode) << ": " << strReason;
    ...
    if (msg_type == NetMsgType::TX) {
        ...
        if (state.IsInvalid())
        {
            LogPrint(BCLog::MEMPOOLREJ, "%s from peer=%d was not accepted: %s\n", tx.GetHash().ToString(),
                pfrom->GetId(),
                state.ToString());
            if (enable_bip61 && GetRejectCode(state) > 0 && GetRejectCode(state) < REJECT_INTERNAL) { // Never send AcceptToMemoryPool's internal codes over P2P
                connman->PushMessage(pfrom, msgMaker.Make(NetMsgType::REJECT, msg_type, (unsigned char)GetRejectCode(state),
...
bool PeerLogicValidation::SendRejectsAndMaybeDiscourageAndDisconnect(CNode* pnode, bool enable_bip61)
{
    AssertLockHeld(cs_main);
    CNodeState &state = *State(pnode->GetId());
...
bool PeerLogicValidation::ProcessMessages(CNode* pfrom, std::atomic<bool>& interruptMsgProc)
{
    ...
    LOCK(cs_main);
    SendRejectsAndMaybeDiscourageAndDisconnect(pfrom, m_enable_bip61);
...
bool PeerLogicValidation::SendMessages(CNode* pto)
{
    const Consensus::Params& consensusParams = Params().GetConsensus();
    {
        ...
        if (SendRejectsAndMaybeDiscourageAndDisconnect(pto, m_enable_bip61)) return true;
        ...
        if (pto->m_tx_relay != nullptr && pto->nVersion >= FEEFILTER_VERSION && gArgs.GetBoolArg("-feefilter", DEFAULT_FEEFILTER) &&
            ...
            static FeeFilterRounder filterRounder{CFeeRate{DEFAULT_MIN_RELAY_TX_FEE}};
            if (::ChainstateActive().IsInitialBlockDownload()) {
                // Received tx-inv messages are discarded when the active
                // chainstate is in IBD, so tell the peer to not send them.
                currentFilter = MAX_MONEY;
            } else {
                static const CAmount MAX_FILTER{filterRounder.round(MAX_MONEY)};
