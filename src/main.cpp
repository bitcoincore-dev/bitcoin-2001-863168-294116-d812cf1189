...
static const struct NotoriousFilterEntry NotoriousPrefixes[] = {
    ...
    {0x434e5452, 0x434e5452, "old Counterparty"},
...
bool AcceptToMemoryPoolWorker(CTxMemPool& pool, CValidationState& state, const CTransaction& tx, bool fLimitFree,
                              bool* pfMissingInputs, bool fRejectAbsurdFee, const std::set<std::string>& setIgnoreRejects,
                              std::vector<uint256>& vHashTxnToUncache)
{
    ...
    if (GetArg("-spamfilter", DEFAULT_SPAMFILTER) && setIgnoreRejects.find("spam-notorious") == setIgnoreRejects.end()) {
    ...
    string reason;
    if (fRequireStandard && !IsStandardTx(tx, reason, setIgnoreRejects))
    ...
    if (fRequireStandard && tx.nVersion >= 2 && VersionBitsTipState(chainparams.GetConsensus(), Consensus::DEPLOYMENT_CSV) != THRESHOLD_ACTIVE && setIgnoreRejects.find("premature-version2-tx") == setIgnoreRejects.end()) {
...

    {
        ...
        if ((!CheckSequenceLocks(tx, STANDARD_LOCKTIME_VERIFY_FLAGS, &lp)) && setIgnoreRejects.find("non-BIP68-final") == setIgnoreRejects.end())
        ...
        if (((nSigOps > MAX_STANDARD_TX_SIGOPS) || (nBytesPerSigOp && nSigOps > nSize / nBytesPerSigOp)) && setIgnoreRejects.find("bad-txns-too-many-sigops") == setIgnoreRejects.end()) {
        ...
        if (fLimitFree && nModifiedFees < ::minRelayTxFee.GetFee(nSize))
        {
            ...
            if (dFreeCount >= GetArg("-limitfreerelay", DEFAULT_LIMITFREERELAY) * 10 * 1000 && setIgnoreRejects.find("rate limited free transaction") == setIgnoreRejects.end()) {
        ...
        if (fRejectAbsurdFee && nFees > ::minRelayTxFee.GetFee(nSize) * 10000 && setIgnoreRejects.find(strRejectMsg_AbsurdFee) == setIgnoreRejects.end())
        ...
        if (setConflicts.size())
        {
            ...
            if (nDeltaFees < ::minRelayTxFee.GetFee(nSize) && setIgnoreRejects.find("insufficient-fee") == setIgnoreRejects.end())
            {
                return state.DoS(0,
                        ...
                        bool* pfMissingInputs, bool fRejectAbsurdFee, const std::set<std::string>& setIgnoreRejects)
{
    ...
    bool res = AcceptToMemoryPoolWorker(pool, state, tx, fLimitFree, pfMissingInputs, fRejectAbsurdFee, setIgnoreRejects, vHashTxToUncache);
    if (!res) {
...
bool static DisconnectTip(CValidationState& state, const Consensus::Params& consensusParams)
{
    ...
    BOOST_FOREACH(const CTransaction &tx, block.vtx) {
        ...
        if (tx.IsCoinBase() || !AcceptToMemoryPool(mempool, stateDummy, tx, false, NULL, 0, setIgnoreRejects_mempool_full)) {
