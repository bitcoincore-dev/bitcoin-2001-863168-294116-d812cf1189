...
{
    if (const SettingsValue* value = FindKey(settings.forced_settings, name)) {
        fn(Source(SettingsSpan(value)).SetForced());
    }
    if (const std::vector<SettingsValue>* value = FindKey(settings.command_line_options, name)) {
        fn(Source(SettingsSpan(*value)));
    }
    if (!section.empty()) {
        if (const std::map<std::string, std::vector<SettingsValue>>* map = FindKey(settings.ro_config, section)) {
            if (const std::vector<SettingsValue>* value = FindKey(*map, name)) {
                fn(Source(SettingsSpan(*value)).SetConfigFile(false));
            }
        }
    }
    SettingsSpan config_file_top_level;
    if (const std::map<std::string, std::vector<SettingsValue>>* map = FindKey(settings.ro_config, "")) {
        if (const std::vector<SettingsValue>* value = FindKey(*map, name)) {
            config_file_top_level = SettingsSpan(*value);
        }
    }
    fn(Source(config_file_top_level).SetConfigFile(true));
}
} // namespace
    ...
    bool skip_nonpersistent)
{
    ...
    MergeSettings(settings, section, name, [&](Source source) {
        ...
        // precedence over early settings, but for backwards compatiblity in
    ...
    bool ignore_default_section_config)
{
    ...
    MergeSettings(settings, section, name, [&](Source source) {
        ...
        if (!source.span.empty()) {
            ...
            const bool add_zombie_config_value = source.config_file && !prev_negated_empty;

            if (!prev_negated || add_zombie_config_value) {
