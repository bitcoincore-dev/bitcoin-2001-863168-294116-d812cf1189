...
std::string ArgsManager::GetArg(const std::string& strArg, const std::string& strDefault) const
{
    ...
    return value.isFalse() ? "0" : value.isTrue() ? "1" : value.get_str();
}
...
int64_t ArgsManager::GetArg(const std::string& strArg, int64_t nDefault) const
{
    ...
    return value.isFalse() ? 0 : value.isTrue() ? 1 : value.isNum() ? value.get_int64() : atoi64(value.get_str());
}
...
std::string ArgsManager::GetChainName() const
{
    auto get_net = [&](const std::string& arg) {
        ...
        util::SettingsValue value = GetSetting(m_settings, /* section= */ "", SettingName(arg),
                                               !ArgsManagerHelper::UseDefaultSection(*this, arg),
                                               /* get_chain_name= */ true);
