...
public:
    ...
    /**
     * Raise an exception if a value is being retrieved as an bool/string/int,
     * but the option was registered with an incompatible set of flags. This
     * indicates an internal bug where either the wrong GetArg overload has
     * been called for an option, or the option was registered incorrectly,
     * with flags that don't actually match how the option is used.
     */
