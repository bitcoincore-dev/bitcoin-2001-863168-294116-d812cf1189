                                      ...
                                      std::string& error)
{
    ...
    if (CheckValue(negated, key, has_value, val, flags, error)) {
        ...
        } else if (!val.empty() || !has_value) {
            args[key] = {val}; // otherwise -foo=bar and -foo override previous settings
        } else {
            args.erase(key); // -foo="" resets any previous setting to default
        }
        return true;
    }
    return false;
...
{
    const unsigned int flags = FlagsOfKnownArg(strArg);
