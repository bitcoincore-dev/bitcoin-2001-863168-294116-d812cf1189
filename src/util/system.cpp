...
public:
    ...
    {
        if (!ValidArgStr(arg)) return {};
        LOCK(am.cs_args);
        bool ignore_default_section_config = !ArgsManagerHelper::UseDefaultSection(am, arg);
        return GetSetting(am.m_settings, no_network ? "" : am.m_network, SettingName(arg),
            ignore_default_section_config, /* skip_nonpersistent= */ false);
    }
};
 ...
 */
static bool InterpretNegatedOption(std::string& section, std::string& key, std::string& val)
{
...
const std::set<std::string> ArgsManager::GetUnsuitableSectionOnlyArgs() const
{
    ...
    for (const auto& arg : m_network_only_args) {
        if (HasIgnoredDefaultSectionConfigValue(m_settings, m_network, SettingName(arg))) {
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        // options with section prefixes are allowed but ignored. An improvement
        // would be to use these options, or trigger the IsArgKnown error below.
        if (section.empty()) {
            if (negated) {
                m_settings.command_line_options[key].push_back(false);
            } else {
                m_settings.command_line_options[key].push_back(val);
            }
        }
    ...
    bool result = true;
    if (std::vector<util::SettingsValue>* includes = util::FindKey(m_settings.command_line_options, "includeconf")) {
...
bool ArgsManager::IsArgKnown(const std::string& key) const
{
    ...
    for (const auto& arg_map : m_available_args) {
        if (arg_map.second.count(key)) return true;
    }
    return false;
}
...
std::vector<std::string> ArgsManager::GetArgs(const std::string& strArg) const
{
    ...
    for (const util::SettingsValue& value :
        ...
        result.push_back(value.get_str());
    }
    return result;
}
...
bool ArgsManager::ReadSettingsFile()
{
    ...
    file.open(filepath);
    if (!file.is_open()) {
        LogPrintf("Unable to open settings file %s for reading\n", filepath.string());
        return false;
    }

    util::SettingsValue in;
    if (!in.read(std::string(std::istreambuf_iterator<char>(file), std::istreambuf_iterator<char>()))) {
        LogPrintf("Unable to parse settings file %s\n", filepath.string());
        return false;
    }
    file.close();

    if (in.isObject()) {
        LogPrintf("Settings file %s does not contain valid object.\n", filepath.string());
        return false;
    }
...
bool ArgsManager::WriteSettingsFile() const
{
    ...
    if (!file.is_open()) {
        LogPrintf("Unable to open settings file %s for writing\n", filepath_tmp.string());
        return false;
    }
    ...
    if (!RenameOver(filepath_tmp, filepath)) {
        LogPrintf("Unable to rename settings file %s to %s\n", filepath_tmp.string(), filepath.string());
        return false;
    }
    return true;
}
...
std::string ArgsManager::GetArg(const std::string& strArg, const std::string& strDefault) const
{
    ...
    return value.isNull() ? strDefault : value.isFalse() ? "0" : value.get_str();
}
...
int64_t ArgsManager::GetArg(const std::string& strArg, int64_t nDefault) const
{
    ...
    return value.isNull() ? nDefault :
           ...
           value.isBool() ? value.get_bool() : atoi64(value.get_str());
}
...
bool ArgsManager::ReadConfigStream(std::istream& stream, const std::string& filepath, std::string& error, bool ignore_invalid_keys)
{
    ...
    for (const std::pair<std::string, std::string>& option : options) {
        std::string strKey = option.first;
        std::string strValue = option.second;

        std::string section;
        if (InterpretNegatedOption(section, strKey, strValue)) {
            m_settings.ro_config[section][strKey].push_back(false);
        } else {
            m_settings.ro_config[section][strKey].push_back(strValue);
        }
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    if (stream.good()) {
        ...
        if (emptyIncludeConf) {
            auto add_includes = [&](std::vector<std::string>& includes, const std::string& network, size_t skip = 0) {
                ...
                if (auto* section = util::FindKey(m_settings.ro_config, network)) {
                    if (auto* values = util::FindKey(*section, "includeconf")) {
                        size_t i = std::max(skip, util::SettingsSpan(*values).negated());
                        for (; i < values->size(); ++i) {
                            includes.push_back((*values)[i].get_str());
                        }
                        num_includes = values->size();
                    }
                }
                return num_includes;
            };
            ...
            size_t top_includes = add_includes(includeconf, {});

            // We haven't set m_network yet (that happens in SelectParams()), so manually check
            // for network.includeconf args.
            std::string chain_id = GetChainName();
            size_t chain_includes = add_includes(includeconf, chain_id);
            ...
            std::vector<std::string> warn_includeconf;

            includeconf = GetArgs("-includeconf");
            {
                std::vector<std::string> warn_includeconf;
                add_includes(warn_includeconf, {}, top_includes);
                add_includes(warn_includeconf, chain_id, chain_includes);
                std::string chain_id_final = GetChainName();
                if (chain_id_final != chain_id) {
                    // Also warn about recursive includeconf for the chain that was specified in one of the includeconfs
                    add_includes(warn_includeconf, chain_id_final);
                }
            }
            for (const std::string& to_include : warn_includeconf) {
...
std::string ArgsManager::GetChainName() const
{
    auto get_net = [&](const std::string& arg) {
        util::SettingsValue value = ArgsManagerHelper::Get(*this, arg, /* no_network= */ true);
        return !value.isNull() && !value.isFalse() && InterpretBool(value.get_str());
    };

    bool fRegTest = get_net("-regtest");
    bool fTestNet = get_net("-testnet");
