...
/**
 * Template deduction guide that's slightly different from the default
 * generated deduction guide because it stores a reference to the substream
 * inside ParamsStream instead of a copy. (Storing a copy instead of a
 * reference is possible by specifying template arguments explicitly and
 * bypassing this deduction guide.)
 */
template<typename Substream, typename Params>
ParamsStream(Substream&, const Params&) -> ParamsStream<Substream&, Params>;

/**
 * Template deduction guide for multiple parameter arguments.
 */
